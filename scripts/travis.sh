...
if [[ "$HEADLESS" == "true" ]] ; then
    ...
    # run headless tests
    mvn test -U -P travis-headless --batch-mode -Dsurefire.printSummary=${PRINT_SUMMARY} -Dsurefire.runOrder=${RUN_ORDER} -Dant.jvm.args="-Djava.awt.headless=${HEADLESS}" -Djava.awt.headless=${HEADLESS} -Dcucumber.options="--tags ~@Ignore" 
else
