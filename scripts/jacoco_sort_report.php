...
$url = 'http://builds.jmri.org/jenkins/job/Development/job/JaCoCo/lastStableBuild/jacoco/';
	...
	if ($showDOMTree) {
		...
		foreach ($elements as $element) {
			if ($element->hasAttribute('href')) {
				...
				$element->setAttribute('href', 'http://builds.jmri.org'.$href);
			}
		}
		...
		foreach ($elements as $element) {
			if ($element->hasAttribute('src')) {
				...
				$element->setAttribute('src', 'http://builds.jmri.org'.$src);
			}
		}
		...
		for ($i=1; $i < $tableElement->childNodes->length; $i++) {
			...
			foreach ($elements as $element) {
				if ($element->hasAttribute('href')) {
					...
					if ($isClassList) {		// Class list
						$href = $element->getAttribute('href');
						$element->setAttribute('href', 'http://builds.jmri.org/jenkins/job/Development/job/JaCoCo/lastStableBuild/jacoco/'.$package.'/'.$href);
					} else {				// Package list
