...
```
  ./scripts/update-HOWTO.sh 4.99.4 4.99.5 4.99.5
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.99.4
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.99.4". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.99.4%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 4.99.4" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri4.99.4.shtml jmri4.99.5.shtml
        $EDITOR jmri4.99.5.shtml
        ...
        git add jmri4.99.5.shtml
        git commit -m"start new 4.99.5 next release note" jmri4.99.5.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.99.4.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.99.4.shtml
```

 - Add any new warnings to the old warnings section of the next (4.99.5) release note:
```
        ${EDITOR} ../website/releasenotes/jmri4.99.5.shtml ../website/releasenotes/jmri4.99.4.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri4.99.4.shtml
```
...
```
        ...
        git commit -m"start for 4.99.5 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"for next release 4.99.5" pom.xml
        git push github
```
...
```
The release-4.99.4 branch has been created.

Maintainers, please set the 4.99.5 milestone on pulls from now on, as that will be the next test release.
...
- Click "Copy Existing Item". Fill out the new 4.99.4 release name at the top. Enter the 4.99.2 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  4.99.4
...
- Check the [signing-wget.log](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.4/ws/%20signing-wget.log) file in the job’s workspace to make sure all the return codes are zero. This is the macOS signing process. A non-zero code indicates a problem with the signature.
...
- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%204.99.4%20files%20available) with a subject line

```
"First 4.99.4 files available":

First JMRI 4.99.4 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.99.4/
...
These instructions result in the initial release-4.99.4 branch label being attached to the master branch sequence.  To put it another way, release-4.99.4 is a parent of master.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to add something to release-4.99.4 and _not_ include other additions to master, you have to branch the contents of release-4.99.4 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-4.99.4 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.99.4 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-4.99.4".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-4.99.4 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.

   - `git cherrypick` just the changes you want onto the release-4.99.4 branch. *This is not the recommended approach, as it is error-prone; we've had to withdraw releases in the past due to this.*  Read the documentation on that command carefully and double check your work. If possible, check the contents of the release branch on the GitHub web site to make sure only the changes you wanted were included.

- Make sure that the 4.99.4 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.99.4 release note.  Merge these as needed to the release-4.99.4 and master branches
...
```
git checkout release-4.99.4
ant realclean compile
cd target
rm -f properties.4.99.4.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.99.4' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.99.4.zip
cd ..
mkdir release
mv target/properties.4.99.4.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 4.99.4
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.4/) individually and put them in a release/ directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri4.99.4.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.4/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v4.99.4 (e.g. leading lower-case "v")
   - @ branch: select the release-4.99.4 release branch
   - "Release title" field gets "Prod/Test Release 4.99.4"
...
---|---
[JMRI.4.99.4+R1d78b88a65.dmg](https://github.com/JMRI/JMRI/releases/download/v4.99.4/JMRI.4.99.4+R1d78b88a65.dmg) | c3a2ca989ea556bc25b83cceb1311b5fe5f8487011f982aadb39f4dea68d1707
[JMRI.4.99.4+R1d78b88a65.exe](https://github.com/JMRI/JMRI/releases/download/v4.99.4/JMRI.4.99.4+R1d78b88a65.exe) | f34c0bba53aed75d0e93d201442cafba29924c08ac33c8c545cc905334eacc5a
[JMRI.4.99.4+R1d78b88a65.tgz](https://github.com/JMRI/JMRI/releases/download/v4.99.4/JMRI.4.99.4+R1d78b88a65.tgz) | 7120a7a47ab000439646f2accc747e1dda7311879b101dc1df9d771fb6d14d2a
...
git merge origin/release-4.99.4
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 4.99.5". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.0 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 4.99.5 milestone to the issue.

- Confirm that the tag for the current release (v4.99.4 for release 4.99.4) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.99.4) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri4.99.4.shtml
```
...
Content:
    ...
    For more information on the issues, new features and bug fixes in 4.99.4 please see the release note:
    <https://www.jmri.org/releasenotes/jmri4.99.4.shtml>
...
- Close the [4.99.4 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+4.99.4%22) with a comment saying that
```
    JMRI 4.99.4 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.5 -n "JMRI.4.99.5+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.4/builds/4/archive/dist/release/JMRI.4.99.5+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.5 -n "JMRI.4.99.5+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.4/builds/4/archive/dist/release/JMRI.4.99.5+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.5 -n "JMRI.4.99.5+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.4/builds/4/archive/dist/release/JMRI.4.99.5+Rd144052.tgz
```
