...
```
  ./scripts/update-HOWTO.sh 5.15.3 5.15.4 5.15.5
```
...
- To check the script ran OK, the following should be the release you're doing now: 5.15.3
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 5.15.3". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.3%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 5.15.3" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri5.15.3.shtml jmri5.15.4.shtml
        $EDITOR jmri5.15.4.shtml
```
...
```
        git add jmri5.15.4.shtml
        git commit -m"start new 5.15.4 next release note" jmri5.15.4.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri5.15.3.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri5.15.3.shtml
```

 - Add any new warnings to the old warnings section of the next (5.15.4) release note:
```
        ${EDITOR} ../website/releasenotes/jmri5.15.3.shtml ../website/releasenotes/jmri5.15.4.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri5.15.3.shtml
```
...
```
        ...
        git commit -m"start for 5.15.4 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"updated 5.15.3 release notes" jmri5.*
        git push github
        git pull
        cd ../../JMRI
```
...
```
        git checkout master
        git pull
        sed -i.bak s/5.15.1-SNAPSHOT/5.15.3-SNAPSHOT/g pom.xml
```
...
```
 - Check that both those edits left 5.15.3 defined in the two files
 
 - Commit them back
 
```
        git commit -m"5.15.3 until next release" release.properties pom.xml
```
...
```
        ...
        git commit -m"SBOM update for 5.15.3" lib/bom-Java-Maven.spdx
...
- Remove the 5.15.3 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A5.15.3)

- Check on GitHub that [`release.properties`](https://github.com/JMRI/JMRI/blob/master/release.properties) on `master` is showing 5.15.3

- Create a `release-5.15.3` branch on GitHub
```
        git checkout -b release-5.15.3
        git push github
```

- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.3%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-5.15.3 branch has been created.

Maintainers, please set the 5.15.4 milestone on pulls from now on, as that will be the next test release.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/5.15.3/)
```
...
Maintainers, please set the (next series, i.e. 5.15.4) milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/TestReleases/job/5.15.3/)

If you're developing any additional (post-5.15.3) changes that you want in the JMRI 5.16 production release, please start from this branch, i.e. do `git fetch; git checkout release-5.15.3; git checkout -b myBranchName` to start your work.
```


- Fetch back to make sure your repository is fully up to date but stay on the release-5.15.3 branch
...
- Click "Copy Existing Item". Fill out the new 5.15.3 release name at the top. Enter the 5.15.3 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  5.15.3
...
- When that's done, put a screen-shot of the four monitor graphs into the "[Create Test Release 5.15.3](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.15.3%22)" Github issue so that historical resource usage info is available.

================================================================================
## Put Files Out For Checking

- Change the release note to point to the just-built files (in CI or where you put them), commit, wait (or force via ["Build Now"](https://builds.jmri.org/jenkins/job/website/job/website-repository/) button). Confirm [visible on web](https://www.jmri.org/releasenotes/jmri5.15.3.shtml).

- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%205.15.3%20files%20available) with a subject line


```
"First 5.15.3 files available":

First JMRI 5.15.3 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/5.15.3/
...
These instructions result in the initial release-5.15.3 branch label being attached to the master branch sequence.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to include _all_ merges to master to date, you can just move the release-5.15.3 branch forward on master to the current head (and update the release note, decoderIndex and help files as needed)

If you want to add something to release-5.15.3 and _not_ include other additions to master, you have to branch the contents of release-5.15.3 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-5.15.3 branch (if working after that was started) or on a branch-from-master that's _before_ the release-5.15.3 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-5.15.3".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-5.15.3 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.
...
- Make sure that the 5.15.3 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 5.15.3 release note.  Merge these as needed to the release-5.15.3 and master branches
...
```
git checkout release-5.15.3
ant realclean compile
cd target
rm -f properties.5.15.3.zip

for x in `find classes -name \*.properties`; do
printf '%s\n' 0a '# from tag v5.15.3' . x | ex $x
done

find classes -name \*.properties | zip -@ properties.5.15.3.zip
cd ..
mkdir release
mv target/properties.5.15.3.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 5.15.3
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/5.15.3/) individually and put them in a `release/` directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri5.15.3.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/5.15.3/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v5.15.3 (note leading lower-case "v"!)
   - @ branch: select the release-5.15.3 release branch
   - "Release title" field gets "Prod/Test Release 5.15.3"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri5.15.3.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.5.15.3+Rfe7ff30e7d.dmg](https://github.com/JMRI/JMRI/releases/download/v5.15.3/JMRI.5.15.3+Rfe7ff30e7d.dmg) | edc3d8c0cd4b408a3401b650be264c59224846df2b3dc197d184452f3ece8ca6
[JMRI.5.15.3+Rfe7ff30e7d.exe](https://github.com/JMRI/JMRI/releases/download/v5.15.3/JMRI.5.15.3+Rfe7ff30e7d.exe) | c813c2688e5a0d23c26675c97ece6c6cf24018e6e0e5bc05b76b0f87fb364dbf
[JMRI.5.15.3+Rfe7ff30e7d.tgz](https://github.com/JMRI/JMRI/releases/download/v5.15.3/JMRI.5.15.3+Rfe7ff30e7d.tgz) | d895c57d8d48f25d79c138ba502f8c96e65e9d3d7796d0d83cc77328c30da56b
...
```
git diff master...release-5.15.3
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 5.15.4". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.16 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 5.15.4 milestone to the issue.

- Confirm that the tag for the current release (v5.15.3 for release 5.15.3) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-5.15.3) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri5.15.3.shtml
```
...
Content:
    ...
    <https://www.jmri.org/releasenotes/jmri5.15.3.shtml#update>

    For more information on the issues, new features and bug fixes in 5.15.3 please see the release note:
    <https://www.jmri.org/releasenotes/jmri5.15.3.shtml>
...
- Close the [5.15.3 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+5.15.3%22) with a comment saying that
```
    JMRI 5.15.3 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.3 -n "JMRI.5.9.8+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.3/builds/4/archive/dist/release/JMRI.5.15.3+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.3 -n "JMRI.5.9.8+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.3/builds/4/archive/dist/release/JMRI.5.15.3+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.15.3 -n "JMRI.5.9.8+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/5.15.3/builds/4/archive/dist/release/JMRI.5.15.3+Rd144052.tgz
```
