...
```
  ./scripts/update-HOWTO.sh 4.7.6 4.7.7 4.7.8
```
...
```
The release-4.7.7 branch has been created. 

From now on, please document your changes in the [jmri4.9.1.shtml](https://github.com/JMRI/website/blob/master/releasenotes/jmri4.9.1.shtml) release note file.

Maintainers, please set the 4.9.1 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](http://jmri.tagadab.com/jenkins/job/TestReleases/job/4.7.7/)
````
...
- Announce the file set via email to jmri-developers@lists.sf.net with a subject line "First 4.7.7 files available":

First JMRI 4.7.7 files are available in the usual way at:

http://jmri.tagadab.com/jenkins/job/TestReleases/job/4.7.7
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.7.7".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
(replace "user" below with your SourceForge.net user name; must have SSH keys for SourceForge.net set up)
 ...
 - (The "./testrelease 4.7.7" local script on shell.sf.net does the following steps, except for the edit, of course)
```
    ssh user,jmri@shell.sf.net create
    ssh user,jmri@shell.sf.net
    curl -o release.zip "http://jmri.tagadab.com/jenkins/job/Test%20Releases/job/4.7.7/ws/dist/release/*zip*/release.zip"
        (use the following instead if building on second Jenkins server)
    curl -o release.zip "http://jmri.tagadab.com/jenkins/job/TestReleases/job/4.7.7/ws/dist/release/*zip*/release.zip"
    rm release/JMRI*
    unzip release.zip
    cd release
...
- Fill out form:
   ...
   - "tag version field" gets v4.7.7 (e.g. leading lower-case "v")
   - @ branch: select the release-4.7.7 release branch
```
"Release title" field gets "Test/Prod Release 4.7.7"
```
   - Description content (the testrelease script above proposed this!):
```    
[Release notes](http://jmri.org/releasenotes/jmri4.7.7.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.7.7.R177ca31.dmg](https://github.com/JMRI/JMRI/releases/download/v4.7.7/JMRI.4.7.7.R177ca31.dmg) | e98de5ccec57fe895a4cee0f56f3a641c08ffea185b031dc1b1d2b6574322e99
[JMRI.4.7.7.R177ca31.exe](https://github.com/JMRI/JMRI/releases/download/v4.7.7/JMRI.4.7.7.R177ca31.exe) | a096201c2216e9666e791a24a2f4c3631c53c9c25364f4efbe8ce089f7338a96
[JMRI.4.7.7.R177ca31.tgz](https://github.com/JMRI/JMRI/releases/download/v4.7.7/JMRI.4.7.7.R177ca31.tgz) | 6586160914ed3c145583151f6132315d37afc2faa6657b558b27db4d982dc498

```

- Attach files by selecting them or dragging them in (you might have to have downloaded them above via e.g. a separate 
```
curl -o release.zip "http://jmri.tagadab.com/jenkins/job/TestReleases/job/4.7.7/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.7.7 -n "JMRI.4.7.7+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.7/workspace/dist/release/JMRI.4.7.7+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.7.7 -n "JMRI.4.7.7+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.7.7+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.7.7 -n "JMRI.4.7.7+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.7.7+Rd144052.tgz 
```
...
- Confirm that the tag for the current release (release-4.7.7) is in place, then manually delete the current release branch via the [GitHub UI](https://github.com/JMRI/JMRI/branches).
...
- Mail announcement to jmriusers@yahoogroups.com
    ...
    Subject is "Test version 4.7.7 of JMRI/DecoderPro is available for download" or "JMRI 4.8 is available for download"
...
If you use JMRI on Linux or Mac and are updating from an earlier version, there’s a necessary migration step. (Not needed on Windows) Please see the release note for details: <http://jmri.org/releasenotes/jmri4.7.7.shtml#migration>

For more information on the issues, new features and bug fixes in 4.7.7 please see the release note:
<http://jmri.org/releasenotes/jmri4.7.7.shtml>
