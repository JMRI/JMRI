...
```
  ./scripts/update-HOWTO.sh 4.25.2 4.25.3 4.25.3
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.25.2

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.25.2". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.25.2%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 4.25.2" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri4.25.2.shtml jmri4.25.3.shtml
        $EDITOR jmri4.25.3.shtml
        ...
        git add jmri4.25.3.shtml
        git commit -m"start new 4.25.3 next release note" jmri4.25.3.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.25.2.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.25.2.shtml
```

 - Add any new warnings to the old warnings section of the next (4.25.3) release note:
```
        ${EDITOR} ../website/releasenotes/jmri4.25.3.shtml ../website/releasenotes/jmri4.25.2.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri4.25.2.shtml
```
...
```
        ...
        git commit -m"start for 4.25.3 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"updated 4.25.3 release note" jmri4.*
        git push github
        git pull
        cd ../../JMRI
```
...
- Remove the 4.25.2 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A4.25.2)

- Update the &lt;version> element in pom.xml to say the next release:
```
        sed -i .bak s/4.25.2-SNAPSHOT/4.25.3-SNAPSHOT/g pom.xml
        git commit -m"for next release 4.25.3" pom.xml
        git push github
```
...
- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.25.2%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-4.25.2 branch has been created.

Maintainers, please set the 4.25.3 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/4.25.2/)
```
...
If you're developing any additional (post-4.25.2) changes that you want in the JMRI 4.24 production release, please start from this branch, i.e. do `git checkout -b release-4.25.2` to start your work.
```
...
- Click "Copy Existing Item". Fill out the new 4.25.2 release name at the top. Enter the 4.23.6 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  4.25.2
...
```
   ...
   <ul> <li style="list-style-type: none;">
    and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.25.2" Github issue so that historical resource usage info is available.
   </li> </ul>
...
- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%204.25.2%20files%20available) with a subject line

```
"First 4.25.2 files available":

First JMRI 4.25.2 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.25.2/
...
- Ideally, start the work on either the release-4.25.2 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.25.2 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-4.25.2".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
  ...
  - Merge master into the release-4.25.2 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.
...
- Make sure that the 4.25.2 milestone is on the original PR and any others that have been pulled in

- If the PR has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.25.2 release note.  Merge these as needed to the release-4.25.2 and master branches
...
```
git checkout release-4.25.2
ant realclean compile
cd target
rm -f properties.4.25.2.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.25.2' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.25.2.zip
cd ..
mkdir release
mv target/properties.4.25.2.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 4.25.2
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/4.25.2/) individually and put them in a release/ directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri4.25.2.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/4.25.2/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v4.25.2 (e.g. leading lower-case "v")
   - @ branch: select the release-4.25.2 release branch
   - "Release title" field gets "Prod/Test Release 4.25.2"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri4.25.2.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.25.2+R1547b1639f.dmg](https://github.com/JMRI/JMRI/releases/download/v4.25.2/JMRI.4.25.2+R1547b1639f.dmg) | 8a9a77c19f38f1ee6248feb9735d9681a18499927db021a61c995b65722c5f74
[JMRI.4.25.2+R1547b1639f.exe](https://github.com/JMRI/JMRI/releases/download/v4.25.2/JMRI.4.25.2+R1547b1639f.exe) | c676b9eb39df623a9da4fc5c3dab2242b2b8f989e6a1e399c8c327a7ed886237
[JMRI.4.25.2+R1547b1639f.tgz](https://github.com/JMRI/JMRI/releases/download/v4.25.2/JMRI.4.25.2+R1547b1639f.tgz) | 025415a532cb6a1020235876cd256fa2f065d17b5893044b96c06ca02b70ec0b
...
git merge origin/release-4.25.2
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 4.25.3". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.26 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 4.25.3 milestone to the issue.

- Confirm that the tag for the current release (v4.25.2 for release 4.25.2) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.25.2) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri4.25.2.shtml
```
...
Content:
    ...
    For more information on the issues, new features and bug fixes in 4.25.2 please see the release note:
    <https://www.jmri.org/releasenotes/jmri4.25.2.shtml>
...
- Close the [4.25.2 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+4.25.2%22) with a comment saying that
```
    JMRI 4.25.2 has been released. Files are available in the GitHub release section.
...
```
curl -o release.zip "https://builds.jmri.org/jenkins/job/TestReleases/job/4.25.3/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip""
```
and expansion;
...
```
        ...
        git checkout -b release-4.25.3
        git push github release-4.25.3
        git checkout master
        git pull
```
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.25.3 -n "JMRI.4.21.6+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.6/workspace/dist/release/JMRI.4.21.6+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.25.3 -n "JMRI.4.21.6+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.6/workspace/dist/release/JMRI.4.21.6+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.25.3 -n "JMRI.4.21.6+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.6/workspace/dist/release/JMRI.4.21.6+Rd144052.tgz
```
