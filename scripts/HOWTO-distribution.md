...
```
  ./scripts/update-HOWTO.sh 4.21.2 4.21.3 4.21.4
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.21.2

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.21.2". (This might already exist, if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
git commit -m"for 4.21.2" scripts/HOWTO-distribution.md
git push github
```
...
- Merge the release note body from help/en/releasenotes/current-draft-note.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.21.2.shtml
...
- Merge the new warnings (if any) from help/en/releasenotes/current-warnings.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.21.2.shtml
     
 - add any new warnings to the old warnings section of the next (4.21.3) release note:
    ${EDITOR} ../website/releasenotes/jmri4.21.3.shtml ../website/releasenotes/jmri4.21.2.shtml
...
- Create the new draft note section
    ...
    git commit -m"start for 4.21.3 release note" help/en/releasenotes/*.shtml
    git push github
    git pull
...
- Commit release note, push and pull back
    ...
    git commit -m"updated 4.21.2 release note" jmri4.21.3.shtml jmri4.21.2.shtml
    git push github
    git pull
    cd ../../JMRI
...
Maintainers, please set the 4.21.3 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/4.21.2/)
```
...
Jenkins will be creating files shortly at the [CI server](http://builds.jmri.org/jenkins/job/TestReleases/job/4.21.2/)

If you're developing any additional (post-4.21.2) changes that you want in the JMRI 4.22 production release, please start from this branch, i.e. do `git checkout -b release-4.21.2` to start your work.
```
...
```
"First 4.21.2 files available":

First JMRI 4.21.2 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.21.2/
...
- Ideally, start the work on either the release-4.21.2 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.21.2 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.21.2".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.21.2 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.
...
- Make sure that the 4.21.2 milestone is on the original PR

- If the PR has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.21.3 release note.  Merge these as needed to the release-4.21.3 and master branches
...
```
git checkout release-4.21.2
ant realclean compile
cd target
rm -f properties.4.21.2.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.21.2' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.21.2.zip
cd ..
mkdir release
mv target/properties.4.21.2.zip release/
ls -lt release/
git checkout master
...
- Fill out form:
   ...
   - "tag version field" gets v4.21.2 (e.g. leading lower-case "v")
   - @ branch: select the release-4.21.2 release branch
   - "Release title" field gets "Prod/Test Release 4.21.2"
...
git merge origin/release-4.21.2
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.21.3". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.22 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```

- Confirm that the tag for the current release (v4.21.2 for release 4.21.2) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.21.2) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
     ${EDITOR}  releasenotes/jmri4.21.2.shtml
```

- Commit site, push to github
```
    git commit -m"4.21.2 web site" .
    git push github
    git pull
```
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.21.2 of JMRI/DecoderPro is available for download" or "JMRI 4.22 is available for download"

    Content:
    
Test version 4.21.2 of JMRI/DecoderPro is available for download.
...
For more information on the issues, new features and bug fixes in 4.21.2 please see the release note:   
<https://www.jmri.org/releasenotes/jmri4.21.2.shtml>
...
- Close the [4.21.3 release GitHub Issue](https://github.com/JMRI/JMRI/issues) with a note saying that
```
JMRI 4.21.2 has been released. Files are available in the GitHub release section.
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.21.3/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
and expansion; 
...
```    
        ...
        git checkout -b release-4.21.3
        git push github release-4.21.3
        git checkout master    
        git pull
```
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.3 -n "JMRI.4.21.3+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.3/workspace/dist/release/JMRI.4.21.3+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.3 -n "JMRI.4.21.3+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.3/workspace/dist/release/JMRI.4.21.3+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.3 -n "JMRI.4.21.3+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.3/workspace/dist/release/JMRI.4.21.3+Rd144052.tgz 
```
