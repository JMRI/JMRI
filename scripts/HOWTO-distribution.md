...
```
  ./scripts/update-HOWTO.sh 4.99.5 4.99.6 4.99.6
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.99.5
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.99.5". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.99.5%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 4.99.5" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri4.99.5.shtml jmri4.99.6.shtml
        $EDITOR jmri4.99.6.shtml
        ...
        git add jmri4.99.6.shtml
        git commit -m"start new 4.99.6 next release note" jmri4.99.6.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.99.5.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.99.5.shtml
```

 - Add any new warnings to the old warnings section of the next (4.99.6) release note:
```
        ${EDITOR} ../website/releasenotes/jmri4.99.6.shtml ../website/releasenotes/jmri4.99.5.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri4.99.5.shtml
```
...
```
        ...
        git commit -m"start for 4.99.6 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
- Remove the 4.99.5 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A4.99.5)
...
```
        ...
        git commit -m"for next release 4.99.6" pom.xml
        git push github
```

- Check that `release.properties` on `master` is showing 4.99.6

- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.99.5%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-4.99.5 branch has been created.

Maintainers, please set the 4.99.6 milestone on pulls from now on, as that will be the next test release.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.5/)
```
...
If you're developing any additional (post-4.99.5) changes that you want in the JMRI 4.26 production release, please start from this branch, i.e. do `git checkout -b release-4.99.6` to start your work.
```
...
- Check the [signing-wget.log](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.5/ws/%20signing-wget.log) file in the job’s workspace to make sure all the return codes are zero. This is the macOS signing process. A non-zero code indicates a problem with the signature.
...
and connect jconsole to the test class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "[Create Test Release 4.99.5](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.99.5%22)" Github issue so that historical resource usage info is available.
...
- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%204.99.5%20files%20available) with a subject line

```
"First 4.99.5 files available":

First JMRI 4.99.5 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.99.5/
...
These instructions result in the initial release-4.99.5 branch label being attached to the master branch sequence.  To put it another way, release-4.99.5 is a parent of master.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to add something to release-4.99.5 and _not_ include other additions to master, you have to branch the contents of release-4.99.5 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-4.99.5 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.99.5 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-4.99.5".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-4.99.5 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.

   - `git cherrypick` just the changes you want onto the release-4.99.5 branch. *This is not the recommended approach, as it is error-prone; we've had to withdraw releases in the past due to this.*  Read the documentation on that command carefully and double check your work. If possible, check the contents of the release branch on the GitHub web site to make sure only the changes you wanted were included.

- Make sure that the 4.99.5 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.99.5 release note.  Merge these as needed to the release-4.99.5 and master branches
...
```
git checkout release-4.99.5
ant realclean compile
cd target
rm -f properties.4.99.5.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.99.5' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.99.5.zip
cd ..
mkdir release
mv target/properties.4.99.5.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 4.99.5
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.5/) individually and put them in a release/ directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri4.99.5.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.5/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v4.99.5 (e.g. leading lower-case "v")
   - @ branch: select the release-4.99.5 release branch
   - "Release title" field gets "Prod/Test Release 4.99.5"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri4.99.5.shtml)
...
git merge origin/release-4.99.5
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 4.99.6". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.0 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 4.99.6 milestone to the issue.

- Confirm that the tag for the current release (v4.99.5 for release 4.99.5) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.99.5) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri4.99.5.shtml
```
...
Content:
    ...
    For more information on the issues, new features and bug fixes in 4.99.5 please see the release note:
    <https://www.jmri.org/releasenotes/jmri4.99.5.shtml>
...
- Close the [4.99.5 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+4.99.5%22) with a comment saying that
```
    JMRI 4.99.5 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.6 -n "JMRI.4.99.6+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.5/builds/4/archive/dist/release/JMRI.4.99.6+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.6 -n "JMRI.4.99.6+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.5/builds/4/archive/dist/release/JMRI.4.99.6+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.6 -n "JMRI.4.99.6+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.5/builds/4/archive/dist/release/JMRI.4.99.6+Rd144052.tgz
```
