...
```
  ./scripts/update-HOWTO.sh 5.9.7 5.9.8 5.9.9
```
...
- To check the script ran OK, the following should be the release you're doing now: 5.9.7
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 5.9.7". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.9.7%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 5.9.7" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri5.9.7.shtml jmri5.9.8.shtml
        $EDITOR jmri5.9.8.shtml
```
...
```
        git add jmri5.9.8.shtml
        git commit -m"start new 5.9.8 next release note" jmri5.9.8.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri5.9.7.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri5.9.7.shtml
```

 - Add any new warnings to the old warnings section of the next (5.9.8) release note:
```
        ${EDITOR} ../website/releasenotes/jmri5.9.7.shtml ../website/releasenotes/jmri5.9.8.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri5.9.7.shtml
```
...
```
        ...
        git commit -m"start for 5.9.8 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"updated 5.9.7 release notes" jmri5.*
        git push github
        git pull
        cd ../../JMRI
```
...
```
        git checkout master
        git pull
        sed -i.bak s/5.9.5-SNAPSHOT/5.9.7-SNAPSHOT/g pom.xml
```
...
```
 - Check that both those edits left 5.9.7 defined in the two files
 
```
        git commit -m"5.9.7 until next release" release.properties pom.xml
```
...
```
        ...
        git commit -m"SBOM update for 5.9.7" lib/bom-Java-Maven.spdx
...
- Remove the 5.9.7 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A5.9.7)

- Check on GitHub that [`release.properties`](https://github.com/JMRI/JMRI/blob/master/release.properties) on `master` is showing 5.9.7

- Create a `release-5.9.7` branch on GitHub
```
        git checkout -b release-5.9.7
        git push github
```

- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.9.7%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-5.9.7 branch has been created.

Maintainers, please set the 5.9.8 milestone on pulls from now on, as that will be the next test release.
...
- Fetch back to make sure your repository is fully up to date but stay on the release-5.9.7 branch
...
- Click "Copy Existing Item". Fill out the new 5.9.7 release name at the top. Enter the 5.9.2 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  5.9.7
...
- When that's done, put a screen-shot of the four monitor graphs into the "[Create Test Release 5.9.7](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.9.7%22)" Github issue so that historical resource usage info is available.

================================================================================
## Put Files Out For Checking

- Change the release note to point to the just-built files (in CI or where you put them), commit, wait (or force via ["Build Now"](https://builds.jmri.org/jenkins/job/website/job/website-repository/) button). Confirm [visible on web](https://www.jmri.org/releasenotes/jmri5.9.7.shtml).

- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%205.9.7%20files%20available) with a subject line


```
"First 5.9.7 files available":

First JMRI 5.9.7 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/5.9.7/
...
These instructions result in the initial release-5.9.7 branch label being attached to the master branch sequence.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to include _all_ merges to master to date, you can just move the release-5.9.7 branch forward on master to the current head (and update the release note, decoderIndex and help files as needed)

If you want to add something to release-5.9.7 and _not_ include other additions to master, you have to branch the contents of release-5.9.7 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-5.9.7 branch (if working after that was started) or on a branch-from-master that's _before_ the release-5.9.7 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-5.9.7".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-5.9.7 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.

   - `git cherrypick` just the changes you want onto the release-5.9.7 branch. *This is not the recommended approach, as it is error-prone; we've had to withdraw releases in the past due to this.*  Read the documentation on that command carefully and double check your work. If possible, check the contents of the release branch on the GitHub web site to make sure only the changes you wanted were included.

- Make sure that the 5.9.7 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 5.9.7 release note.  Merge these as needed to the release-5.9.7 and master branches
...
```
git checkout release-5.9.7
ant realclean compile
cd target
rm -f properties.5.9.7.zip

for x in `find classes -name \*.properties`; do
printf '%s\n' 0a '# from tag v5.9.7' . x | ex $x
done

find classes -name \*.properties | zip -@ properties.5.9.7.zip
cd ..
mkdir release
mv target/properties.5.9.7.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 5.9.7
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/5.9.7/) individually and put them in a `release/` directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri5.9.7.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/5.9.7/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v5.9.7 (note leading lower-case "v"!)
   - @ branch: select the release-5.9.7 release branch
   - "Release title" field gets "Prod/Test Release 5.9.7"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri5.9.7.shtml)
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 5.9.8". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.10 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 5.9.8 milestone to the issue.

- Confirm that the tag for the current release (v5.9.7 for release 5.9.7) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-5.9.7) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri5.9.7.shtml
```
...
[jmriusers@groups.io](mailto:jmriusers@groups.io?subject=Test%20release%205.9.7%20of%20JMRI/DecoderPro%20is%20available%20for%20download&body=Test%20release%205.9.7%20of%20JMRI/DecoderPro%20is%20available%20for%20download.%0A%0AThis%20is%20the%20next%20in%20a%20series%20of%20test%20releases%20that%20will%20culminate%20in%20a%20production%20release,%20hopefully%20in%20December%202024.%20It's%20really%20helpful%20when%20people%20download,%20install%20and%20use%20these%20test%20releases%20so%20we%20can%20find%20and%20fix%20any%20inadvertent%20new%20problems%20early.%0A%0A-%20Alt:%20There%20have%20been%20a%20lot%20of%20updates%20in%20this%20release,%20so%20it%20should%20be%20considered%20experimental.%0A-%20Alt:%20We're%20getting%20close%20to%20the%20end%20of%20the%20development%20series,%20so%20we'd%20appreciate%20feedback%20on%20whether%20or%20not%20this%20release%20works%20for%20your%20layout.%0A%0AIf%20you%20are%20currently%20using%20JMRI%204.99.10%20or%20earlier,%20there%20is%20an%20update%20process%20that%20we%20strongly%20recommend.%20See%20the%20release%20note%20section%20on%20updates:%0A<https://www.jmri.org/releasenotes/jmri5.9.7.shtml#update>%0A%0AFor%20more%20information%20on%20the%20issues,%20new%20features%20and%20bug%20fixes%20in%205.9.7%20please%20see%20the%20release%20note:%0A<https://www.jmri.org/releasenotes/jmri5.9.7.shtml>%0A%0ANote%20that%20JMRI%20is%20made%20available%20under%20the%20GNU%20General%20Public%20License.%20For%20more%20information,%20please%20see%20our%20copyright%20and%20licensing%20page.%0A<https://www.jmri.org/Copyright.html>%0A%0AThe%20download%20links,%20along%20with%20lots%20of%20other%20information%20which%20we%20hope%20you'll%20read,%20can%20be%20found%20on%20the%20release%20note%20page:%0A<https://www.jmri.org/releasenotes/jmri5.9.7.shtml>%0A)

Subject:

   "Test release 5.9.7 of JMRI/DecoderPro is available for download"
    or
...
Content:
    ...
    <https://www.jmri.org/releasenotes/jmri5.9.7.shtml#update>

    For more information on the issues, new features and bug fixes in 5.9.7 please see the release note:
    <https://www.jmri.org/releasenotes/jmri5.9.7.shtml>
...
- Close the [5.9.7 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+5.9.7%22) with a comment saying that
```
    JMRI 5.9.7 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.9.7 -n "JMRI.5.9.7+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/5.9.7/builds/4/archive/dist/release/JMRI.5.9.7+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.9.7 -n "JMRI.5.9.7+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/5.9.7/builds/4/archive/dist/release/JMRI.5.9.7+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.9.7 -n "JMRI.5.9.7+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/5.9.7/builds/4/archive/dist/release/JMRI.5.9.7+Rd144052.tgz
```
