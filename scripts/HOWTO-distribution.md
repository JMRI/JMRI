...
```
  ./scripts/update-HOWTO.sh 5.1.1 5.1.2 5.1.2
```
...
- To check the script ran OK, the following should be the release you're doing now: 5.1.1
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 5.1.1". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.1.1%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 5.1.1" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri5.1.1.shtml jmri5.1.2.shtml
        $EDITOR jmri5.1.1.shtml
        ...
        git add jmri5.1.2.shtml
        git commit -m"start new 5.1.2 next release note" jmri5.1.2.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri5.1.1.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri5.1.1.shtml
```

 - Add any new warnings to the old warnings section of the next (5.1.2) release note:
```
        ${EDITOR} ../website/releasenotes/jmri5.1.1.shtml ../website/releasenotes/jmri5.1.2.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri5.1.1.shtml
```
...
```
        ...
        git commit -m"start for 5.1.2 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
```
        ...
        git commit -m"updated 5.1.1 release notes" jmri5.*
        git push github
        git pull
        cd ../../JMRI
```
...
```
        git checkout master
        git pull
        sed -i .bak s/4.99.10-SNAPSHOT/5.1.1-SNAPSHOT/g pom.xml
        git commit -m"5.1.1 until next release" pom.xml
```
...
```
        ...
        git commit -m"5.1.1 until next release" release.properties
        git push github
```
...
- Remove the 5.1.1 milestone on [unmerged PRs or [click here]](https://github.com/JMRI/JMRI/pulls?q=is%3Aopen+is%3Apr+milestone%3A5.1.1)

- Check on GitHUb that [`release.properties`](https://github.com/JMRI/JMRI/blob/master/release.properties) on `master` is showing 5.1.1

- Create a `release-5.1.1` branch on GitHub
```
...
- Put the following comment in the [release GitHub item](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.1.1%22) saying the branch exists, and all future changes should be documented in the new release note: (NOT FOR THE LAST TEST RELEASE FROM MASTER BEFORE A PRODUCTION RELEASE, see just below)

```
The release-5.1.1 branch has been created.

Maintainers, please set the 5.1.2 milestone on pulls from now on, as that will be the next test release.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/testreleases/job/5.1.1/)
```
...
Maintainers, please set the (next series, i.e. 5.1.2) milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](https://builds.jmri.org/jenkins/job/TestReleases/job/5.1.1/)

If you're developing any additional (post-5.1.1) changes that you want in the JMRI 5.0 production release, please start from this branch, i.e. do `git fetch; git checkout release-5.1.1; git checkout -b myBranchName` to start your work.
```


- Fetch back to make sure your repository is fully up to date but stay on the release-5.1.1 branch
...
- Click "Copy Existing Item". Fill out the new 5.1.1 release name at the top. Enter the 4.99.10 most recent release at the bottom.  Click "OK"

- Update

        Description
        Source Code Management:
           Branch Specified:  5.1.1
...
- Check the [signing-wget.log](https://builds.jmri.org/jenkins/job/testreleases/job/5.1.1/ws/%20signing-wget.log) file in the job’s workspace to make sure all the return codes are zero. This is the macOS signing process. A non-zero code indicates a problem with the signature.
...
- When that's done, put a screen-shot of the four monitor graphs into the "[Create Test Release 5.1.1](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+5.1.1%22)" Github issue so that historical resource usage info is available.
...
- Announce the file set via [email to jmri@jmri-developers.groups.io](mailto:jmri@jmri-developers.groups.io?subject=First%205.1.1%20files%20available) with a subject line


```
"First 5.1.1 files available":

First JMRI 5.1.1 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/5.1.1/
...
These instructions result in the initial release-5.1.1 branch label being attached to the master branch sequence.  If somebody commits to master after that, master will move along with additional contents. Once that happens, if you want to add something to release-5.1.1 and _not_ include other additions to master, you have to branch the contents of release-5.1.1 away from master.  This section describes how to do that.

- Ideally, start the work on either the release-5.1.1 branch (if working after that was started) or on a branch-from-master that's _before_ the release-5.1.1 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-5.1.1".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- If somebody has merged their change into master (or it's branched from master later than the release tag), you have two choices:
   ...
   - Merge master into the release-5.1.1 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.

   - `git cherrypick` just the changes you want onto the release-5.1.1 branch. *This is not the recommended approach, as it is error-prone; we've had to withdraw releases in the past due to this.*  Read the documentation on that command carefully and double check your work. If possible, check the contents of the release branch on the GitHub web site to make sure only the changes you wanted were included.

- Make sure that the 5.1.1 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 5.1.1 release note.  Merge these as needed to the release-5.1.1 and master branches
...
cd target
rm -f properties.5.1.1.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v5.1.1' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.5.1.1.zip
cd ..
mkdir release
mv target/properties.5.1.1.zip release/
ls -lt release/
git checkout master
...
```
./scripts/releasesummary 5.1.1
```
(This attempts a very large download.  If it fails, [download the files](https://builds.jmri.org/jenkins/job/testreleases/job/5.1.1/) individually and put them in a `release/` directory in your working directory, then repeat the command)
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri5.1.1.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/5.1.1/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v5.1.1 (e.g. leading lower-case "v")
   - @ branch: select the release-5.1.1 release branch
   - "Release title" field gets "Prod/Test Release 5.1.1"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri5.1.1.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.5.1.1+Redc443cff0.dmg](https://github.com/JMRI/JMRI/releases/download/v5.1.1/JMRI.5.1.1+Redc443cff0.dmg) | 26fad20009292be1a1b3a875693589543257b568900c72becf2a87ea7452a4af
[JMRI.5.1.1+Redc443cff0.exe](https://github.com/JMRI/JMRI/releases/download/v5.1.1/JMRI.5.1.1+Redc443cff0.exe) | 820024d28f03eea95dd4fd969c51546a30118df8705dd161dd50382f4a36048c
[JMRI.5.1.1+Redc443cff0.tgz](https://github.com/JMRI/JMRI/releases/download/v5.1.1/JMRI.5.1.1+Redc443cff0.tgz) | 03802d775868f754af71b975b5657cc76354865707e50952f458e76827c65754
...
git merge origin/release-5.1.1
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 5.1.2". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.2 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 5.1.2 milestone to the issue.

- Confirm that the tag for the current release (v5.1.1 for release 5.1.1) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-5.1.1) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri5.1.1.shtml
```
...
Content:
    ...
    For more information on the issues, new features and bug fixes in 5.1.1 please see the release note:
    <https://www.jmri.org/releasenotes/jmri5.1.1.shtml>
...
- Close the [5.1.1 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+5.1.1%22) with a comment saying that
```
    JMRI 5.1.1 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.1.1 -n "JMRI.5.1.1+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/5.1.1/builds/4/archive/dist/release/JMRI.5.1.1+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.1.1 -n "JMRI.5.1.1+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/5.1.1/builds/4/archive/dist/release/JMRI.5.1.1+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v5.1.1 -n "JMRI.5.1.1+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/5.1.1/builds/4/archive/dist/release/JMRI.5.1.1+Rd144052.tgz
```
