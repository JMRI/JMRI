...
```
  ./scripts/update-HOWTO.sh 4.13.5 4.13.6 4.13.7
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.13.5
...
```
git commit -m"for 4.13.5" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git pull 
        cp jmri4.13.5.shtml jmri4.13.6.shtml
        (edit the new release note accordingly)
            change numbers throughout
            move new warnings to old
            remove old-version change notes
        git add jmri4.13.6.shtml
        git commit -m"start new 4.13.6 release note" jmri4.13.6.shtml
        git push github
        cd (local JMRI copy)
```
...
```
git commit -m"for next release 4.13.6" pom.xml
git push github
```
...
```
The release-4.13.5 branch has been created. 

From now on, please document your changes in the [jmri4.13.6.shtml](https://github.com/JMRI/website/blob/master/releasenotes/jmri4.13.6.shtml) release note file.

Maintainers, please set the 4.13.6 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](http://builds.jmri.org/jenkins/job/TestReleases/job/4.13.5/)
```
...
If you're developing any additional (post-4.13.5) changes that you want in the JMRI 4.14 production release, please start from this branch, i.e. do `git checkout -b release-4.13.5` to start your work.
```
...
- Update
        ...
        Description
        Git Modules: Branch
           Clone (Advanced) refspec should be +refs/heads/master:refs/remotes/origin/release-4.13.5
...
- On your local machine, open jvisualvm. Start "ant alltest" and attach jvisualvm to it. When it's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.13.5" Github issue so that historical resource usage info is available.
...
```
"First 4.13.5 files available":

First JMRI 4.13.5 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.13.5
...
- Ideally, start the work on either the release-4.13.5 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.13.5 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.13.5".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.13.5 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` file will have the wrong minor number in it:  You'll have to edit and commit that to get the right number in the release.
...
```
git checkout release-4.13.5
ant clean compile
cd target
rm -f properties.4.13.5.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.13.5' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.13.5.zip
cd ..
mkdir release
mv target/properties.4.13.5.zip release/
```
...
- Fill out form:
   ...
   - "tag version field" gets v4.13.5 (e.g. leading lower-case "v")
   - @ branch: select the release-4.13.5 release branch
```
"Release title" field gets "Test/Prod Release 4.13.5"
```
   - Description should contain (the releasesummary script above proposed this!):

```   
[Release notes](http://jmri.org/releasenotes/jmri4.13.5.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.13.5+R49e7a86.dmg](https://github.com/JMRI/JMRI/releases/download/v4.13.5/JMRI.4.13.5+R49e7a86.dmg) | 1c551c99825aa45331454bde7f600a861b2cc16c5687ca576234c4eac75cac79
[JMRI.4.13.5+R49e7a86.exe](https://github.com/JMRI/JMRI/releases/download/v4.13.5/JMRI.4.13.5+R49e7a86.exe) | c218a2f0d910a530867c079ceaf90f1534d523cd71f142b3e8174ab0f51ff0da
[JMRI.4.13.5+R49e7a86.tgz](https://github.com/JMRI/JMRI/releases/download/v4.13.5/JMRI.4.13.5+R49e7a86.tgz) | 9d577470a8f32e7f8ee365a959f1d910dce06fb4ee9914a9a27f1b1389f87abe
...
git merge origin/release-4.13.5
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.13.6". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.14 cycle. It's intended to be released around (July 12) from HEAD of master.
```

- Confirm that the tag for the current release (v4.13.5 for release 4.13.5) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.13.5) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.13.5 of JMRI/DecoderPro is available for download" or "JMRI 4.12 is available for download"
...
<http://jmri.org/releasenotes/jmri4.13.5.shtml>
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.13.5 -n "JMRI.4.13.5+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.13.1/workspace/dist/release/JMRI.4.13.1+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.13.5 -n "JMRI.4.13.5+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.13.5+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.13.1 -n "JMRI.4.13.5+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.13.5+Rd144052.tgz 
```
