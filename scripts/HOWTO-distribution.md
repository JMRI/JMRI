...
```
  ./scripts/update-HOWTO.sh 4.19.3 4.19.4 4.19.5
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.19.3

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create release-4.19.3". (This might already exist, if it was properly created at the end of the last build cycle)
...
```
git commit -m"for 4.19.3" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git checkout master
        git pull 
        cp jmri4.19.3.shtml jmri4.19.4.shtml
        (edit the new release note accordingly)
            change numbers throughout
            move new warnings to old
            remove old-version change notes
        git add jmri4.19.4.shtml
        git commit -m"start new 4.19.4 release note" jmri4.19.4.shtml
        git push github
        cd ../../(local JMRI copy)
```
...
- Merge the release note body from help/en/releasenotes/current-draft-note.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     bbedit help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.19.3.shtml
...
- Merge the new warnings (if any) from help/en/releasenotes/current-warnings.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     bbedit help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.19.3.shtml
     
 - add any new warnings to the old warnings section of the next (4.19.4) release note:
    bbedit ../website/releasenotes/jmri4.19.3.shtml ../website/releasenotes/jmri4.19.4.shtml
...
- Create the new draft note section
    ...
    git commit -m"start for 4.19.4 release note" help/en/releasenotes/current-draft-*.shtml

- Commit release note, push and pull back

    cd ../website/releasenotes
    git commit -m"updated 4.19.4 release note" jmri4.19.3.shtml jmri4.19.4.shtml
    git push github
    git pull
    cd ../../JMRI
...
```
git commit -m"for next release 4.19.4" pom.xml
git push github
```
...
```
The release-4.19.3 branch has been created. 

Maintainers, please set the 4.19.4 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](http://builds.jmri.org/jenkins/job/TestReleases/job/4.19.3/)
```
...
If you're developing any additional (post-4.19.3) changes that you want in the JMRI 4.18 production release, please start from this branch, i.e. do `git checkout -b release-4.19.3` to start your work.
```
...
and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.19.3" Github issue so that historical resource usage info is available.
...
```
"First 4.19.3 files available":

First JMRI 4.19.3 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.19.3
...
- Ideally, start the work on either the release-4.19.3 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.19.3 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.19.3".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.19.3 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.
...
```
git checkout release-4.19.3
ant realclean compile
cd target
rm -f properties.4.19.3.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.19.3' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.19.3.zip
cd ..
mkdir release
mv target/properties.4.19.3.zip release/
ls -lt release/
git checkout master
...
- Fill out form:
   ...
   - "tag version field" gets v4.19.3 (e.g. leading lower-case "v")
   - @ branch: select the release-4.19.3 release branch
   - "Release title" field gets "Prod/Test Release 4.19.3"
...
git merge origin/release-4.19.3
```
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.19.3 of JMRI/DecoderPro is available for download" or "JMRI 4.20 is available for download"
...
For more information on the issues, new features and bug fixes in 4.19.3 please see the release note:   
<https://www.jmri.org/releasenotes/jmri4.19.3.shtml>
...
- Close the [4.19.3 release GitHub Issue](https://github.com/JMRI/JMRI/issues) with a note saying that
```
JMRI 4.19.3 has been released. Files are available in the GitHub release section.
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.19.3/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
and expansion; 
...
```    
        ...
        git checkout -b release-4.19.3
        git push github release-4.19.3
        git checkout master    
        git pull
```
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.19.3 -n "JMRI.4.19.3+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.19.3/workspace/dist/release/JMRI.4.19.3+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.19.3 -n "JMRI.4.19.3+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.19.3/workspace/dist/release/JMRI.4.19.3+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.19.3 -n "JMRI.4.19.3+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.19.3/workspace/dist/release/JMRI.4.19.3+Rd144052.tgz 
```
