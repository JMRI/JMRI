...
```
  ./scripts/update-HOWTO.sh 4.21.4 4.21.5 4.21.6
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.21.4

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.21.4". (This might already exist, if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
git commit -m"for 4.21.4" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git checkout master
        git pull 
        cp jmri4.21.4.shtml jmri4.21.5.shtml
        $EDITOR jmri4.21.5.shtml
        ...
        git add jmri4.21.5.shtml
        git commit -m"start new 4.21.5 next release note" jmri4.21.5.shtml
        git push github
        cd ../../(local JMRI copy)
```
...
- Merge the release note body from help/en/releasenotes/current-draft-note.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.21.4.shtml
...
- Merge the new warnings (if any) from help/en/releasenotes/current-warnings.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.21.4.shtml
     
 - add any new warnings to the old warnings section of the next (4.21.5) release note:
    ${EDITOR} ../website/releasenotes/jmri4.21.5.shtml ../website/releasenotes/jmri4.21.4.shtml
...
- Create the new draft note section
    ...
    git commit -m"start for 4.21.5 release note" help/en/releasenotes/*.shtml
    git push github
    git pull
...
- Commit release note, push and pull back
    ...
    git commit -m"updated 4.21.4 release note" jmri4.21.5.shtml jmri4.21.4.shtml
    git push github
    git pull
    cd ../../JMRI
...
```
git commit -m"for next release 4.21.5" pom.xml
git push github
```
...
```
The release-4.21.4 branch has been created. 

Maintainers, please set the 4.21.5 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.
...
and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.21.4" Github issue so that historical resource usage info is available.
...
```
"First 4.21.4 files available":

First JMRI 4.21.4 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.21.4/
...
- Ideally, start the work on either the release-4.21.4 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.21.4 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.21.4".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.21.4 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` and `pom.xml` files will have the wrong minor number in them:  You'll have to edit and commit that to get the right number in the release.
...
- Make sure that the 4.21.4 milestone is on the original PR

- If the PR has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.21.5 release note.  Merge these as needed to the release-4.21.5 and master branches
...
```
git checkout release-4.21.4
ant realclean compile
cd target
rm -f properties.4.21.4.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.21.4' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.21.4.zip
cd ..
mkdir release
mv target/properties.4.21.4.zip release/
ls -lt release/
git checkout master
...
- Fill out form:
   ...
   - "tag version field" gets v4.21.4 (e.g. leading lower-case "v")
   - @ branch: select the release-4.21.4 release branch
   - "Release title" field gets "Prod/Test Release 4.21.4"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```   
[Release notes](https://jmri.org/releasenotes/jmri4.21.4.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.21.4+R63094d48b.dmg](https://github.com/JMRI/JMRI/releases/download/v4.21.4/JMRI.4.21.4+R63094d48b.dmg) | 20c2000b00ca9b6c403bb17a187a2e9670b4c27e4a64a12ca3d592b888ca1a3a
[JMRI.4.21.4+R63094d48b.exe](https://github.com/JMRI/JMRI/releases/download/v4.21.4/JMRI.4.21.4+R63094d48b.exe) | b4ed4c1cd7c61becbd10f964cba7e85579e3b7101f7b3e858f28272fb7e86a6d
[JMRI.4.21.4+R63094d48b.tgz](https://github.com/JMRI/JMRI/releases/download/v4.21.4/JMRI.4.21.4+R63094d48b.tgz) | 057fa69849f5383def665dd9e7e84335ada9e39a5d8b60c5f1c39227f9ba40ae
...
git merge origin/release-4.21.4
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.21.5". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.22 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```

- Confirm that the tag for the current release (v4.21.4 for release 4.21.4) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.21.4) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
     ${EDITOR}  releasenotes/jmri4.21.4.shtml
```

- Commit site, push to github
```
    git commit -m"4.21.4 web site" .
    git push github
    git pull
```
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.21.4 of JMRI/DecoderPro is available for download" or "JMRI 4.22 is available for download"

    Content:
    
Test version 4.21.4 of JMRI/DecoderPro is available for download.
...
For more information on the issues, new features and bug fixes in 4.21.4 please see the release note:   
<https://www.jmri.org/releasenotes/jmri4.21.4.shtml>
...
- Close the [4.21.5 release GitHub Issue](https://github.com/JMRI/JMRI/issues) with a note saying that
```
JMRI 4.21.4 has been released. Files are available in the GitHub release section.
...
```    
        ...
        git checkout -b release-4.21.5
        git push github release-4.21.5
        git checkout master    
        git pull
```
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.5 -n "JMRI.4.21.5+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.5/workspace/dist/release/JMRI.4.21.5+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.5 -n "JMRI.4.21.5+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.5/workspace/dist/release/JMRI.4.21.5+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.21.5 -n "JMRI.4.21.5+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.21.5/workspace/dist/release/JMRI.4.21.5+Rd144052.tgz 
```
