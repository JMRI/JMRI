...
- To check the script ran OK, the following should be the release you're doing now: 4.17.2

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create release-4.17.2". (This might already exist, if it was properly created at the end of the last build cycle)
...
```
git commit -m"for 4.17.2" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git pull 
        cp jmri4.17.2.shtml jmri4.17.3.shtml
        (edit the new release note accordingly)
            change numbers throughout
            move new warnings to old
            remove old-version change notes
        git add jmri4.17.3.shtml
        git commit -m"start new 4.17.3 release note" jmri4.17.3.shtml
        git push github
        cd (local JMRI copy)
```
...
- Commit release note, push and pull back
    ...
    git commit -m"updated 4.17.2 release note" jmri4.17.2.shtml jmri4.17.3.shtml
    git push github
    git pull
    cd ../../JMRI
...
```
git commit -m"for next release 4.17.3" pom.xml
git push github
```
...
```
The release-4.17.2 branch has been created. 

Maintainers, please set the 4.17.3 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.
...
From now on, please document your changes in the [jmri4.17.2.shtml](https://github.com/JMRI/website/blob/master/releasenotes/jmri4.17.2.shtml) release note file.
...
If you're developing any additional (post-4.17.2) changes that you want in the JMRI 4.16 production release, please start from this branch, i.e. do `git checkout -b release-4.17.2` to start your work.
```
...
and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.17.2" Github issue so that historical resource usage info is available.
...
```
"First 4.17.2 files available":

First JMRI 4.17.2 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.17.2
...
- Ideally, start the work on either the release-4.17.2 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.17.2 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.17.2".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.17.2 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` file will have the wrong minor number in it:  You'll have to edit and commit that to get the right number in the release.
...
ant clean compile
cd target
rm -f properties.4.17.2.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.17.2' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.17.2.zip
cd ..
mkdir release
mv target/properties.4.17.2.zip release/
ls -lt release/
git checkout master
...
- Fill out form:
   ...
   - "tag version field" gets v4.17.2 (e.g. leading lower-case "v")
   - @ branch: select the release-4.17.2 release branch
   - "Release title" field gets "Prod/Test Release 4.17.2"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```   

[Release notes](http://jmri.org/releasenotes/jmri4.17.2.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.17.2+Rf006fc4.dmg](https://github.com/JMRI/JMRI/releases/download/v4.17.2/JMRI.4.17.2+Rf006fc4.dmg) | e4a438800095aded9d8d36d3fa7b766f3e17db366327d435a8f125634b31b4d3
[JMRI.4.17.2+Rf006fc4.exe](https://github.com/JMRI/JMRI/releases/download/v4.17.2/JMRI.4.17.2+Rf006fc4.exe) | a0212fa68718214074cfd4202bf31d29e479b909eec1210e61eab03869b92990
[JMRI.4.17.2+Rf006fc4.tgz](https://github.com/JMRI/JMRI/releases/download/v4.17.2/JMRI.4.17.2+Rf006fc4.tgz) | ec280f9c317f6b458e5b21e207a04666b89f9525f41a0e097aa08c817cb777f7
...
git merge origin/release-4.17.2
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.17.3". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.16 cycle. It's intended to be created around (July 12) from the `HEAD` of the `master` branch.
```

- Confirm that the tag for the current release (v4.17.2 for release 4.17.2) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.17.2) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.17.2 of JMRI/DecoderPro is available for download" or "JMRI 4.16 is available for download"
...
<http://jmri.org/releasenotes/jmri4.17.2.shtml>
...
- Close the [4.17.2 release GitHub Issue](https://github.com/JMRI/JMRI/issues) with a note saying that
```
JMRI 4.17.2 has been released. Files are available in the GitHub release section.
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.17.2/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
and expansion; 
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.17.2 -n "JMRI.4.17.2+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.17.2/workspace/dist/release/JMRI.4.17.2+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.17.2 -n "JMRI.4.17.2+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.17.2/workspace/dist/release/JMRI.4.17.2+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.17.2 -n "JMRI.4.17.2+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.17.2/workspace/dist/release/JMRI.4.17.2+Rd144052.tgz 
```
