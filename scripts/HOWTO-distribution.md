...
```
  ./scripts/update-HOWTO.sh 4.15.6 4.15.7 4.15.7
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.15.6

================================================================================
## Notification

- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create release-4.15.6". (This might already exist, if it was properly created at the end of the last build cycle)
...
```
git commit -m"for 4.15.6" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git add jmri4.15.7.shtml
        git commit -m"start new 4.15.7 release note" jmri4.15.7.shtml
        git push github
        cd (local JMRI copy)
```
...
- Merge the release note body from help/en/releasenotes/current-draft-note.shtml in the JMRI/JMRI repository into the actual release note in website repository:
     bbedit help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.15.6.shtml
...
```
git commit -m"for next release 4.15.7" pom.xml
git push github
```
...
```
The release-4.15.6 branch has been created. 
...
Jenkins will be creating files shortly at the [CI server](http://builds.jmri.org/jenkins/job/TestReleases/job/4.15.6/)
```
...
From now on, please document your changes in the [jmri4.15.6.shtml](https://github.com/JMRI/website/blob/master/releasenotes/jmri4.15.6.shtml) release note file.
...
If you're developing any additional (post-4.15.6) changes that you want in the JMRI 4.16 production release, please start from this branch, i.e. do `git checkout -b release-4.15.6` to start your work.
```
...
and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.15.6" Github issue so that historical resource usage info is available.
...
```
"First 4.15.6 files available":

First JMRI 4.15.6 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.15.6
...
- Ideally, start the work on either the release-4.15.6 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.15.6 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.15.6".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.15.6 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` file will have the wrong minor number in it:  You'll have to edit and commit that to get the right number in the release.
...
```
git checkout release-4.15.6
ant clean compile
cd target
rm -f properties.4.15.6.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.15.6' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.15.6.zip
cd ..
mkdir release
...
- Fill out form:
   ...
   - "tag version field" gets v4.15.6 (e.g. leading lower-case "v")
   - @ branch: select the release-4.15.6 release branch
   - "Release title" field gets "Prod/Test Release 4.15.6"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```   
[Release notes](http://jmri.org/releasenotes/jmri4.15.6.shtml)
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.15.6 of JMRI/DecoderPro is available for download" or "JMRI 4.16 is available for download"

    Content:
    
Test version 4.15.6 of JMRI/DecoderPro is available for download.
...
For more information on the issues, new features and bug fixes in 4.15.6 please see the release note:   
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.15.6/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
and expansion; 
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.15.6 -n "JMRI.4.15.6+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.15.6/workspace/dist/release/JMRI.4.15.6+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.15.6 -n "JMRI.4.15.6+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.15.6/workspace/dist/release/JMRI.4.15.6+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.15.6 -n "JMRI.4.15.6+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.15.6/workspace/dist/release/JMRI.4.15.6+Rd144052.tgz 
```
