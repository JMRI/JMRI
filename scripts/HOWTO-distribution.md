...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create release-4.17.1". (This might already exist, if it was properly created at the end of the last build cycle)
...
```
git commit -m"for 4.17.1" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git pull 
        cp jmri4.17.1.shtml jmri4.17.2.shtml
        (edit the new release note accordingly)
            change numbers throughout
            move new warnings to old
            remove old-version change notes
        git add jmri4.17.2.shtml
        git commit -m"start new 4.17.2 release note" jmri4.17.2.shtml
        git push github
        cd (local JMRI copy)
```
...
- Create the new draft note section
    ...
    git commit -m"start for 4.17.2 release note" help/en/releasenotes/current-draft-note.shtml
...
```
git commit -m"for next release 4.17.2" pom.xml
git push github
```
...
and attach jvisualvm to the AllTest class when it appears. When that's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.17.1" Github issue so that historical resource usage info is available.
...
```
"First 4.17.1 files available":

First JMRI 4.17.1 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.17.1
...
- Ideally, start the work on either the release-4.17.1 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.17.1 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.17.1".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
```
git checkout release-4.17.1
ant clean compile
cd target
rm -f properties.4.17.1.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.17.1' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.17.1.zip
cd ..
mkdir release
mv target/properties.4.17.1.zip release/
ls -lt release/
git checkout master
...
- Fill out form:
   ...
   - "tag version field" gets v4.17.1 (e.g. leading lower-case "v")
   - @ branch: select the release-4.17.1 release branch
   - "Release title" field gets "Prod/Test Release 4.17.1"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```   

[Release notes](http://jmri.org/releasenotes/jmri4.17.1.shtml)
...
git merge origin/release-4.17.1
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.17.2". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.16 cycle. It's intended to be created around (July 12) from the `HEAD` of the `master` branch.
```

- Confirm that the tag for the current release (v4.17.1 for release 4.17.1) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.17.1) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
- Mail announcement to jmriusers@groups.io
    ...
    Subject is "Test version 4.17.1 of JMRI/DecoderPro is available for download" or "JMRI 4.16 is available for download"

    Content:
    
Test version 4.17.1 of JMRI/DecoderPro is available for download.
...
For more information on the issues, new features and bug fixes in 4.17.1 please see the release note:   
<http://jmri.org/releasenotes/jmri4.17.1.shtml>
...
- Close the [4.17.1 release GitHub Issue](https://github.com/JMRI/JMRI/issues) with a note saying that
```
JMRI 4.17.1 has been released. Files are available in the GitHub release section.
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.17.1/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
and expansion; 
