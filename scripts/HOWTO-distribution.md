...
```
  ./scripts/update-HOWTO.sh 4.11.6 4.11.7 4.11.8
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.11.6
...
```
git commit -m"for 4.11.6" scripts/HOWTO-distribution.md
git push github
```
...
```    
        ...
        git pull 
        cp jmri4.11.6.shtml jmri4.11.7.shtml
        (edit the new release note accordingly)
            change numbers throughout
            move new warnings to old
            remove old-version change notes
        git add jmri4.11.7.shtml
        git commit -m"start new 4.11.7 release note" jmri4.11.7.shtml
        git push github
        cd (local JMRI copy)
```
...
```
The release-4.11.6 branch has been created. 

From now on, please document your changes in the [jmri4.11.7.shtml](https://github.com/JMRI/website/blob/master/releasenotes/jmri4.11.7.shtml) release note file.

Maintainers, please set the 4.11.7 milestone on pulls from now on, as that will be the next test release from the HEAD of the master branch.

Jenkins will be creating files shortly at the [CI server](http://builds.jmri.org/jenkins/job/TestReleases/job/4.11.6/)
````
...
If you're developing any additional (post-4.11.6) changes that you want in the JMRI 4.10 production release, please start from this branch, i.e. do
```
git checkout -b release-4.11.6
```
to start your work.
```
...
- On your local machine, open jvisualvm. Start "ant alltest" and attach jvisualvm to it. When it's done, put a screen-shot of the four monitor graphs into the "Create Test Release 4.11.6" Github issue so that historical resource usage info is available.
...
- Announce the file set via email to jmri-developers@lists.sf.net with a subject line "First 4.11.6 files available":

First JMRI 4.11.6 files are available in the usual way at:

http://builds.jmri.org/jenkins/job/TestReleases/job/4.11.6
...
- Ideally, start the work on either the release-4.11.6 branch (if working after that was started) or on a branch-from-master that's _before_ the release-4.11.6 branch was created.  That way, the change can be cleanly included in the release branch, and also directly onto master.
...
- On the GitHub web site, make _two_ pull requests:  
   ...
   - One to the release branch e.g. "release-4.11.6".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Merge master into the release-4.11.6 branch.  This will bring _everything_ that's been merged in, so remember to update the version markers on those PRs.  Effectively, you've just started the release process later.  Note that the `release.properties` file will have the wrong minor number in it:  You'll have to edit and commit that to get the right number in the release.
...
cd target
rm -f properties.4.11.6.zip

# the following will take several minutes
foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.11.6' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.11.6.zip

Then decide what to do with it. Tentatively, we're just attaching the properties.4.11.6.zip file to the release without further mention.
...
- Fill out form:
   ...
   - "tag version field" gets v4.11.6 (e.g. leading lower-case "v")
   - @ branch: select the release-4.11.6 release branch
```
"Release title" field gets "Test/Prod Release 4.11.6"
```
   - Description should contain (the releasesummary script above proposed this!):

```   
[Release notes](http://jmri.org/releasenotes/jmri4.11.6.shtml)

Checksums:

File | SHA256 checksum
---|---
[JMRI.4.11.6.R72446d4.dmg](https://github.com/JMRI/JMRI/releases/download/v4.11.6/JMRI.4.11.6.R72446d4.dmg) | 7852de77d6bf44c8a677d3229ebaa38115258a0afc518667e3e7f8d6a89a2ce3
[JMRI.4.11.6.R72446d4.exe](https://github.com/JMRI/JMRI/releases/download/v4.11.6/JMRI.4.11.6.R72446d4.exe) | 8a2b32d6c1da0664502b2d4660c93c5853fc9b988a29bfd0255586c56fe91499
[JMRI.4.11.6.R72446d4.tgz](https://github.com/JMRI/JMRI/releases/download/v4.11.6/JMRI.4.11.6.R72446d4.tgz) | 46bf97d949c43e6d7c95d61c741efb12698158658b5ccebd3cffb892e3803c19
...
```
curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.11.6/lastSuccessfulBuild/artifact/dist/release/*zip*/release.zip"" 
```
...
git merge origin/release-4.11.6
```
...
- Create the next [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.11.7". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 4.12 cycle. It's intended to be released around May 12 from HEAD of master.
```

- Confirm that the tag for the current release (v4.11.6 for release 4.11.6) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.11.6) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).
...
- Mail announcement to jmriusers@yahoogroups.com
    ...
    Subject is "Test version 4.11.6 of JMRI/DecoderPro is available for download" or "JMRI 4.8 is available for download"

    Content:
    
Test version 4.11.6 of JMRI/DecoderPro is available for download.
...
If you use JMRI on Linux or Mac and are updating from JMRI 4.7.3 or earlier, there’s a necessary migration step. (Not needed on Windows) Please see the release note for details: <http://jmri.org/releasenotes/jmri4.11.6.shtml#migration>

For more information on the issues, new features and bug fixes in 4.11.6 please see the release note:
<http://jmri.org/releasenotes/jmri4.11.6.shtml>
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.11.6 -n "JMRI.4.11.6+Rd144052.dmg" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.11.6/workspace/dist/release/JMRI.4.11.6+Rd144052.dmg 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.11.6 -n "JMRI.4.11.6+Rd144052.exe" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.11.6+Rd144052.exe 
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.11.6 -n "JMRI.4.11.6+Rd144052.tgz" -f /var/lib/jenkins/jobs/TestReleases/jobs/4.7.5/workspace/dist/release/JMRI.4.11.6+Rd144052.tgz 
```
...
(replace "user" below with your SourceForge.net user name; must have SSH keys for SourceForge.net set up)
 ...
 - (The "./testrelease 4.11.6" local script on shell.sf.net does the following steps, except for the edit, of course)
```
    ssh user,jmri@shell.sf.net create
    ssh user,jmri@shell.sf.net
    curl -o release.zip "http://builds.jmri.org/jenkins/job/Test%20Releases/job/4.11.6/ws/dist/release/*zip*/release.zip"
        (use the following instead if building on second Jenkins server)
    curl -o release.zip "http://builds.jmri.org/jenkins/job/TestReleases/job/4.11.6/ws/dist/release/*zip*/release.zip"
    rm release/JMRI*
    unzip release.zip
    cd release
