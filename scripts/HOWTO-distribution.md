...
```
  ./scripts/update-HOWTO.sh 4.99.6 4.99.7 4.99.7
```
...
- To check the script ran OK, the following should be the release you're doing now: 4.99.6
...
- Create a [GitHub Issue](https://github.com/JMRI/JMRI/issues) to hold discussion with conventional title "Create Test Release 4.99.6". (This [might already exist](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+is%3Aopen+%22Create+Test+Release+4.99.6%22), if it was properly created at the end of the last build cycle)  Typical content:
```
...
```
        git commit -m"for 4.99.6" scripts/HOWTO-distribution.md
        git push github
```
...
```
        ...
        git checkout master
        git pull
        cp jmri4.99.6.shtml jmri4.99.7.shtml
        $EDITOR jmri4.99.7.shtml
        ...
        git add jmri4.99.7.shtml
        git commit -m"start new 4.99.7 next release note" jmri4.99.7.shtml
        git push github
        cd ../../JMRI
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-note.shtml ../website/releasenotes/jmri4.99.6.shtml
```
...
```
        ${EDITOR} help/en/releasenotes/current-draft-warnings.shtml ../website/releasenotes/jmri4.99.6.shtml
```

 - Add any new warnings to the old warnings section of the next (4.99.7) release note:
```
        ${EDITOR} ../website/releasenotes/jmri4.99.7.shtml ../website/releasenotes/jmri4.99.6.shtml
```

- Clean out the unneeded sections from the release note
```
        ${EDITOR} ../website/releasenotes/jmri4.99.6.shtml
```
...
```
        ...
        git commit -m"start for 4.99.7 release note" help/en/releasenotes/*.shtml
        git push github
        git pull
```
...
Maintainers, please set the 4.99.7 milestone on pulls from now on, as that will be the next test release.
...
- Check the [signing-wget.log](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.6/ws/%20signing-wget.log) file in the job’s workspace to make sure all the return codes are zero. This is the macOS signing process. A non-zero code indicates a problem with the signature.
...
First JMRI 4.99.6 files are available in the usual way at:

https://builds.jmri.org/jenkins/job/testreleases/job/4.99.6/
...
- On the GitHub web site, make _two_ pull requests:
   ...
   - One to the release branch e.g. "release-4.99.6".  The comment on this PR should explain why this should be included instead of waiting for the next release.
...
- Make sure that the 4.99.6 milestone is on the original PR and any others that have been pulled in

- If the PR(s) has any changes to the help/en/releasenotes directory, go through the steps to update the master if any section(s) were added, and to move notes and warnings to the 4.99.6 release note.  Merge these as needed to the release-4.99.6 and master branches
...
cd target
rm -f properties.4.99.6.zip

foreach x ( `find classes -name \*.properties` )
printf '%s\n' 0a '# from tag v4.99.6' . x | ex $x
end

find classes -name \*.properties | zip -@ properties.4.99.6.zip
cd ..
mkdir release
mv target/properties.4.99.6.zip release/
ls -lt release/
git checkout master
...
```
${EDITOR} ../website/releaselist ../website/releasenotes/jmri4.99.6.shtml scripts/HOWTO-distribution.md
```
...
- Disable the Jenkins [release-build project](https://builds.jmri.org/jenkins/job/testreleases/job/4.99.6/); this is so it doesn't fail after later steps
...
- Fill out form:
   ...
   - "tag version field" gets v4.99.6 (e.g. leading lower-case "v")
   - @ branch: select the release-4.99.6 release branch
   - "Release title" field gets "Prod/Test Release 4.99.6"
   - Description should contain text like (the releasesummary script above provided the correct filenames and hashes):

```
[Release notes](https://jmri.org/releasenotes/jmri4.99.6.shtml)
...
git merge origin/release-4.99.6
```
...
- Create the [next GitHub Issue](https://github.com/JMRI/JMRI/issues/new) to hold discussion with conventional title "Create Test Release 4.99.7". Add the next release milestone (created above) to it. Typical text (get the date from the [milestone page](https://github.com/JMRI/JMRI/milestones)); for later releases in the series copy specific text from the milestone page:
```
This is the next release in the 5.0 cycle. It's intended to be created from the `HEAD` of the `master` branch.
```
- Add the 4.99.7 milestone to the issue.

- Confirm that the tag for the current release (v4.99.6 for release 4.99.6) is in place via the [tags page](https://github.com/JMRI/JMRI/tags), then manually delete the current release branch (release-4.99.6) via the [GitHub branches page](https://github.com/JMRI/JMRI/branches).  (N.B. We are experimenting with having the `release*` branches protected, in which case you may have to go to Setting; Branches; then edit the release* branch name to releaseX* to disable the protection before removing the branch.  If you do that, remember to replace the protection!)
...
```
        ${EDITOR}  releasenotes/jmri4.99.6.shtml
```
...
Content:
    ...
    For more information on the issues, new features and bug fixes in 4.99.6 please see the release note:
    <https://www.jmri.org/releasenotes/jmri4.99.6.shtml>
...
- Close the [4.99.6 release GitHub Issue](https://github.com/JMRI/JMRI/issues?q=is%3Aissue+%22Create+Test+Release+4.99.6%22) with a comment saying that
```
    JMRI 4.99.6 has been released. Files are available in the GitHub release section.
...
```
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.7 -n "JMRI.4.99.7+Rd144052.dmg" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.6/builds/4/archive/dist/release/JMRI.4.99.7+Rd144052.dmg
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.7 -n "JMRI.4.99.7+Rd144052.exe" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.6/builds/4/archive/dist/release/JMRI.4.99.7+Rd144052.exe
github-release upload -s {github_secret} -u JMRI -r JMRI -t v4.99.7 -n "JMRI.4.99.7+Rd144052.tgz" -f /var/lib/jenkins/jobs/testreleases/jobs/4.99.6/builds/4/archive/dist/release/JMRI.4.99.7+Rd144052.tgz
```
