...
# version 1.4 - 3/5/21 Support for NCE address packet format by SteamBigBoy
...
def whenSendDccButtonClicked(event) :
     ...
     if  typePacket == 0 :
         ...
         addr = int(sgAddress.text) + addressShift # change text to a number and subtrac one
...
def whenSkButtonClicked(event) :
     global addressShift
     addressShift = 3
     return

def whenSmButtonClicked(event) :
     global addressShift
     addressShift = -1
     return
...
skButton = javax.swing.JRadioButton("NCE Address Shift")
smButton = javax.swing.JRadioButton("NMRA Address")
...
skButton.actionPerformed = whenSkButtonClicked
smButton.actionPerformed = whenSmButtonClicked
...
f = javax.swing.JFrame("LocoNet Send Tool for NCE")
...
group1.add(sgButton)
group1.add(skButton)
group2 = javax.swing.ButtonGroup()
group2.add(skButton)
group2.add(smButton)
