...
print "Loading USBDriver : PLAYSTATION 3 Controller"

class USBDriver :
    def __init__(self):
        self.componentNextThrottleFrame = "B"  # Component for throttle frames browsing
        self.valueNextThrottleFrame = 1
        self.componentPreviousThrottleFrame = "Y"
        self.valuePreviousThrottleFrame = 1
        
        self.componentNextRunningThrottleFrame = "X"  # Component for running throttle frames browsing
        self.valueNextRunningThrottleFrame = 1
        self.componentPreviousRunningThrottleFrame = "A"
        self.valuePreviousRunningThrottleFrame = 1
        
        # From there available only when no throttle is active in current window  
        self.componentNextRosterBrowse = "Start"  # Component for roster browsing
        self.valueNextRoster = 1
        self.componentPreviousRosterBrowse = "Select"
        self.valuePreviousRoster = 1
        
        self.componentRosterSelect = "Mode"  # Component to select a roster
        self.valueRosterSelect = 1
        
        # From there available only when a throttle is active in current window        
        self.componentThrottleRelease = "Mode"  # Component to release current throttle
