...
/**
 * Create a Block that can control a locomotive
 * within a specific Block based on an RpsSensor.  
 * It sets speed based on aspect of a specific signal
 *
 *
...
public class RpsBlock implements java.beans.PropertyChangeListener, jmri.ThrottleListener {
    ...
    void handleParameterChange(String property, 
                                ...
                                Object source) {
        if (log.isDebugEnabled()) log.debug("Change "+property+" from "+source);
    ...
    void acquireThrottle(Integer num) {
        ...
        if (o != null) return;  // already present
    }
