...
abstract public class AbstractAlgorithmTest extends TestCase {
	...
	public void testCalc(Point3d[] receivers, double[] distances, Point3d result, String label,
	    ...
	    if (!(Math.abs(m.y-result.y)<tolerance)) System.err.println(label+" y not close "+m.y+" "+result.y);
	    Assert.assertEquals(label+" y close", true, Math.abs(m.y-result.y)<tolerance);

	    if (!(Math.abs(m.z-result.z)<tolerance)) System.err.println(label+" z not close "+m.z+" "+result.z);
	    Assert.assertEquals(label+" z close", true, Math.abs(m.z-result.z)<tolerance);

	    if (m.getCode()>codemax || m.getCode()<codemin) System.err.println(label+" bad code "+m.getCode());
