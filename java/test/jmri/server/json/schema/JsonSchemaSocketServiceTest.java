...
public class JsonSchemaSocketServiceTest {
    ...
    public void testOnMessageInvalidRequests() throws IOException, JmriException, JsonException {
        ...
        // DELETE should fail
        try {
            instance.onMessage(JSON.SCHEMA, data, JSON.DELETE, locale);
        ...
        try {
            instance.onMessage(JSON.TYPE, data.put(JSON.NAME, "invalid-type"), JSON.GET, locale);
            Assert.fail("Expected exception not thrown.");
        } catch (JsonException ex) {
        ...
        // POST should fail
        try {
            instance.onMessage(JSON.SCHEMA, data, JSON.POST, locale);
            Assert.fail("Expected exception not thrown.");
        } catch (JsonException ex) {
            Assert.assertEquals("Error is HTTP 405", 405, ex.getCode());
        ...
        // PUT should fail
        try {
            instance.onMessage(JSON.SCHEMA, data, JSON.PUT, locale);
            Assert.fail("Expected exception not thrown.");
        } catch (JsonException ex) {
            Assert.assertEquals("Error is HTTP 405", 405, ex.getCode());
        ...
        try {
            instance.onMessage(JSON.SCHEMA, data, "TRACE", locale);
            Assert.fail("Expected exception not thrown.");
        } catch (JsonException ex) {
            Assert.assertEquals("Error is HTTP 405", 405, ex.getCode());
    ...
    public void testOnMessageValidRequests() throws IOException, JmriException, JsonException {
        ...
        try {
            instance.onMessage(type, mapper.readTree("{\"name\":\"schema\", \"client\":false}"), JSON.GET, locale);
        } catch (JsonSchemaException ex) {
            JUnitAppender.assertErrorMessage("Failed to load json schema!");
            Assume.assumeNoException("Unable to reach json-schema.org", ex);
        }
        ...
        try {
            instance.onMessage(type, mapper.readTree("{\"name\":\"schema\", \"client\":true}"), JSON.GET, locale);
        } catch (JsonSchemaException ex) {
            JUnitAppender.assertErrorMessage("Failed to load json schema!");
            Assume.assumeNoException("Unable to reach json-schema.org", ex);
        }
