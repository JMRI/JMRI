...
public class JsonSignalHeadHttpServiceTest {
    ...
    public void testDoGet() throws JmriException {
        ...
        try {
            ...
            result = service.doGet(JsonSignalHead.SIGNAL_HEAD, sysName, service.getObjectMapper().createObjectNode(), locale, 42);
            ...
            result = service.doGet(JsonSignalHead.SIGNAL_HEAD, userName, service.getObjectMapper().createObjectNode(), locale, 42);
            Assert.assertNotNull(result);
            Assert.assertEquals(sysName, result.path(JSON.DATA).path(JSON.NAME).asText());
