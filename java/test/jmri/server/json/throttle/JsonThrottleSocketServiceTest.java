...
public class JsonThrottleSocketServiceTest {
    ...
    public void testOnList() {
        ...
        try {
            service.onList(JsonThrottle.THROTTLE, connection.getObjectMapper().createObjectNode(), Locale.ENGLISH);
    ...
    public void testRunThrottleAddress() throws IOException, JmriException, JsonException {
        ...
        // get the throttle
        data.put(JsonThrottle.THROTTLE, "42").put(JSON.ADDRESS, 42);
        service.onMessage(JsonThrottle.THROTTLE, data, JSON.POST, Locale.ENGLISH);
        ...
        Assert.assertEquals("Throttle status has 35 data elements", 35, message.findPath(JSON.DATA).size());
        connection.sendMessage((JsonNode) null); // clear messages
        // set a speed of 50% in reverse
        data = connection.getObjectMapper().createObjectNode().put(JsonThrottle.THROTTLE, "42");
    ...
    public void testRunThrottleRosterEntry() throws IOException, JmriException, JsonException {
        ...
        // get the throttle
        data.put(JsonThrottle.THROTTLE, "42").put(JSON.ID, 42);
        service.onMessage(JsonThrottle.THROTTLE, data, JSON.POST, Locale.ENGLISH);
        Assert.assertEquals("One throttle", 1, manager.getThrottles().size());
    ...
    public void testRequestThrottleNoAddress() throws IOException, JmriException {
        ...
        JsonThrottleSocketService service = new JsonThrottleSocketService(connection);
        ObjectNode data = connection.getObjectMapper().createObjectNode();
        data.put(JsonThrottle.THROTTLE, "42");
        try {
            service.onMessage(JsonThrottle.THROTTLE, data, JSON.POST, Locale.ENGLISH);
            Assert.fail("Expected exception not thrown");
        } catch (JsonException ex) {
