...
public class SerialAddressTest {
    ...
    public void testValidateSystemNameFormat() {
        Assert.assertTrue("valid format - KL2", SerialAddress.validSystemNameFormat("KL2", 'L', "K"));

        Assert.assertTrue("invalid format - KL", !SerialAddress.validSystemNameFormat("KL", 'L', "K"));
        JUnitAppender.assertWarnMessage("missing numerical node address in system name: KL");

        Assert.assertTrue("valid format - KL2005", SerialAddress.validSystemNameFormat("KL2005", 'L', "K"));
        Assert.assertTrue("valid format - KT2005", SerialAddress.validSystemNameFormat("KT2005", 'T', "K"));
        Assert.assertTrue("valid format - KS205", SerialAddress.validSystemNameFormat("KS205", 'S', "K"));

        Assert.assertTrue("invalid format - KY2005", !SerialAddress.validSystemNameFormat("KY2005", 'L', "K"));
        JUnitAppender.assertErrorMessage("invalid character in header field of system name: KY2005");

        Assert.assertTrue("valid format - KL1", SerialAddress.validSystemNameFormat("KL1", 'L', "K"));
        Assert.assertTrue("valid format - KL1000", SerialAddress.validSystemNameFormat("KL1000", 'L', "K"));

        // note: address format is invalid (out of range) as checked upon user input
        Assert.assertTrue("invalid format - KL0", !SerialAddress.validSystemNameFormat("KL0", 'L', "K"));
        JUnitAppender.assertWarnMessage("node address field out of range in system name - KL0");

        Assert.assertTrue("valid format - KL2999", SerialAddress.validSystemNameFormat("KL2999", 'L', "K"));

        Assert.assertTrue("valid format - KL7999", SerialAddress.validSystemNameFormat("KL7999", 'L', "K"));
