...
public class SlotManagerTest {
    ...
    @Test
    public void testReadCVDirectBit() throws jmri.ProgrammerException {
        log.debug(".... start testReadCVDirect ...");
        String CV1 = "35";
        slotmanager.setMode(ProgrammingMode.DIRECTBITMODE);
        slotmanager.readCV(CV1, lstn);
        Assert.assertEquals("read message",
                "EF 0E 7C 0B 00 00 00 00 00 22 00 7F 7F 00",
                lnis.outbound.elementAt(lnis.outbound.size() - 1).toString());
        Assert.assertEquals("one message sent", 1, lnis.outbound.size());
        Assert.assertEquals("initial status", -999, status);
        ...
        value = 0;
        log.debug("send E7 reply back");
        slotmanager.message(new LocoNetMessage(new int[]{0xE7, 0x0E, 0x7C, 0x0B, 0x00, 0x00, 0x02, 0x47, 0x00, 0x1C, 0x79, 0x7F, 0x7F, 0x3B}));
        JUnitUtil.waitFor(()->{return value == 121;},"value == 121 not set");
        log.debug("checking..");
        Assert.assertEquals("reply status", 0, status);
        Assert.assertEquals("reply value", 121, value);
    ...
    @Test
    public void testWriteCVDirectBit() throws jmri.ProgrammerException {
        String CV1 = "19";
        int val2 = 38;
        ProgListener p3 = null;
        slotmanager.setMode(ProgrammingMode.DIRECTBITMODE);
        slotmanager.writeCV(CV1, val2, p3);
        Assert.assertEquals("write message",
                "EF 0E 7C 4B 00 00 00 00 00 12 26 7F 7F 00",
                lnis.outbound.elementAt(lnis.outbound.size() - 1).toString());
    }
    ...
    public void testGetProgrammingModes() {
        ...
        Assert.assertEquals("programming mode list length ok", 6, l.size());
        ...
        Assert.assertEquals("programming mode 2", ProgrammingMode.DIRECTBITMODE, l.get(2));
        Assert.assertEquals("programming mode 3", ProgrammingMode.REGISTERMODE, l.get(3));
        Assert.assertEquals("programming mode 4", ProgrammingMode.ADDRESSMODE, l.get(4));
        Assert.assertEquals("programming mode 5", "LOCONETCSOPSWMODE", l.get(5).getStandardName());
    }
