...
public class LocoNetSlotTest {
    ...
    public void testConsistingStateVsSpeedAccept() throws LocoNetException {
        ...
        lm = new LocoNetMessage(ib);
        t.setSlot(lm);
        Assert.assertEquals("OPC_LOCO_SPD ignored when consist-sub",18, t.speed());
        id[2] = 0x00;
        lm = new LocoNetMessage(id);
        t.setSlot(lm);
        Assert.assertEquals("Functions F0, F4-F1 set but not direction for consist-sub slot",0x20, t.dirf());
        id[2] = 0x3F;
        lm = new LocoNetMessage(id);
        t.setSlot(lm);
