...
public class LnTurnoutTest extends jmri.implementation.AbstractTurnoutTestBase {
    ...
    public void checkClosedMsgSent() throws InterruptedException {
        ...
        // CLOSED/ON loconet message
        Assert.assertEquals("B0 14 30 00", lnis.outbound.elementAt(lnis.outbound.size() - 2).toString());
        // CLOSED/OFF loconet message
        Assert.assertEquals("B0 14 20 00", lnis.outbound.elementAt(lnis.outbound.size() - 1).toString());
        lnis.outbound.clear();
    }
    ...
    public void checkThrownMsgSent() throws InterruptedException {
        ...
        Assert.assertEquals("B0 14 10 00", lnis.outbound.elementAt(lnis.outbound.size() - 2).toString());
        // THROWN/OFF loconet message
        Assert.assertEquals("B0 14 00 00", lnis.outbound.elementAt(lnis.outbound.size() - 1).toString());
        lnis.outbound.clear();
    }
    ...
    public void testLnTurnoutStatusMsg() {
        ...
        Assert.assertTrue(lnis.outbound.elementAt(0).toString().equals("B0 14 30 00")); // CLOSED loconet message
        Assert.assertTrue(t.getCommandedState() == CLOSED);
    ...
    public void testLnTurnoutStatusMsgAck() {
        ...
        Assert.assertTrue(lnis.outbound.elementAt(0).toString().equals("BD 14 10 00")); // thrown loconet message
        Assert.assertTrue(t.getCommandedState() == THROWN);
    ...
    public void testLnTurnoutExactFeedback() {
        ...
        // because this is the first time, the state is UNKNOWN; never goes back to that
        // (in this test)
        ...
        // notify the Ln of first feedback (out of order) - SWITCH is thrown, so moved
        // off - ignored
        m = new LocoNetMessage(4);
        m.setOpCode(0xb1);
        m.setElement(1, 0x14);
        ...
        // notify the Ln of second feedback (out of order) - SWITCH is closed, so moved
        // on
        m = new LocoNetMessage(4);
        m.setOpCode(0xb1);
        m.setElement(1, 0x14);
    ...
    // test that only two messages are sent when property SendOnAndOff is true, even
    // if (ulenbook) binary set.
    @Test
    ...
    public void testTurnoutLocks() {
        ...
        Assert.assertFalse("check turnoutPushbuttonLockout(false) is false 3",
                lnt.getLocked(CABLOCKOUT + PUSHBUTTONLOCKOUT));
        ...
        Assert.assertFalse("check turnoutPushbuttonLockout(false) is false 6",
                lnt.getLocked(CABLOCKOUT + PUSHBUTTONLOCKOUT));
        ...
        Assert.assertFalse("check turnoutPushbuttonLockout(false) is false 9",
                lnt.getLocked(CABLOCKOUT + PUSHBUTTONLOCKOUT));
    }
    ...
    public void testMyAddress() {
        ...
        for (int i = 0; i < 2048; ++i) {
            if (i != 20) {
                m.setElement(1, i & 0x7f);
                m.setElement(2, ((i >> 7) & 0xf) + 0x30);
                lnt.messageFromManager(m);
                Assert.assertEquals("check turnout 21 known state after message turnout " + i + " unknown.", UNKNOWN,
                        lnt.getKnownState());
                Assert.assertEquals("check turnout 21 commanded state after message turnout " + i + " unknown.",
                        UNKNOWN, lnt.getCommandedState());
            }
        }
        Assert.assertEquals("get final turnout 21 known state after bunch of opc_sw_req messages", UNKNOWN,
                lnt.getKnownState());
        Assert.assertEquals("get final turnout 21 commanded state after bunch of opc_sw_req messages", UNKNOWN,
                lnt.getCommandedState());

        m.setElement(1, 20 & 0x7f);
        m.setElement(2, ((20 >> 7) & 0xf) + 0x30);
        lnt.messageFromManager(m);
        Assert.assertEquals("check turnout 21 known state closed after opc_sw_req message turnout 20 closed.", CLOSED,
                lnt.getKnownState());
        Assert.assertEquals("check turnout 21 commanded state closed after opc_sw_req message turnout 20 closed.",
                CLOSED, lnt.getCommandedState());
        ...
        for (int i = 0; i < 2048; ++i) {
            if (i != 20) {
                m.setElement(1, i & 0x7f);
                m.setElement(2, ((i >> 7) & 0xf) + 0x10);
                lnt.messageFromManager(m);
                Assert.assertEquals(
                        "check turnout 21 known state after opc_sw_rep (output rep) message turnout " + i + " thrown.",
                        CLOSED, lnt.getKnownState());
                Assert.assertEquals("check turnout 21 commanded state after opc_sw_rep (output rep) message turnout " +
                        i +
                        " thrown.", CLOSED, lnt.getCommandedState());
            }
        }
        Assert.assertEquals("get turnout 21 known state after bunch of opc_sw_rep (output rep) messages", CLOSED,
                lnt.getKnownState());
        Assert.assertEquals("get turnout 21 commanded state after bunch of opc_sw_rep (output rep) messages", CLOSED,
                lnt.getCommandedState());

        m.setElement(1, 20 & 0x7f);
        m.setElement(2, ((20 >> 7) & 0xf) + 0x10);
        lnt.messageFromManager(m);
        Assert.assertEquals("check turnout 21 known state closed after opc_sw_req message turnout 21 thrown.", THROWN,
                lnt.getKnownState());
        Assert.assertEquals("check turnout 21 commanded state closed after opc_sw_req message turnout 21 thrown.",
                THROWN, lnt.getCommandedState());

        m.setElement(1, 20 & 0x7f);
        m.setElement(2, ((20 >> 7) & 0xf) + 0x30);
        lnt.messageFromManager(m);
        Assert.assertEquals("check turnout 21 known state closed after opc_sw_req message turnout 21 closed+thrown.",
                THROWN + CLOSED, lnt.getKnownState());
        Assert.assertEquals(
    ...
    public void testSetStateClosedAndThrown() {
        ...
        Assert.assertEquals("checking commanded state is Unknown after trying to send THROWN AND CLOSED", UNKNOWN,
                t.getKnownState());
        Assert.assertEquals("checking known state is Unknown after trying to send THROWN AND CLOSED", UNKNOWN,
                t.getKnownState());
    ...
    public void testWarningSendingOffWhenUsingOffAsConfirmation() {
        ...
        jmri.util.JUnitAppender.assertWarnMessage(
                "Turnout 21 is using OPC_SWREQ off as confirmation, but is sending OFF commands itself anyway");
