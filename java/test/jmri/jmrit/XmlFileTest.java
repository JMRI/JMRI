            ...
            for (Type theDTD : Type.values()) {
                for (Type theSchema : Type.values()) {
                    ...
                    if (theSchema == Type.INVALID && checkSchema) {
                        passes = false; // Cannot find the declaration of element 'decoderIndex-config'.
                    }
                    if (theSchema == Type.ABSENT && checkSchema) {
                        passes = false; // Cannot find the declaration of element 'decoderIndex-config'.
                    }
                    if (theDTD == Type.INVALID && checkDTD) {
                        passes = false; // Document root element "decoderIndex-config", must match DOCTYPE root "layout-config".
                    }
