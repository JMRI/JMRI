...
public abstract class CrrSection extends jmri.jmrit.automat.AbstractAutomaton {
    ...
    /**
     * Obtain the output object, sets the output to an initial state to make
     * sure everything is consistent at the start.
     */
    protected void init() {
    ...
    protected boolean handle() {
        ...
        if ((oldAppearance != newAppearance) && log.isDebugEnabled()) {
            switch (newAppearance) {
                case DARK:
                    log.debug(sig.getUserName() + "set DARK");
                    break;
                case RED:
                    log.debug(sig.getUserName() + "set RED");
                    break;
                case YELLOW:
                    log.debug(sig.getUserName() + "set YELLOW");
                    break;
                case GREEN:
                    log.debug(sig.getUserName() + "set GREEN");
                    break;
                default:
                    log.warn(sig.getUserName() + "set to unknown new appearance: " + newAppearance);
                    break;
            }
        }
    ...
    CrrSection() {
        ...
        if (tu == null) {
            ...
            // check for error!
            if (tu.length != 26 + 1) {
                log.error("Unexpected tu[] length: " + tu.length);
            }
            for (int i = 1; i < tu.length; i++) {
                if (tu[i] == null) {
                    log.error("tu[" + i + "] unexpectedly null");
                }
            }
        }
        ...
        if (bo == null) {
            ...
            // check for error!
            if (bo.length != 34 + 1) {
                log.error("Unexpected bo[] length: " + bo.length);
            }
            for (int i = 1; i < bo.length; i++) {
                if (bo[i] == null) {
                    log.error("bo[" + i + "] unexpectedly null");
                }
            }
            ...
            if (gate == null) {
                log.error("Failed to initialize gate");
            }
        }
        ...
        if (si == null) {
            ...
            if (n != 132 + 1) {
                log.error("unexpected number of found si[] entries: " + n);
            }
            if (si[0] != null) {
                log.error("unexpected non-null si[0]");
            }
            if (si[132] == null) {
                log.error("unexpected null in si[132]");
            }
        }
    }
    ...
    /**
     * Java array of Sensor objects corresponding to TU() turnout sensors in the
     * C/MRI code. Note that TU[1] is like TU(1) in BASIC, so there's an extra
     * null entry at the beginning (as TU[0]).
     * <P>
    ...
    /**
     * Java array of Sensor objects corresponding to BO() occupancy sensors in
     * the C/MRI code. Note that BO[1] is like BO(1) in BASIC, so there's an
     * extra null entry at the beginning (as BO[0]).
     * <P>
     * Initialization of the contents happens when the first CrrSection object
     * constructor is run.
     */
    ...
    /**
     * Java array of Turnout objects corresponding to SI() signal outputs in the
     * C/MRI code. Note that SI[1] is like SI(1) in BASIC, so there's an extra
     * null entry at the beginning (as SI[0]).
     * <P>
     * Initialization of the contents happens when the first CrrSection object
     * constructor is run.
     */
