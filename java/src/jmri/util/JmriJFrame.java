 ...
 * Features:
 * <ul>
 * <LI>Size limited to the maximum available on the screen, after removing any menu bars (Mac) and taskbars (Windows)
 * <LI>Cleanup upon closing the frame: When the frame is closed (WindowClosing event), the dispose() method is invoked
 * to do cleanup. This is inherited from JFrame itself, so super.dispose() needs to be invoked in the over-loading
 * methods.
 * <LI>Maintains a list of existing JmriJFrames
 * </ul>
 * 
 * <h3>Window Closing</h3>
 * Normally, a JMRI window wants to be disposed when it closes. This is what's needed when each invocation of the
 * corresponding action can create a new copy of the window. To do this, you don't have to do anything in your subclass.
 * This class has
 * <p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * If you want this behavior, but need to do something when the window is closing, override the
 * {@link #windowClosing(java.awt.event.WindowEvent)} method to do what you want. Also, if you override dispose(), make
 * sure to call super.dispose().
 * <p>
 * If you want the window to just do nothing or just hide, rather than be disposed, when closed, set the
 * DefaultCloseOperation to DO_NOTHING_ON_CLOSE or HIDE_ON_CLOSE depending on what you're looking for.
 * 
...
public class JmriJFrame extends JFrame implements java.awt.event.WindowListener, jmri.ModifiedFlag,
	 ...
	 * 
	 * @param saveSize
	 *            - Set true to save the last known size
	 * @param savePosition
	...
	/**
	 * Creates a JFrame with with given name plus standard settings, including optional save/restore of size and
	 * position.
	 * 
	 * @param name
	 *            - Title of the JFrame
	 * @param saveSize
	 *            - Set true to save the last knowm size
	 * @param savePosition
	 *            - Set true to save the last known location
	 */
	...
	/**
	 * Tries to get window to fix entirely on screen. First choice is to move the origin up and left as needed, then to
	 ...
	 * 
	 * @param ref
	 *            JHelp reference for the desired window-specific help page
	 * @param direct
	 *            true if the help menu goes directly to the help system, e.g. there are no items in the help menu
	 */
	 ...
	 * <p>
	 * Binds an AbstractAction to the Escape key. If an action is already bound to the Escape key, that action will be
	 * replaced. Passing <code>null</code> unbinds any existing actions from the Escape key.
	 * <p>
	 * Note that binding the Escape key to any action may break expected or standardized behaviors. See <a
	 * href="http://java.sun.com/products/jlf/ed2/book/Appendix.A.html">Keyboard Shortcuts, Mnemonics, and Other
	 * Keyboard Operations</a> in the Java Look and Feel Design Guidelines for standardized behaviors.
	 * 
	 * @param action
	 *            The AbstractAction to bind to.
	 ...
	 * 
	 * @return An AbstractAction or null if no action is bound to the Escape key.
	 ...
	 * <p>
	 * If closesWindow is true, this method creates an action that triggers the "window is closing" event; otherwise
	 * this method removes any actions from the Escape key.
	 * 
	 * @param closesWindow
	 *            Create or destroy an action to close the window.
	 ...
	 * 
	 * @return <code>true</code> if Escape key is bound to action created by setEscapeKeyClosesWindow,
	 ...
	 * <P>
	 * Some of the methods used here return null pointers on some Java implementations, however, so this will return the
	 * superclasses's maximum size if the algorithm used here fails.
	 */
	...
	/**
	 * The preferred size must fit on the physical screen, so calculate the lesser of either the preferred size from the
	...
	/**
	 * Get a List of the currently-existing JmriJFrame objects. The returned list is a copy made at the time of the
	 * call, so it can be manipulated as needed by the caller.
	 */
	 ...
	 * 
	 * @param subClass
	 *            The Class the list should be limited to.
