...
public class Profile implements Comparable<Profile> {
    ...
    public Profile(@Nonnull String name, @Nonnull String id, @Nonnull File path) throws IOException, IllegalArgumentException {
        ...
        if (Profile.inProfile(path) || Profile.inProfile(pathWithExt)) {
            if (Profile.inProfile(path)) log.warn("Exception: Path {} is within an existing profile.", path); // NOI18N
            if (Profile.inProfile(pathWithExt)) log.warn("Exception: pathWithExt {} is within an existing profile.", pathWithExt); // NOI18N
