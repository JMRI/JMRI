...
public interface SignalGroup extends NamedBean {
     ...
     */
    @CheckForNull
    String getSignalMastAspectByIndex(int x);
     ...
     */
    @CheckForNull
    String getHeadItemNameByIndex(int x);
    ...
    int ONACTIVE = 0;    // group head conditional fires if sensor goes active
    int ONINACTIVE = 1;  // group head conditional fires if sensor goes inactive

    int ONCLOSED = 2;    // group head conditional fires if turnout goes closed
    int ONTHROWN = 4;  // group head conditional fires if turnout goes thrown
}
