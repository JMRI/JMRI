...
public class NceConsistEditPanel extends jmri.jmrix.nce.swing.NcePanel implements
    ...
    private void toggleAdrButton(JTextField locoTextField, JButton adrButton) {
        ...
        if (locoTextField.getText().isEmpty()) {
    ...
    private boolean updateRoster(String consistNumber) {
        ...
        if (locoTextField2.getText().isEmpty()) {
            return false;
        }
        NceConsistRosterEntry nceConsistRosterEntry;
    ...
    private boolean canLoad() {
        if (locoTextField1.getText().isEmpty()) {
            return false;
        }
    ...
    private void shiftOneLine(JTextField locoTextFieldLow, JButton adrButtonLow,
        ...
        if (locoTextFieldLow.getText().isEmpty() && !locoTextFieldHigh.getText().equals((""))) {
    ...
    private void changeButtons(boolean rosterDisplay) {
        if (rosterDisplay) {
            ...
            saveLoadButton.setText(Bundle.getMessage("KeySAVE"));
            saveLoadButton.setToolTipText(Bundle.getMessage("ToolTipSave"));
            clearCancelButton.setEnabled(!locoTextField1.getText().isEmpty());
        }
    ...
    private void checkForRosterMatch() {
        ...
        if (nceConsistRosterEntry == null) {
            if (checkBoxConsist.isSelected() && !locoTextField1.getText().isEmpty()) {
                consistStatus.setText(Bundle.getMessage("EditStateUNKNOWN"));
            } else {
                textConRoadName.setText("");
            }
    ...
    private boolean updateLocoFields(NceReply r, int index,
        ...
        if (locoAddrText.isEmpty() || locoAddrText.equals(Bundle.getMessage("REPLACE_LOCO"))) {
    ...
    private boolean verifyAllLocoAddr() {
        ...
        if (checkBoxVerify.isSelected()) {
            int i = 0;
            if (!locoTextField1.getText().isEmpty() && validLocoAdr(locoTextField1.getText()) > 0) {
                locoVerifyList[i++] = getLocoAddr(locoTextField1, adrButton1);
            }
            if (!locoTextField2.getText().isEmpty() && validLocoAdr(locoTextField2.getText()) > 0) {
                locoVerifyList[i++] = getLocoAddr(locoTextField2, adrButton2);
            }
            if (!locoTextField3.getText().isEmpty() && validLocoAdr(locoTextField3.getText()) > 0) {
                locoVerifyList[i++] = getLocoAddr(locoTextField3, adrButton3);
            }
            if (!locoTextField4.getText().isEmpty() && validLocoAdr(locoTextField4.getText()) > 0) {
                locoVerifyList[i++] = getLocoAddr(locoTextField4, adrButton4);
            }
            if (!locoTextField5.getText().isEmpty() && validLocoAdr(locoTextField5.getText()) > 0) {
                locoVerifyList[i++] = getLocoAddr(locoTextField5, adrButton5);
            }
            if (!locoTextField6.getText().isEmpty() && validLocoAdr(locoTextField6.getText()) > 0) {
    ...
    private void initLocoFields() {
        initLocoRow(1, Bundle.getMessage("LeadLabel"), textLoco1, locoTextField1, locoRosterBox1,
                adrButton1, dirButton1, cmdButton1);
        initLocoRow(2, Bundle.getMessage("RearLabel"), textLoco2, locoTextField2, locoRosterBox2,
                adrButton2, dirButton2, cmdButton2);
        initLocoRow(3, Bundle.getMessage("MidLabel", "1"), textLoco3, locoTextField3, locoRosterBox3,
                adrButton3, dirButton3, cmdButton3);
        initLocoRow(4, Bundle.getMessage("MidLabel", "2"), textLoco4, locoTextField4, locoRosterBox4,
                adrButton4, dirButton4, cmdButton4);
        initLocoRow(5, Bundle.getMessage("MidLabel", "3"), textLoco5, locoTextField5, locoRosterBox5,
                adrButton5, dirButton5, cmdButton5);
        initLocoRow(6, Bundle.getMessage("MidLabel", "4"), textLoco6, locoTextField6, locoRosterBox6,
                adrButton6, dirButton6, cmdButton6);
    }
    ...
    private void initLocoRow(int row, String s, JLabel textLoco,
        ...
        adrButton.setEnabled(false);
        adrButton.setToolTipText(Bundle.getMessage("ToolTipAddressType"));
        ...
        locoRosterBox.setEnabled(false);
        locoRosterBox.setToolTipText(Bundle.getMessage("ToolTipSelectLoco"));
        ...
        dirButton.setEnabled(false);
        dirButton.setToolTipText(Bundle.getMessage("ToolTipDirection"));
        ...
        cmdButton.setEnabled(false);
        cmdButton.setToolTipText(Bundle.getMessage("ToolTipAdd"));
        cmdButton.addActionListener(this::buttonActionCmdPerformed);

        locoTextField.setText("");
        locoTextField.setEnabled(false);
        locoTextField.setToolTipText(Bundle.getMessage("ToolTipEnterLoco"));
    ...
    private void initConsistRoster(JComboBox<String> conRosterBox) {
        ...
        conRosterBox.setEnabled(false);
        conRosterBox.setToolTipText(Bundle.getMessage("ToolTipSelectConsist"));
        conRosterBox.addActionListener(consistRosterListener = this::consistRosterSelected);
    }
    ...
    private void queueError(int errorCode) {
        ...
        errorThread.setName("Report Error"); // NOI18N
        errorThread.start();
    }
