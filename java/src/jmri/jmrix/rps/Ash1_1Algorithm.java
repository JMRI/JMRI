...
<P> 
When the receivers are in a plane or nearly so there is a
spurious solution on the other side of the plane from the correct solution
and in certain situations the version 1.0 program may choose the wrong
solution.  
<P>
It turns out that those situations are when the receiver configuration is
not sufficiently non-planar for the size of the measurement errors.  The
greater the errors, the greater the non-planarity must be to avoid he bug.  
<P>
...
near, and using the closest ones.  There is a maximum that are used, still
set at 15.  
<P>
...
toward or away from it such that the distance is equal to the measured
value.  After going through the receivers once in order, it then chooses
them at random until it has iterated some fixed number of times.  This is
set at 1000 although the procedure usually converges in 20-50 iterations;
for occasional positions the convergence is much slower.  The random order
is used because the procedure was occasionally observed to get stuck in a
loop when using a repetitive fixed order.  Rather than start with the origin
as the initial position, it now starts from a position far, far below.  This
removes the restriction that the origin must be below the receivers.  
<P>
...
position suddenly jumps to somewhere else, or perhaps it jumps back and
forth depending on measurement errors.  An extreme example is 8 receivers at
the corners of a cube, which is about as non-planar as it gets.  In this
case the transmitter can go outside the cube by several times the width of
the cube, both laterally and vertically, before the program gets into
trouble.  
</OL>
<P>
...
and/or ceiling.  Other configurations are Larry Wade's rather-small oval
test track with 4 receivers and a 14-foot square and an 8-foot cube
(mentioned above).   Large, 100-receiver configurations include a 25 x 10 x
5-foot space with receivers randomly located throughout (rather unrealistic)
and a 100 x 10 x 5-foot space with receivers arranged in 4 rows of 25, one
row on each long wall and two rows on the ceiling.  Performance (i.e.
accuracy of the measured transmitter position) is excellent throughout this
latter space.  
<P>
...
these domes, but surprisingly it is no better with 46 receivers than with
16, near the perimeter a bit worse.  Presumably this is because of the
limited number of closest receivers used by the position program.  In order
to do justice to this, or any other configuration with closely-spaced
receivers, the program needs to use data from more than the 15 receivers
currently used.  
<P>
...
/**
 * The following is the original algorithm, as 
 * provided by Ash as a C routine
 */
...
RetVal RPSpos(int nr, double Tr[], double Xr[], double Yr[], double Zr[],// many
  ...
  ns = 0;  Rs[NMAX-1] = TMAX; Rmax = Vs * TMAX	;//  Sort receivers by delay
  for (i = 0; i < nr; i++)			{
    if (Tr[i] == 0.0)		      continue	;//   Discard failures
    Rq = Vs * (Tr[i] + OFFSET)			;//   Compute range from delay
    if ((Rq >= Rmax)||(Rq < Vs * TMIN)) continue;//  Discard too long or short
    if (ns == 0)				{
      Rs[0] = Rq;  Xs[0] = Xr[i];  Ys[0] = Yr[i];  Zs[0] = Zr[i];
      ns = 1					;}//  1st entry
    else					{
      j = ((ns == NMAX) ? (ns-1) : (ns++))	;//   Keep no more than NMAX
      for (;; j--)				{//   Bubble sort
        if ((j > 0)&&(Rq < Rs[j-1]))		{
          Rs[j] = Rs[j-1];  Xs[j] = Xs[j-1]	;//    Move old entries
          Ys[j] = Ys[j-1];  Zs[j] = Zs[j-1]	;}
        else					{
  ...
  if (ns < 3)					{//   Failed:
    Xt = Yt = Zt = 9.9999999e99;  return new RetVal(1,Xt,Yt,Zt,Vs);}//   Too few usable receivers


  x = y = 0.0;   z = -100000.0			;//  Initial solution
...
//if (i%10 == 1) printf("\n%4d   %8.3lf %8.3lf %8.3lf",i,x,y,z);
    ...
    y = w*(y - Ys[j]) + Ys[j]			;
    z = w*(z - Zs[j]) + Zs[j]			;}// 1000 random receivers

  Ww = Xw = Yw = Zw = 0.0			;//  Final solution
  for (i = 0; i < ns-2; i++)			{//   Weighted average
    xi = Xs[i];  yi = Ys[i];  zi = Zs[i];  ri = Rs[i];//            of all sets
    for (j = i+1; j < ns-1; j++)		{
      xj = Xs[j];  yj = Ys[j];  zj = Zs[j];  rj = Rs[j];
      for (k = j+1; k < ns; k++)		{
        xk = Xs[k];  yk = Ys[k];  zk = Zs[k];  rk = Rs[k];
        if (gps3() == 0)				{//    Solve for each set
          if ((w = wgt()) > 0.0)		{//     Add to averages
            Ww += w;  Xw += w*x0;  Yw += w*y0;  Zw += w*z0;}}}}}

  if (Ww > 0.0)					{
    Xt = Xw/Ww;  Yt = Yw/Ww;	Zt = Zw/Ww	;//   Computed position
    return new RetVal(0,Xt,Yt,Zt,Vs);}//   Success
  else						{
    Xt = Yt = Zt = 9.9999999e99; return new RetVal(2,Xt,Yt,Zt,Vs);}//   Failed:  No solution
						}//  End of RPSpos()
...
int gps3()						{// GPS Position Solver
  ...
  xik = xi - xk;  yik = yi - yk;  zik = zi - zk	;//  Solve with absolute ranges
  ...
  if (e1 <= e2)					{//       (also global inputs)
    x0 = x1;  y0 = y1;  z0 = z1			;}//Solution (global variables)
  else						{
...
// ******************
    ...
    static Logger log = LoggerFactory.getLogger(Ash1_1Algorithm.class.getName());
