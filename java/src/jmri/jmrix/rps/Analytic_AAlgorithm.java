 ...
 * from Sam Storm van Leeuwen <samsvl@nlr.nl>, ported to
 * Java by Norris Weimer <norris.weimer@ualberta.ca>, and
...
public class Analytic_AAlgorithm extends AbstractCalculator {
    ...
    public Measurement convert(Reading r) {
        ...
        for (int i =0; i<33; i++) SV[i] = false;
        
        double[] P = new double[33];
        double[] Xr = new double[3];Xr[0]=Xr[1]=Xr[2]=0;
...
/** *************************************************************************
  *
  * @param	Xs		array with 3 columns and 32 rows, for the coordinates of the sat's
  * @param	SV		valid prn's
  * @param	P		pseudoranges
  *
  *					(note: arrays actually have 33 rows, but row 0 is unused, 
  *                        in order to index by actual prn number)
  *
