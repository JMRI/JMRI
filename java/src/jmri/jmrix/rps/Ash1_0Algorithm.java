...
RetVal RPSpos(int nr, double Tr[], double Xr[], double Yr[], double Zr[],// many
  ...
  for (i = 0; i < nr; i++)			{
    if (Tr[i] == 0)		    continue	;//   Discard failures
    Rq = Vs * (Tr[i] + OFFSET)			;//   Compute range from delay
    if (Rq >= Rmax)		    continue	;//    Discard if too long
    if (ns == 0)				{
  ...
  for (i = 0; i < ns; i++)			{//     to reject spurious sol.
    j = (i+1)%ns;	k = (i+2)%ns		;//   For ns sets
    xi = Xs[i];  yi = Ys[i];  zi = Zs[i];  ri = Rs[i];
    xj = Xs[j];  yj = Ys[j];  zj = Zs[j];  rj = Rs[j];
    xk = Xs[k];  yk = Ys[k];  zk = Zs[k];  rk = Rs[k];
    if (gps3() == 0)				{//    Solve for one set
      x = x1;    y = y1;    z = z1		;//     Reject if no solution
      if (wgt() > 0.0)				{
        x = x2;    y = y2;    z = z2		;//         or no weight
        if (wgt() > 0.0)			{
  ...
  if ((da == 0.0)&&(db == 0.0))			{//   No solution found
    ...
    if ((d11 < d12)&&(d11 < d21)&&(d11 < d22))	{//   Choose solution
      x = (x1a+x1b)/2;  y = (y1a+y1b)/2;  z = (z1a+z1b)/2;}
    else if ((d12 < d21)&&(d12 < d22))		{//          which is
      x = (x1a+x2b)/2;  y = (y1a+y2b)/2;  z = (z1a+z2b)/2;}
    else if (d21 < d22)				{//          closest between
      x = (x2a+x1b)/2;  y = (y2a+y1b)/2;  z = (z2a+z1b)/2;}
    else					{//          max. and min. sets
      x = (x2a+x2b)/2;  y = (y2a+y2b)/2;  z = (z2a+z2b)/2;}}
  ...
  for (i = 0; i < ns-2; i++)			{//   Weighted average
    ...
    for (j = i+1; j < ns-1; j++)		{
      ...
      for (k = j+1; k < ns; k++)		{
        ...
        if (gps3() == 0)				{//    Solve for one set
          if (Ww == 0.0)			{
            x = x0;  y = y0;  z = z0		;}//       Initial position
          if ((w = wgt()) > 0.0)		{
            Ww += w;  Xw += w*x0;  Yw += w*y0;  Zw += w*z0;// Add to averages
            x = Xw/Ww;  y = Yw/Ww;  z = Zw/Ww	;}}}}}//   Latest position
  ...
  if (Ww > 0.0)					{
    ...
    return new RetVal(2,Xt,Yt,Zt,Vs);}//   Failed:  No solution
						}// End of RPSpos()
...
double    Ca, Cb, Cc, Cd, Ce, Cf, Ci, Cj, Cx, Cy, Cz;//  xi, yi, zi, ri
  ...
  return 0					;
    ...
    static Logger log = LoggerFactory.getLogger(Ash1_0Algorithm.class.getName());
