...
public final class OlcbEventNameStoreXml extends XmlFile {
     ...
     *<p>
     * Note when this actually migrates, it opens and loads the IdTag Manager
     * which can result in name confusion when Reporters
     * are later created by a panel file.  If this has already happened,
     * the internal reporters generated by that have to be removed
     * before this migration takes place. This means that there's
     * no real good place to run this as part of  JMRI startup.
     * Instead, it's left as a public method so it can (when needed)
     * be invoked by e.g. a script after the name clean up has been done.
     */
    public void migrateFromIdTagStore() {
        ...
        log.info("Checking for IdTag-stored Event Names to migrate");
        
        // get the IdTags, find any that are Event Names, and migrate those
   ...
   private void writeFile(String fileName) throws FileNotFoundException, java.io.IOException {
    ...
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(OlcbEventNameStoreXml.class);
