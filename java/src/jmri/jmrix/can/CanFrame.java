...
import static jmri.util.StringUtil.twoHexFromInt;

/**
 * Base interface for immutable messages in a CANbus based message/reply protocol.
 * <p>
 * It is expected that any CAN based system will be based upon basic CAN concepts such as ID/header (standard or
...
public interface CanFrame {
    ...
    /**
     * Get formatted monitor String. Includes if Frame is Extended. Header value at start. All values hex format. Only
     * valid data elements are included.
     *
    ...
    /**
     * Get formatted toString. Does NOT include if Frame is Extended. All values hex format. Only valid data elements
     * are included.
     *
    ...
    default void appendHexElements(StringBuilder sb) {
        for (int i = 0; i < getNumDataElements(); i++) {
            sb.append(" ");
            sb.append(twoHexFromInt(getElement(i)));
        }
    }
    ...
    default boolean isEqual(Object a, Object b) {
        if (a instanceof CanFrame && b instanceof CanFrame) {
            ...
            if (aa.isRtr() == bb.isRtr() && aa.isExtended() == bb.isExtended() && aa.getHeader() == bb.getHeader() && dataFramesEqual(aa, bb)) {
                return true;
            }
        }
        return false;
