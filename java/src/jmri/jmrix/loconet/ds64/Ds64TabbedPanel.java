 ...
 * configuration.
 * <P>
 * The read and write operations require a sequence of operations, which
 * are handled with a state variable.
 * <P>
 * Programming of the DS64 is done via LocoNet configuration messages, so
 * the DS64 should not be manually put into its programming mode via the
 * DS64 built-in pushbutton while this tool is in use.
 * <P>
 ...
 * <P>
 * Extensions to include read/write of turnout output addresses and
 * routes are based on reverse-engineering of DS64 operating characteristics by
 * B. Milhaupt.  As such, this tool may not be compatible with all DS64 devices.
 * <hr>
...
public class Ds64TabbedPanel extends AbstractBoardProgPanel {
     ...
     * Allows the instantiating method to specify whether the basic 
     * feature configuration information will be read upon instantiation.
     * 
     ...
     * <P>
     * At instantiation, the object will automatically assume BoardID 1 and 
     * will not pre-read the basic board configuration information.
     * 
     */
     ...
     * Ds64TabbedPanel constructor when the boardNum is already known.  
     * <P>
     * When instantiated, the object will not automatically read the 
     ...
     * instantiation.
     * <P>
     * When instantiated, the object will automatically read the 
     * basic configuration information if readOnInit is true.
     * 
     * @param readOnInit true to automatically read the basic configuration info
    ...
    public Ds64TabbedPanel(boolean readOnInit, Integer[] boardNums ) {
        ...
        if (debugIsEnabled) log.debug("into DS64 tabbed panel with list of boards of length " + boardNums.length); // NOI18N
        if (debugIsEnabled) log.debug("boardNums[0] = " + boardNums[0]); // NOI18N
        ...
        for (int boardNum: boardNums) {
            if (debugIsEnabled) log.debug("board "+boardNum); // NOI18N
            boardNumsEntryValue.add(boardNum);
        }
     ...
     * feature configuration information will be read upon instantiation.
     * 
     * @param readBoardOnInit true to automatically read the basic configuration info.
     */
...
// local input controls
    ...
    @Override public String getHelpTarget() { return "package.jmri.jmrix.loconet.ds64.DS64Frame"; }

    @Override public String getTitle() {
        return getTitle(DS64Bundle.getMessage("MenuItemDS64PanelEnhanced"));
    }
    ...
    protected void updateDisplay() {
        ...
        routesControl.setSelectedIndex((opsw[16 ] ? 0 : ((opsw[11] ?2 : 1))));

        localControlOfOutputsStyle.setSelectedIndex(opsw[15] ? 0 : (opsw[12] ? 2 : 1));
        log.info("setting sensorMessageTrigger based on opsw[13]="+opsw[13]);
        sensorMessageTrigger.setSelectedIndex(opsw[13] ? 1 : 0);   // selection 0 - only for A inputs; 1 - both A and S inputs
    ...
    protected int nextState(int state) {
        ...
        switch (state) {
            case  1: {
                if (colorizedObject == null) colorizedObject = outputType;
            ...
            case  2: {
                if (colorizedObject != null) colorizedObject.setBackground(null);
                colorizedObject = delayTime;
                colorizedObject.setBackground(Color.blue.brighter());
                return 3;
            }
            ...
            case 48: {
                ...
                if (isRead == true) {
                    ...
                    if ((opsw[47] == false) && (opsw[48] == false) ) {
                        ...
                        if (debugIsEnabled) log.debug("Read of low value in index "+indexToRead+" is a valid entry."); // NOI18N
                        return 33;
                    }
                    else {
                        if (debugIsEnabled) log.debug("Read of low value in index "+indexToRead+" is an invalid entry."); // NOI18N
            ...
            case 64: {
                if (isRead == true) {
                    ...
                    if ((opsw[63] == false) && (opsw[64] == false) ) {
                        ...
                        if (debugIsEnabled) log.debug("Read of high value in index "+indexToRead+" is a valid entry."); // NOI18N
                        ...
                        // Do need to update internal opSw bits so that they imply an unused
                        // entry
                        if (debugIsEnabled) log.debug("Read of high value in index "+indexToRead+" is an invalid entry."); // NOI18N
                        for (int i = 49; i< 62; ++i)
                            opsw[i] = true;
                        opsw[56] = false;
    ...
    private void changeGuiElementUnHighlight(Integer reportedState, Integer reportedIndexToRead) {
        ...
        switch (reportedState) {
            ...
            case 48:
                ...
                if (jc != null) changeComponentBgColor(jc,noAccessColor);
                return;
            case 61:
            case 64:
    ...
    private void changeGuiElementHighlight(Integer reportedState, Integer reportedIndexToRead) {
        ...
        if (reportedState == 48) {
            ...
            jc = whichComponent(49, reportedIndexToRead);
            if (jc != null) changeComponentBgColor(jc,accessColor);
        }
        if (reportedState == 64) {
            jc = whichComponent(49, reportedIndexToRead);
    ...
    private enum OpSwOpType {OutputsRead, OutputsWrite,
    ...
    protected void updateOpswForWrite(int index) {
        ...
        switch (index) {
            case 0: {
                try { value1Address = Integer.parseInt(outAddr1.getText()); }
                catch (Exception e) { value1Address = 2048; }

                try { value2Address = Integer.parseInt(outAddr2.getText()); }
                catch (Exception e) { value2Address = 2048; }

                updateOpSwsOutAddr(value1Address, false, false, value2Address, false, false);
                break;
            }
            case 1: {
                try { value1Address = Integer.parseInt(outAddr3.getText()); }
                catch (Exception e) { value1Address = 2048; }

                try { value2Address = Integer.parseInt(outAddr4.getText()); }
                catch (Exception e) { value2Address = 2048; }
            ...
            case 16: case 20: case 24: case 28:
                ...
                opsw[47] = false; opsw[48] = false; // assume valid turnout address entry
                opsw[63] = false; opsw[64] = false; // assume valid turnout address entry
                ...
                if (routeTop[extractedIndex].getIsUnused()) {
                    ...
                    try { value1Address = Integer.parseInt(routeTop[extractedIndex].addressField.getText()); }
                    catch (Exception e) { 
                        value1Address = 2048;
                        value1IsUnused = true;
                        value1DirectionIsClosed = true;
                ...
                if (routeA2[extractedIndex].getIsUnused()) {
                    ...
                    try { value2Address = Integer.parseInt(routeA2[extractedIndex].addressField.getText()); }
                    catch (Exception e) {
                        value2Address = 2048;
                        value2IsUnused = true;
                        value2DirectionIsClosed = true;
                ...
                if (value1IsUnused) {
                    opsw[46] = true; opsw[47] = true; opsw[48] = true; // mark entry as invalid
                ...
                if (value2IsUnused){
                    opsw[62] = true; opsw[63] = true; opsw[64]=true; // mark entry as invalid
            ...
            case 17: case 21: case 25: case 29:
                ...
                if (routeA3[extractedIndex].getIsUnused()) {
                    ...
                    value1IsUnused = false;
                    try { value1Address = Integer.parseInt(routeA3[extractedIndex].addressField.getText()); }
                    catch (Exception e) { 
                        value1Address = 2048;
                        value1IsUnused = true;
                    }
                ...
                if (routeA4[extractedIndex].getIsUnused()) {
                    ...
                    value2IsUnused = false;
                    try { value2Address = Integer.parseInt(routeA4[extractedIndex].addressField.getText()); }
                    catch (Exception e) {
                        value2Address = 2048;
                        value2IsUnused = true;
                    }
            ...
            case 18: case 22: case 26: case 30:
                ...
                if (routeA5[extractedIndex].getIsUnused()) {
                    ...
                    value1IsUnused = false;
                    try { value1Address = Integer.parseInt(routeA5[extractedIndex].addressField.getText()); }
                    catch (Exception e) { 
                        value1Address = 2048;
                        value1IsUnused = true;
                    }
                ...
                if (routeA6[extractedIndex].getIsUnused()) {
                    ...
                    value2IsUnused = false;
                    try { value2Address = Integer.parseInt(routeA6[extractedIndex].addressField.getText()); }
                    catch (Exception e) {
                        value2Address = 2048;
                        value2IsUnused = true;
                    }
            ...
            case 19: case 23: case 27: case 31:
                ...
                if (routeA7[extractedIndex].getIsUnused()) {
                    ...
                    value1IsUnused = false;
                    try { value1Address = Integer.parseInt(routeA7[extractedIndex].addressField.getText()); }
                    catch (Exception e) { 
                        value1Address = 2048;
                        value1IsUnused = true;
                    }
                ...
                if (routeA8[extractedIndex].getIsUnused()) {
                    ...
                    value2IsUnused = false;
                    try { value2Address = Integer.parseInt(routeA8[extractedIndex].addressField.getText()); }
                    catch (Exception e) {
                        value2Address = 2048;
                        value2IsUnused = true;
                    }
    ...
    private void unhighlightAllBasicOpSws() {
     ...
     @Override public void message(LocoNetMessage m) {
        super.message(m);

        if (m.getOpCode() == LnConstants.OPC_LONG_ACK ) {
            if ( (
                    (m.getElement(1) == LnConstants.RE_LACK_SPEC_CASE1 ) || 
        ...
        if (m.getOpCode() == LnConstants.OPC_SW_REQ) {
            ...
            boolean dir = ((m.getElement(2)&0x20) == 0x20) ? true:false;
            if (swAddr == Integer.parseInt(outAddr1.getText())) {
                outState1.setText(dir ? DS64Bundle.getMessage("LabelTurnoutCurrentlyIsClosed") : 
                        DS64Bundle.getMessage("LabelTurnoutCurrentlyIsThrown"));
                outState1.updateUI();
            }
            if (swAddr == Integer.parseInt(outAddr2.getText())) {
                outState2.setText(dir ? DS64Bundle.getMessage("LabelTurnoutCurrentlyIsClosed") : 
                        DS64Bundle.getMessage("LabelTurnoutCurrentlyIsThrown"));
                outState2.updateUI();
            }
            if (swAddr == Integer.parseInt(outAddr3.getText())) {
                outState3.setText(dir ? DS64Bundle.getMessage("LabelTurnoutCurrentlyIsClosed") : 
                        DS64Bundle.getMessage("LabelTurnoutCurrentlyIsThrown"));
                outState3.updateUI();
            }
            if (swAddr == Integer.parseInt(outAddr4.getText())) {
                outState4.setText(dir ? DS64Bundle.getMessage("LabelTurnoutCurrentlyIsClosed") : 
                        DS64Bundle.getMessage("LabelTurnoutCurrentlyIsThrown"));
                outState4.updateUI();
            }
        }
...
@SuppressWarnings(value="UPM_UNCALLED_PRIVATE_METHOD",justification="Functionality not yet confirmed with hardware; may be useful at a future date.")
    private void boardFactoryReset() {
        ...
        Object[] dialogBoxButtonOptions = {
            DS64Bundle.getMessage("ButtonTextResetToFactoryDefault"), 
            DS64Bundle.getMessage("ButtonTextCancelBoardReset")};
        int userReply = JOptionPane.showOptionDialog(this.getParent(),
                DS64Bundle.getMessage("DialogTextBoardResetWarning"),
                DS64Bundle.getMessage("DialogHeadingBoardResetWarning"), 
                JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE,
                null, dialogBoxButtonOptions, dialogBoxButtonOptions[1]);
        if (userReply != 0) return; // compare only to exactly the value for executing the reset!
    ...
    @Override public void initComponents() {
        ...
        addrField.setText(DS64Bundle.getMessage("LABEL_UNIT_ADDRESS"));

        String[] outputTypes = {DS64Bundle.getMessage("ComboBoxOutputType0"),
            DS64Bundle.getMessage("ComboBoxOutputType1")};
        outputTypeLabel = new JLabel(DS64Bundle.getMessage("LabelOutputType"));
        outputType     = new JComboBox<String>(outputTypes); // opSw 1
        outputType.setToolTipText(DS64Bundle.getMessage("ToolTipOutputType"));
        outputType.addActionListener(basicConfigChangeActionListener);

        String[] availableDelayTimes = new String[16];
        availableDelayTimes[0] = DS64Bundle.getMessage("ComboBoxPulseTime0point1");
        availableDelayTimes[1] = DS64Bundle.getMessage("ComboBoxPulseTime0point2");
        availableDelayTimes[2] = DS64Bundle.getMessage("ComboBoxPulseTime0point4");
        availableDelayTimes[3] = DS64Bundle.getMessage("ComboBoxPulseTime0point6");
        availableDelayTimes[4] = DS64Bundle.getMessage("ComboBoxPulseTime0point8");
        availableDelayTimes[5] = DS64Bundle.getMessage("ComboBoxPulseTime1point0");
        availableDelayTimes[6] = DS64Bundle.getMessage("ComboBoxPulseTime1point2");
        availableDelayTimes[7] = DS64Bundle.getMessage("ComboBoxPulseTime1point4");
        availableDelayTimes[8] = DS64Bundle.getMessage("ComboBoxPulseTime1point6");
        availableDelayTimes[9] = DS64Bundle.getMessage("ComboBoxPulseTime1point8");
        availableDelayTimes[10] = DS64Bundle.getMessage("ComboBoxPulseTime2point0");
        availableDelayTimes[11] = DS64Bundle.getMessage("ComboBoxPulseTime2point2");
        availableDelayTimes[12] = DS64Bundle.getMessage("ComboBoxPulseTime2point4");
        availableDelayTimes[13] = DS64Bundle.getMessage("ComboBoxPulseTime2point6");
        availableDelayTimes[14] = DS64Bundle.getMessage("ComboBoxPulseTime2point8");
        availableDelayTimes[15] = DS64Bundle.getMessage("ComboBoxPulseTime3point0");

        delayTimeLabel = new JLabel(DS64Bundle.getMessage("LabelPulseTimeout")); // opSws 2-5
        delayTime = new JComboBox<String>(availableDelayTimes);
        delayTime.setToolTipText(DS64Bundle.getMessage("ToolTipPulseTimeout"));
        delayTime.addActionListener(basicConfigChangeActionListener);

        String[] initialOutputStates = {DS64Bundle.getMessage("ComboBoxOutputPowerupType0"),
            DS64Bundle.getMessage("ComboBoxOutputPowerupType1")};
        outputStatesLabel = new JLabel(DS64Bundle.getMessage("LabelPowerUpOutputActivity"));
        outputStates     = new JComboBox<String>(initialOutputStates);    // opsw 6
        outputStates.setToolTipText(DS64Bundle.getMessage("ToolTipOutputStates"));
        outputStates.addActionListener(basicConfigChangeActionListener);

        String[] startupDelays = {DS64Bundle.getMessage("ComboBoxOutputPowerupDelayType0"),
            DS64Bundle.getMessage("ComboBoxOutputPowerupDelayType1")};
        startupDelayLabel = new JLabel(DS64Bundle.getMessage("LabelInitialPowerUpDelay"));
        startupDelay     = new JComboBox<String>(startupDelays);  // opsw 8
        startupDelay.setToolTipText(DS64Bundle.getMessage("ToolTipStartupDelay"));
        startupDelay.addActionListener(basicConfigChangeActionListener);

        String[] staticOutputShutoffs = {DS64Bundle.getMessage("ComboBoxOutputPowerManagementType0"),
        DS64Bundle.getMessage("ComboBoxOutputPowerManagementType1")};
        staticOutputShutoffLabel = new JLabel(DS64Bundle.getMessage("LabelOutputPowerManagementStyle"));
        staticOutputShutoff     = new JComboBox<String>(staticOutputShutoffs); // opSw 9
        staticOutputShutoff.setToolTipText(DS64Bundle.getMessage("ToolTipLabelOutputPowerManagementStyle"));
        staticOutputShutoff.addActionListener(basicConfigChangeActionListener);

        // command sources
        String[] commandTypes = {DS64Bundle.getMessage("ComboBoxCommandsRecognizedFromType0"),
            DS64Bundle.getMessage("ComboBoxCommandsRecognizedFromType1")};
        commandTypeLabel = new JLabel(DS64Bundle.getMessage("LabelAcceptedSwitchCommandTypes"));
        commandType    = new JComboBox<String>(commandTypes); //opSw 10
        commandType.setToolTipText(DS64Bundle.getMessage("ToolTipLabelAcceptedSwitchCommandTypes"));
        commandType.addActionListener(basicConfigChangeActionListener);

        String[] commandSources = {DS64Bundle.getMessage("ComboBoxCommandSourceType0"),
            DS64Bundle.getMessage("ComboBoxCommandSourceType1")};
        commandSourceLabel = new JLabel(DS64Bundle.getMessage("LabelAcceptSwitchCommandsFrom"));
        commandSource    = new JComboBox<String>(commandSources); // opSw14
        commandSource.setToolTipText(DS64Bundle.getMessage("ToolTipCommandSource"));
        commandSource.addActionListener(basicConfigChangeActionListener);

    // Crossbuck Flasher controls
        output1CrossbuckFlasherCheckBox    = new JCheckBox(DS64Bundle.getMessage("CheckBoxOutput1Crossbuck"));
        output1CrossbuckFlasherCheckBox.setToolTipText(DS64Bundle.getMessage("ToolTipCheckBoxOutput1Crossbuck"));
        output1CrossbuckFlasherCheckBox.addActionListener(basicConfigChangeActionListener);
        output2CrossbuckFlasherCheckBox    = new JCheckBox(DS64Bundle.getMessage("CheckBoxOutput2Crossbuck"));
        output2CrossbuckFlasherCheckBox.setToolTipText(DS64Bundle.getMessage("ToolTipCheckBoxOutput2Crossbuck"));
        output2CrossbuckFlasherCheckBox.addActionListener(basicConfigChangeActionListener);
        output3CrossbuckFlasherCheckBox    = new JCheckBox(DS64Bundle.getMessage("CheckBoxOutput3Crossbuck"));
        output3CrossbuckFlasherCheckBox.setToolTipText(DS64Bundle.getMessage("ToolTipCheckBoxOutput3Crossbuck"));
        output3CrossbuckFlasherCheckBox.addActionListener(basicConfigChangeActionListener);
        output4CrossbuckFlasherCheckBox    = new JCheckBox(DS64Bundle.getMessage("CheckBoxOutput4Crossbuck"));
        output4CrossbuckFlasherCheckBox.setToolTipText(DS64Bundle.getMessage("ToolTipCheckBoxOutput4Crossbuck"));
        output4CrossbuckFlasherCheckBox.addActionListener(basicConfigChangeActionListener);

        // DS64 routes
        String[] routesControls = {DS64Bundle.getMessage("ComboBoxEntryRoutesOption0"),
                DS64Bundle.getMessage("ComboBoxEntryRoutesOption1"),
                DS64Bundle.getMessage("ComboBoxEntryRoutesOption2")};
        routesControlLabel = new JLabel (DS64Bundle.getMessage("LabelTriggerDs64Routes"));
        routesControl = new JComboBox<String>(routesControls);    // opSws 11, 16
        routesControl.setToolTipText(DS64Bundle.getMessage("ToolTipLabelRouteControlOptions"));
        routesControl.addActionListener(basicConfigChangeActionListener);
        
    // local input controls

        String[] localControlOfOutputsStyles = {
            DS64Bundle.getMessage("ComboBoxInputsControlOutputsType0"),
            DS64Bundle.getMessage("ComboBoxInputsControlOutputsType1"),
            DS64Bundle.getMessage("ComboBoxInputsControlOutputsType2")};
        localControlOfOutputsStyleLabel = new JLabel (DS64Bundle.getMessage("LabelLocalInputsControlOutputs"));
        localControlOfOutputsStyle    = new JComboBox<String>(localControlOfOutputsStyles); // opSw12
        localControlOfOutputsStyle.setToolTipText(DS64Bundle.getMessage("ToolTipLocalInputsControl"));
        localControlOfOutputsStyle.addActionListener(basicConfigChangeActionListener);
        
        String[] sensorMessageTriggers = {DS64Bundle.getMessage("ComboBoxInputsCauseMessagesType0"),
            DS64Bundle.getMessage("ComboBoxInputsCauseMessagesType1")};
        sensorMessageTriggerLabel = new JLabel(DS64Bundle.getMessage("LabelBetweenForMessageTypeSent"));
        sensorMessageTrigger = new JComboBox<String>(sensorMessageTriggers); // opSw13
        sensorMessageTrigger.setToolTipText(DS64Bundle.getMessage("ToolTipSensorMessageTrigger"));
        sensorMessageTrigger.addActionListener(basicConfigChangeActionListener);

        String[] localSensorTypes = {DS64Bundle.getMessage("ComboBoxSensorMessageTypeSentType0"),
            DS64Bundle.getMessage("ComboBoxSensorMessageTypeSentType1")};
        localSensorType    = new JComboBox<String>(localSensorTypes); // opSw21
        localSensorType.setToolTipText(DS64Bundle.getMessage("ToolTipLocalSensorsType"));
        localSensorType.addActionListener(basicConfigChangeActionListener);

        factoryResetButton = new JToggleButton(DS64Bundle.getMessage("ButtonTextResetToFactoryDefault"));
        factoryResetButton.setToolTipText(DS64Bundle.getMessage("ToolTipButtonTextResetToFactoryDefault"));
        ...
        resetRouteButton = new JToggleButton(DS64Bundle.getMessage("ButtonTextResetRoute"));
        resetRouteButton.setToolTipText(DS64Bundle.getMessage("ToolTipButtonTextResetRoute"));
        resetRouteButton.setEnabled(false);
        ...
        readAllButton.setText(DS64Bundle.getMessage("ButtonTextReadFullSheet"));
        readAllButton.setToolTipText(DS64Bundle.getMessage("ToolTipButtonTextReadFullSheet"));

        writeAllButton.setPreferredSize(null);
        writeAllButton.setText(DS64Bundle.getMessage("ButtonTextWriteFullSheet"));
        writeAllButton.setToolTipText(DS64Bundle.getMessage("ToolTipButtonTextWriteFullSheet"));
        ...
        ((javax.swing.JTextField)addressingPanel.getComponent(2)).setVisible(false);
        ...
        allOutputControlsTitleBorder = javax.swing.BorderFactory.createTitledBorder(blackline,
                DS64Bundle.getMessage("TitledBorderLabelOutputControls"));
    ...
    // command sources
        ...
        ds64CommandSourcesTitleBorder = javax.swing.BorderFactory.createTitledBorder(blackline,
                DS64Bundle.getMessage("TitledBorderLabelCommandSources"));
    ...
    // DS64 routes
        ...
        localRoutesTitleBorder = javax.swing.BorderFactory.createTitledBorder(blackline,
                DS64Bundle.getMessage("TitledBorderLabelDS64Routes"));
    ...
    // local input controls
        JPanel localInputControlsPanel = new JPanel();
        ...
        localInputControlsTitleBorder = javax.swing.BorderFactory.createTitledBorder(blackline,
                DS64Bundle.getMessage("TitledBorderLabelLocalInputControls"));
        ...
        JPanel sensorMessageTriggerPanel = new JPanel(new FlowLayout());
        ...
        generalTabbedPane.addTab(DS64Bundle.getMessage("TabTextBasicSettings"), null,
                generalPanel, DS64Bundle.getMessage("TabToolTipBasicSettings"));
        
        // opsws panel
        JPanel opswsPanel = new JPanel();
        ...
        for (int i = 1; i <= 21; i++) {
            if (i != 7) {
                ...
                opswThrown[i] = new JRadioButtonWithInteger(i, "Thrown");
                opswClosed[i] = new JRadioButtonWithInteger(i, "Closed");
                g[i] = new ButtonGroup();
...
//                        updateDisplay() ;
        ...
        JScrollPane opswsScrollPane = new JScrollPane(opswsValues);
        ...
        p.add(new JLabel(DS64Bundle.getMessage("LabelCautionReadingWritingCanCauseOutputChanges")));
        outputAddrsPanel.add(p);

        p = new JPanel();
        p.setLayout(new FlowLayout());
        p.add(new JLabel(DS64Bundle.getMessage("LabelTextOutput1")));
        outAddr1 = new ValidatedTextField(5, false, 1, 2048, DS64Bundle.getMessage("ErrorTextNonBlankAddressInvalid"));
        outState1 = new JLabel(DS64Bundle.getMessage("LabelTurnoutCurrentlyIsUnknown"));
        p.add(outAddr1);
        p.add(outState1);
        outputAddrsPanel.add(p);
        ...
        p = new JPanel();
        p.setLayout(new FlowLayout());
        p.add(new JLabel(DS64Bundle.getMessage("LabelTextOutput2")));
        outAddr2 = new ValidatedTextField(5, false, 1, 2048, DS64Bundle.getMessage("ErrorTextNonBlankAddressInvalid"));
        outState2 = new JLabel(DS64Bundle.getMessage("LabelTurnoutCurrentlyIsUnknown"));
        p.add(outAddr2);
        p.add(outState2);
        outputAddrsPanel.add(p);
        ...
        p = new JPanel();
        p.setLayout(new FlowLayout());
        p.add(new JLabel(DS64Bundle.getMessage("LabelTextOutput3")));
        outAddr3 = new ValidatedTextField(5, false, 1, 2048, DS64Bundle.getMessage("ErrorTextNonBlankAddressInvalid"));
        outState3 = new JLabel(DS64Bundle.getMessage("LabelTurnoutCurrentlyIsUnknown"));
        p.add(outAddr3);
        p.add(outState3);
        outputAddrsPanel.add(p);
        ...
        p = new JPanel();
        p.setLayout(new FlowLayout());
        p.add(new JLabel(DS64Bundle.getMessage("LabelTextOutput4")));
        outAddr4 = new ValidatedTextField(5, false, 1, 2048, DS64Bundle.getMessage("ErrorTextNonBlankAddressInvalid"));
        outState4 = new JLabel(DS64Bundle.getMessage("LabelTurnoutCurrentlyIsUnknown"));
        p.add(outAddr4);
        p.add(outState4);
        outputAddrsPanel.add(p);
        ...
        p = new JPanel();
        p.setLayout(new FlowLayout());
        p.add(new JLabel(DS64Bundle.getMessage("LabelOutputsTabSensorNotes")));
        outputAddrsPanel.add(p);

        generalTabbedPane.addTab(DS64Bundle.getMessage("TabTextOutputAddrs"), null,
                    outputAddrsPanel, DS64Bundle.getMessage("TabToolTipOutputAddrs"));
        ...
        for (int i = 1; i <= 8; ++i) {
            ...
            routePanel[i].add(new JLabel(DS64Bundle.getMessage("TabTextSpecificRoute", 
                    Integer.toString(i))));
            routePanel[i].add(new JSeparator());
            JPanel q = new JPanel(new FlowLayout());
            q.add(new JLabel(DS64Bundle.getMessage("LabelCautionReadingWritingCanCauseOutputChanges")));
            routePanel[i].add(q);
            routePanel[i].add(new JSeparator());
            ...
            routePanel[i].add(routeTop[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",
                    DS64Bundle.getMessage("LabelTextRouteXTopTurnout"))));
            routePanel[i].add( routeA2[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",2)));
            routePanel[i].add( routeA3[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",3)));
            routePanel[i].add( routeA4[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",4)));
            routePanel[i].add( routeA5[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",5)));
            routePanel[i].add( routeA6[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",6)));
            routePanel[i].add( routeA7[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",7)));
            routePanel[i].add( routeA8[i].createEntryPanel(DS64Bundle.getMessage("LabelTextRouteXSpecificTurnout",8)));

            routesTabbedPane.addTab(
                    DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(i)),
                    null,
                    routePanel[i],
                    DS64Bundle.getMessage("TabToolTipSpecificRoute", Integer.toString(i))
            );
        }

        generalTabbedPane.addTab(DS64Bundle.getMessage("TabTextRoutes"), 
                null, routesTabbedPane, 
                DS64Bundle.getMessage("ToolTipTabTextRoutes"));
        ...
        appendLine(factoryResetButtonPanel);
        JPanel statusPanel = new JPanel();
        setStatus(" ");
        statusPanel.add(new JSeparator());
        statusPanel.add((JLabel) provideStatusLine());
        statusPanel.add(new JSeparator());
        ...
        routesTabbedPane.addChangeListener(new ChangeListener() {
            ...
            String route1TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(1));
            String route2TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(2));
            String route3TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(3));
            String route4TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(4));
            String route5TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(5));
            String route6TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(6));
            String route7TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(7));
            String route8TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(8));
        ...
        generalTabbedPane.addChangeListener(new ChangeListener() {
            // This method is called whenever the selected tab changes
                ...
                String route7TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(7));
                String route8TabText = DS64Bundle.getMessage("TabTextSpecificRoute", Integer.toString(8));
                String outputsTabText = DS64Bundle.getMessage("TabTextOutputAddrs");
            ...
            public void stateChanged(ChangeEvent evt) {
                ...
                // Get current tab
                if ((activeTabTitle.equals(DS64Bundle.getMessage("TabTextRoutes")))) {
                ...
                ((JmriJFrame) pane.getRootPane().getParent()).setPreferredSize(null);
                ((JmriJFrame) pane.getRootPane().getParent()).pack();
            };
        });
        ...
        switch (indexToRead) {
            case 0: {
                ...
                outState2.setText(direction ? 
                        DS64Bundle.getMessage("LabelTurnoutCurrentlyIsClosed") : 
                                DS64Bundle.getMessage("LabelTurnoutCurrentlyIsThrown"));
                break;
           }
            case 1: {
    ...
    private ActionListener basicConfigChangeActionListener = new ActionListener() {
        public void actionPerformed(ActionEvent e) {
            log.debug("basicConfigChangeActionListener");
            copyToOpsw();
            updateBasicOpSwTab();
        }
    };
