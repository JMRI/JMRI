 ...
 * This is because there are a lot of SdfMacros in realistic
 * sound file, and the per-object overhead needed would be too large.
 * Hence (or perhaps because of no need), there is no support for
 * simultaneous editing of a single macro instruction updating multiple
 * windows.
 * You can have multiple editors open on a single SdfBuffer, but these
 * are not interlocked against each other.
 * (We could fix this by having a shared pool of "objects to be notified
 * of changes in the SdfBuffer, acccessed by reference during editing (to
 * avoid another dependency), but that's a project for another 
 * day)
 *
...
public abstract class SdfMacro implements SdfConstants {
     ...
     * @return Fixed numher of bytes defined (a constant for the instruction type)
     */
     ...
     * instructions.
     * @return Newline terminated string, never null
     * @param indent String inserted at the start of each output line,
     ...
     * children, but each subclass needs to store it's own
     ...
     * <P>
     * Note that multiple values can be returned, e.g. this can be
     * used to scan for individual bits set in a variable.
     * @param input Single value to be matched
     * @param values Array of possible values which the input might match
     * @param masks Array of masks to be applied when comparing against the corresponding items in the values array.  This is separate for each possible value to e.g. allow the encoding of a set of independent bits.
     * @param labels Should there be a match-under-mask of a value, the corresponding label is returned
    ...
    String decodeState(int input, int[]values, String[] labels) {
        ...
        if (val == null) return labels[labels.length-1];
        return val;
    }
