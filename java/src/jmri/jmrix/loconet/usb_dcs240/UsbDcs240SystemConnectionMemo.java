 ...
 */
public class UsbDcs240SystemConnectionMemo extends LocoNetSystemConnectionMemo {

    public UsbDcs240SystemConnectionMemo(LnTrafficController lt,
            SlotManager sm) {
        super(lt, sm);
    }

    public UsbDcs240SystemConnectionMemo() {
        super();
    }
    ...
    final static int DCS240USBSTANDALONEPROGRAMMERMODE = 0x00;
    final static int MS100MODE = 0x01;

    int mode = DCS240USBSTANDALONEPROGRAMMERMODE;
    private ShutDownTask restoreToLocoNetInterfaceModeTask;

    /**
     * Configure the subset of LocoNet managers valid for the DCS240 USB interface in PR2 mode.
     */
    public void configureManagersPR2() {
        mode = DCS240USBSTANDALONEPROGRAMMERMODE;
        ...
        // Establish a ShutDownTask so that the DCS240 USB interface should be be returned to
        ...
        if (restoreToLocoNetInterfaceModeTask == null) {
            restoreToLocoNetInterfaceModeTask = new QuietShutDownTask("Restore DCS240 USB interface to LocoNet Interface Mode") {    // NOI18N
                @Override
                public boolean execute() {

                    if (mode == DCS240USBSTANDALONEPROGRAMMERMODE) {
                        ...
                        if (tc != null) {
                            ...
                            log.info("Configuring DCS240 USB interface for 'LocoNet Interface' mode"); // NOI18N
                        }
                    }
                    return true;
                }
            ...
            if (InstanceManager.getNullableDefault(jmri.ShutDownManager.class) != null) {
                ...
                log.warn("The DCS240 USB interface will not be automatically returned to 'LocoNet interface' mode upon quit!"); // NOI18N
            }
        }
    }
    ...
    /**
     * Configure the LocoNet managers valid for the DCS240 USB interface in MS100 
    ...
    @Override
    public void dispose() {
        InstanceManager.deregister(this, UsbDcs240SystemConnectionMemo.class);
        if(tm!=null){
           tm.dispose();
        }
        super.dispose();
