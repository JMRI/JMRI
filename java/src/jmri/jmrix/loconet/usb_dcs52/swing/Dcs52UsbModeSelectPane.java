 ...
 * @author Bob Jacobsen Copyright (C) 2005
  */
public class Dcs52UsbModeSelectPane extends jmri.jmrix.loconet.swing.LnPanel implements LocoNetListener {
    ...
    public void message(LocoNetMessage msg) {
        if ((msg.getOpCode() == LnConstants.OPC_PEER_XFER)
                ...
                && (msg.getElement(4) == 0x01)) {  // Digitrax form, check "DCS52 USB as standalone programmer" or "DCS52 USB as LocoNet interface" mode
            int mode = msg.getElement(8) & 0x0C;
            if (mode == 0x00) {
                // PR2 format (i.e. "DCS52 USB interface as standalone programmer" mode)
                status.setText(Bundle.getMessage("StatusPr2"));
            } else {
                // MS100 format (i.e. "DCS52 USB interface as LocoNet interface" mode)
                status.setText(Bundle.getMessage("StatusMs100"));
            }
        }
    }
