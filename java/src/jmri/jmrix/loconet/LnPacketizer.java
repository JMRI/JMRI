...
public class LnPacketizer extends LnTrafficController {
    ...
    /*
     * Alternate Packetizer(s) start here. They can be invoked by the use of properties on the command line.
     */
     ...
     * the LnPortController via <code>connectPort</code>.
     */
    protected class RcvHandler_Strict implements Runnable {
        ...
        public RcvHandler_Strict(LnTrafficController lt) {
            trafficController = lt;
        }
     ...
     * Captive class to handle transmission
     */
    class XmtHandler_Strict implements Runnable {
                    ...
                    try {
                        if (ostream != null) {
                            ...
                            while (reTryRequired) {
                                ...
                                if (waitCount > 19) {
                                    try {
                                        log.warn("Retry Send for Lost Packet [{}] Count[{}]", waitForMsg.toString(), reTryCount); // NOI18N
    ...
    public void startThreads() {
        // see if alternative packetizers have been invoked
        String strictPacketizer = "false";
        strictPacketizer = System.getProperty("digitrax.strictpacketizer");
        if (strictPacketizer == null) {
            strictPacketizer = "FALSE";
        }
        if (strictPacketizer.toUpperCase().equals("TRUE")) {
            // Invoke alternative packetizer strict
            ...
            if (xmtHandler == null) {
                xmtHandler = new XmtHandler_Strict();
            }
            ...
            if (rcvHandler == null) {
                rcvHandler = new RcvHandler_Strict(this);
            }
