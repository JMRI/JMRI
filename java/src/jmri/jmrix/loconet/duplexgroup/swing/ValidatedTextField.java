 ...
 * function.
 *
 * Supports two types of validated field:
 *  .   generic text fields with length and/or character set limited by a
...
public class ValidatedTextField extends javax.swing.JTextField  {
     ...
     * matches the "Last Queried Value", the field is colorized using the default 
     * field foreground and background colors.  If instead the current field value
     * does not match the "Last Queried Value", the field background color is 
     ...
     * would require the text field to be a 5 or 6 character string which starts 
     * with exactly two digits and followed by either 3 or 4 upper-case or lower-case
     * letters.
     *
     ...
     * Provides a validated text field, where the validation mechanism requires
     * a String value which passes the matching defined in validationRegExpr,
     * and where the string begins with a number which must be within a specified
     * integral range.
     *
     * Validation occurs as part of the process of focus leaving the field.
     * When validation fails, the focus remains within the field, and the field
     ...
     * Parameter len defines the width of the text field entry box, in characters.
     *
     * Parameter allow0LengthValue determines if a value of 0 characters is allowed
     * as a valid value.
     *
     * Parameter forceUppercase determines if all alphabetic characters are
     * forced to uppercase.
    ...
    /**
     * Method to colorize enabled field based on comparison with the last queried
     * value.  If the field is disabled, no colorization occurs.
     */
    ...
    /**
     * Validates the field information.  Does not make any GUI changes.
    ...
    public boolean isValid() {
        ...
        if (fieldType == FieldType.TEXT) {
            ...
            try {
                if ((allow0Length == true) && (value.length() == 0)) return true;
            ...
            if ((location != -1) && (location < findLocation)) findLocation = location;
            location = value.indexOf('C');
            if ((location != -1) && (location < findLocation)) findLocation = location;
            ...
            location = value.indexOf('T');
            if ((location != -1) && (location < findLocation)) findLocation = location;
            if (findLocation == 999) return false;
    ...
    /**
     * Sets the color used for the field background when the field value is invalid.
     * <p>
    ...
    static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(ValidatedTextField.class.getName());
}
