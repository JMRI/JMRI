 ...
 * @author Bob Jacobsen Copyright (C) 2005
  */
public class Pr4SelectPane extends jmri.jmrix.loconet.swing.LnPanel implements LocoNetListener {
    ...
    public void message(LocoNetMessage msg) {
        if ((msg.getOpCode() == LnConstants.OPC_PEER_XFER)
                ...
                && (msg.getElement(4) == 0x01)) {  // Digitrax form, check "PR4 standalone programmer" or "PR4 LocoNet interface" mode
            int mode = msg.getElement(8) & 0x0C;
            if (mode == 0x00) {
                // PR2 format (i.e. "PR4 standalone programmer" mode)
                status.setText(Bundle.getMessage("StatusPr2"));
            } else {
                // MS100 format (i.e. "PR4 LocoNet interface" mode)
                status.setText(Bundle.getMessage("StatusMs100"));
            }
        }
    }
