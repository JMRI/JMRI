 ...
 * <P>
 * System names are "USnnn:yy", where nnn is the Marklin Object Number for a given
        ...
        if(r.getPriority()==MarklinConstants.PRIO_1 && r.getCommand()>=MarklinConstants.FEECOMMANDSTART && r.getCommand()<=MarklinConstants.FEECOMMANDEND ){
            if(r.getCommand()==MarklinConstants.S88EVENT){
                ...
                if(sensorList==null){
                    ...
                    tc.sendMarklinMessage(m, this);
                    if(log.isDebugEnabled()) log.debug("New module added " + module);
                }
                ...
                if(log.isDebugEnabled()) log.debug("State s88Module not registered " + s88Module);
            }
        }
    }
