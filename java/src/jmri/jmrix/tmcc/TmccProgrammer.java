 ...
 */
public class TmccProgrammer extends AbstractProgrammer implements SerialListener {
    ...
    /**
     * Internal method to create the TMCC SerialMessage for programmer task start.
     ...
     */
    protected SerialMessage progTaskStart(ProgrammingMode mode, int val, int cvnum) throws jmri.ProgrammerException {
        // val = -1 for read command; mode is direct, etc
        if (val < 0) {
            // read
            if (getMode().equals(ProgrammingMode.PAGEMODE)) {
                return null; // ??? return EasyDccMessage.getReadPagedCV(cvnum);
            } else {
                return null; // ??? return EasyDccMessage.getReadRegister(registerFromCV(cvnum));
            }
        } else {
            // write
            if (getMode().equals(ProgrammingMode.PAGEMODE)) {
                return null; // ??? return EasyDccMessage.getWritePagedCV(cvnum, val);
            } else {
                return null; // ??? return EasyDccMessage.getWriteRegister(registerFromCV(cvnum), val);
            }
        }
    }
     ...
     */
    @Override
    synchronized public void reply(SerialReply m) {
    ...
    private final static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(TmccProgrammer.class);
