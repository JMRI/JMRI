 ...
 *
 * Note: Currently anything to do with pulse width for pulsing a turnout is commented out.  This code 
 *		from the C/MRI version was not deleted in case it is needed in the future.
 *
...
//		panel1.add(panel13);
    ...
    public void addButtonActionPerformed() {
        ...
        if ( !readPollTimeout() ) return;
        if ( !readSendDelay() ) return;
//		if ( !readPulseWidth() ) return;
		if ( !readNumInputBits() ) return;
		if ( !readNumOutputBits() ) return;
...
//        curNode.setPulseWidth(pulseWidth);
     ...
     *     If successful, a node address in the range 1-99 is returned.
