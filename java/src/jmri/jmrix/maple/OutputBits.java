 ...
 * <P>
 * All of the Maple HMI panels receive the same output bits.  This keeps them synchronized.  Output is 
 *		sent in "broadcast" mode to Station Address 0.  All HMI's receive the output.
 * Output is sent at the end of each polling cycle, whether or not anything has changed.  That way, if 
 *		an HMI panel is plugged in, it will be up-to-date within one polling cycle.
 * Serial systems with unique output bits for each node keep their output array in each node.  That 
 *		code has been moved to this utility class for Maple Systems because all nodes share the same 
 *		output bits.
 * Coil bits within Maple Systems HMI's are divided into input (1-1000) and output (1001-9000), so 
 *		input bits are read starting from HMI address 1, and output bits are written starting at HMI
 *		address 1001.
 * <P>
...
//	private int mPulseWidth = 500;
	...
	// access routines
	public static void setNumOutputBits(int n) {mNumOutputBits = n;}
	public static int getNumOutputBits() {return mNumOutputBits;}
//	public int getPulseWidth() {return (mPulseWidth);}
//	public void setPulseWidth(int width) {mPulseWidth = width;}
	public static void setSendDelay(int n) {mSendDelay = n;}
	public static int getSendDelay() {return mSendDelay;}
    ...
    public SerialMessage createOutPacket(int startBitNum, int endBitNum) {
        ...
        for (int i = 0; i < nBits; i++) {
            ...
            if (j==0) m.setElement(11+i, ((val&0x01)!=0) ? '1' : '0');
            else if (j==1) m.setElement(11+i, ((val&0x02)!=0) ? '1' : '0');
            else if (j==2) m.setElement(11+i, ((val&0x04)!=0) ? '1' : '0');
            else if (j==3) m.setElement(11+i, ((val&0x08)!=0) ? '1' : '0');
            else if (j==4) m.setElement(11+i, ((val&0x10)!=0) ? '1' : '0');
            else if (j==5) m.setElement(11+i, ((val&0x20)!=0) ? '1' : '0');
            else if (j==6) m.setElement(11+i, ((val&0x40)!=0) ? '1' : '0');
            else if (j==7) m.setElement(11+i, ((val&0x80)!=0) ? '1' : '0');
        }
    ...
    static Logger log = Logger.getLogger(OutputBits.class.getName());
}
