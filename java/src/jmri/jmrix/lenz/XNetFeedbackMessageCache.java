...
public class XNetFeedbackMessageCache implements XNetListener {
    ...
    public XNetFeedbackMessageCache(XNetTrafficController controller) {
        ...
        for(int i=0;i<256;i++) messageCache[i][0]=messageCache[i][1]=null;
        messagePending=new Boolean[256][2];
        for(int i=0;i<256;i++) messagePending[i][0]=messagePending[i][1]=false;
        tc=controller;
    ...
    synchronized public void requestCachedStateFromLayout(XNetTurnout turnout)
    {
       int pNumber=turnout.getNumber();
            if(messagePending[(pNumber-1)/4][((pNumber-1)%4)<2?0:1]) return;
        try {
            if(messageCache[(pNumber-1)/4][((pNumber-1)%4)<2?0:1]!=null)
            {
               if(log.isDebugEnabled()) log.debug("Message for turnout " + pNumber + " cached.");
    ...
    synchronized public void requestCachedStateFromLayout(XNetSensor sensor)
    {
       int pNumber=sensor.getNumber();
            if(messagePending[sensor.getBaseAddress()][sensor.getNibble()>>4]) return;
        try {
            if(messageCache[sensor.getBaseAddress()][sensor.getNibble()>>4]!=null)
            {
               if(log.isDebugEnabled()) log.debug("Message for sensor " + pNumber + " cached.");
    ...
    static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(XNetFeedbackMessageCache.class.getName());
