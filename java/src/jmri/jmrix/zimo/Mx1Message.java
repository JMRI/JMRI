...
public class Mx1Message extends jmri.jmrix.NetMessage implements Serializable {
    ...
    public int getModule(){
        ...
        if((mod&MX1) == MX1) return MX1;
        if((mod&MX8) == MX8) return MX8;
        if((mod&MX9) == MX9) return MX9;
        return mod;
    }
    ...
    public boolean messageSource(){
        if((getElement(0)&0x08) == 0x08) return PC;
        return CS;
    }
    ...
    public boolean isCRCError() { return crcError; }
        /**
...
// programma komt hier volgens mij nooit
    ...
    public String getStringMsg(){
        ...
        if(protocol == Mx1Packetizer.BINARY){
            if(isCRCError()) txt.append(" === CRC ERROR === ");
            ...
            int offset=0;
            if(getMessageType()==PRIMARY) txt.append(" Prim");
            else if(getMessageType()==ACKREP1) txt.append(" Ack/Reply 1");
            else if(getMessageType()==REPLY2) txt.append(" Reply 2");
            else if(getMessageType()==ACK2) txt.append(" Ack 2");
            else txt.append(" Unknown msg");
            if(getModule() == MX1){  //was (getElement(1)&0x00) == 0x00
                ...
                switch (getPrimaryMessage()) {  //was getElement(2)
                    ...
                    case 4: txt.append(" Loco Funct ");
                            break;
                    case 5: txt.append(" Loco Acc/Dec ");
                            break;
                    ...
                    case 8: txt.append(" Loco Status ");
                            break;
                    case 9: txt.append(" Acc Status ");
                            break;
                    case 10: txt.append(" Address Control ");
                            break;
                    case 11: txt.append(" CS State ");
                            break;
                    case 12: txt.append(" Read/Write CS CV ");
                            break;
                    case 13: txt.append(" CS Equip Query ");
                            break;
                    case 17: txt.append(" Tool Type ");
                            break;
                    case PROGCMD : offset=0;
                            if(getMessageType()==ACKREP1){
                                txt.append(" Prog CV ");
                                break;
                            }
                            ...
                            txt.append("Loco: "+getLocoAddress(getElement((3+offset)), getElement(4+offset)));
                            if((getElement(3+offset)&0x80)==0x80) txt.append(" DCC");
                    ...
                    case 254: txt.append(" Cur Acc Memory ");
                            break;
                    case 255: txt.append(" Cur Loco Memory ");
                            break;
    ...
    String getErrorCode(int i){
        switch(i){
            ...
            case ERR_ADDRESS : return "Invalid Address";
            case ERR_INDEX : return "Invalid Index";
            case ERR_FORWARD : return "Could not be Forwarded";
            case ERR_BUSY : return "CMD Busy";
            case ERR_NO_MOT : return "Motorola Jump Off";
            case ERR_NO_DCC : return "DCC Jump Off";
            case ERR_CV_ADDRESS : return "Invalid CV";
            case ERR_SECTION : return "Invalid Section";
            case ERR_NO_MODUL : return "No Module Found";
            case ERR_MESSAGE : return "Error in Message";
            case ERR_SPEED : return "Invalid Speed";
    ...
    /**
     * @param locoAddress : address of the loco, if left blank then programming track is used
     * @param cv : CV that is to be either read in or written
     * @param value : Value to be written to the CV, if set to -1, then a read is done
