...
public class ControlPanel extends JPanel implements PropertyChangeListener, AddressListener {
    ...
    private ImageIcon scaleTo(Document svgImage, Float f ) {        
        ...
        try {
            ...
            // log it, but continue
            log.debug("Exception while transposing {} : {}", svgImage.getBaseURI(), ex.getMessage());
        } 
    ...
    private void initGUI(boolean withPopupMenu) {
        ...
        add(speedPanel, BorderLayout.CENTER);
        ...
        speedSpinner.getActionMap().put("giveUpFocus", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
               InstanceManager.getDefault(ThrottleFrameManager.class).getCurentThrottleController().getRootPane().requestFocusInWindow();
            }
        });
    ...
    private void changeOrientation() {
        ...
        if (getWidth() > getHeight()) {
            ...
            if ( preferences.isUsingExThrottle() && preferences.isUsingFunctionIcon() && preferences.isUsingLargeSpeedSlider() ) {
                int bpw = getHeight()*5/2;
                if (bpw > getWidth()/2) {
                    bpw = getWidth()/2;
                }
                buttonPanel.setSize(bpw, getHeight());
                resizeButtons();
            }
            remove(buttonPanel);
            ...
            speedSliderContinuous.setOrientation(JSlider.VERTICAL);                           
            if ( preferences.isUsingExThrottle() && preferences.isUsingFunctionIcon() && preferences.isUsingLargeSpeedSlider() ) {
                int bph = getWidth()*2/5;
                if (bph > getHeight()/2) {
                    bph = getHeight()/2;
                }
                buttonPanel.setSize(getWidth(), bph);
                resizeButtons();
            }
            remove(buttonPanel);
            add(buttonPanel, BorderLayout.SOUTH);
