 ...
 */
public class SwitchAdder extends JPanel implements ListSelectionListener {
    ...
    public SwitchAdder(boolean allowDeletes) {
        this();
    ...
    public SwitchAdder(String type) {
        this();
        _type = type;
        initDefaultIcons();
    }
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                for (int j = 0; j < states.length; j++) {
                    if (name.equals(states[j])
                            || leaf.getName().equals(Bundle.getMessage("SignalHeadStateDark"))
    ...
    private final static Logger log = LoggerFactory.getLogger(SwitchAdder.class.getName());
}
