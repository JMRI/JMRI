...
public class LayoutEditorAuxTools {
    ...
    private void checkConnectivity(LayoutConnectivity c, boolean[] found) {
        ...
        LayoutEditor.HitPointTypes type = c.getConnectedType();
    ...
    public void addBeanSettings(Path p, LayoutConnectivity lc, LayoutBlock layoutBlock) {
        ...
        LayoutEditor.HitPointTypes typeCurConnection = LayoutEditor.HitPointTypes.NONE;
        BeanSetting bs = null;
        ...
        if (lc.getBlock1() == layoutBlock) {    // block1 is this LayoutBlock
            ...
            if (curConnection != null) {        // connected track in this block is a track segment
                ...
                typeCurConnection = LayoutEditor.HitPointTypes.TRACK;
                // is this Track Segment connected to a RH, LH, or WYE turnout at the continuing or diverging track?
                if ((lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_B
                        || lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_C)
                    ...
                    } else if (lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_C) {
                ...
                else if (LayoutEditor.HitPointTypes.isTurnoutHitType(lc.getConnectedType())
                    ...
                    if (((ltz.getTurnoutType() == LayoutTurnout.TurnoutType.RH_XOVER)
                            && ((lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_B)
                            || (lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_D)))
                            || ((ltz.getTurnoutType() == LayoutTurnout.TurnoutType.LH_XOVER)
                            && ((lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_A)
                            || (lc.getConnectedType() == LayoutEditor.HitPointTypes.TURNOUT_C)))) {
                ...
                else if (LayoutEditor.HitPointTypes.isSlipHitType(lc.getConnectedType())) {
                    LayoutSlip lsz = (LayoutSlip) prevConnection;
                    if (lsz.getSlipType() == LayoutSlip.TurnoutType.SINGLE_SLIP) {
                        if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_C) {
                        ...
                        } else if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_B) {
                            if (lsz.getTurnout() != null) {
                        ...
                        } else if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_A) {
                            log.debug("At connection A of a single slip which could go in two different directions");  // NOI18N
                        } else if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_D) {
                        ...
                        if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_A) {
                        ...
                        } else if (lc.getConnectedType() == LayoutEditor.HitPointTypes.SLIP_C) {
            ...
            if (lc.getConnectedObject() != null) {
                ...
                if (LayoutEditor.HitPointTypes.isTurnoutHitType(typeCurConnection)) {
                    ...
                    if (LayoutTurnout.hasEnteringDoubleTrack(turnoutType)) {
                        ...
                        if ((turnoutType == LayoutTurnout.TurnoutType.DOUBLE_XOVER)
                                || ((turnoutType == LayoutTurnout.TurnoutType.RH_XOVER) && ((typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_A) || (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_C)))
                                || ((turnoutType == LayoutTurnout.TurnoutType.LH_XOVER) && ((typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_B) || (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_D)))) {
                            ...
                            if (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_A) {
                                curConnection = ((LayoutTurnout) curConnection).getConnectB();
                            } else if (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_B) {
                                curConnection = ((LayoutTurnout) curConnection).getConnectA();
                            } else if (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_C) {
                    ...
                    else if (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_A) {
                ...
                else if (typeCurConnection == LayoutEditor.HitPointTypes.LEVEL_XING_A) {
                    prevConnection = curConnection;
                    curConnection = ((LevelXing) curConnection).getConnectC();
                    typeCurConnection = LayoutEditor.HitPointTypes.TRACK;
                } else if (typeCurConnection == LayoutEditor.HitPointTypes.LEVEL_XING_C) {
                    prevConnection = curConnection;
                    curConnection = ((LevelXing) curConnection).getConnectA();
                    typeCurConnection = LayoutEditor.HitPointTypes.TRACK;
                } else if (typeCurConnection == LayoutEditor.HitPointTypes.LEVEL_XING_B) {
                    prevConnection = curConnection;
                    curConnection = ((LevelXing) curConnection).getConnectD();
                    typeCurConnection = LayoutEditor.HitPointTypes.TRACK;
                } else if (typeCurConnection == LayoutEditor.HitPointTypes.LEVEL_XING_D) {
                    prevConnection = curConnection;
        ...
        while (curConnection != null) {
            if (typeCurConnection == LayoutEditor.HitPointTypes.TRACK) {
                ...
                if (curTS.getLayoutBlock() != layoutBlock) {
                    ...
                    if (typeCurConnection == LayoutEditor.HitPointTypes.POS_POINT) {
            ...
            } else if (LayoutEditor.HitPointTypes.isTurnoutHitType(typeCurConnection)) {
                ...
                if (lt.hasEnteringSingleTrack()) {
                    ...
                    if ((typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_A)
                            || (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_C)) {
                    ...
                    if ((typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_B)
                            || (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_D)) {
                        // entry is at turnout throat, cannot follow possible path any further
                        curConnection = null;
                    } else if (typeCurConnection == LayoutEditor.HitPointTypes.TURNOUT_A) {
                        // entry is at continuing track of turnout
            ...
            } else if (LayoutEditor.HitPointTypes.isSlipHitType(typeCurConnection)) {
                ...
                if (ls.getLayoutBlock() != layoutBlock) {
                    ...
                    if (typeCurConnection == LayoutEditor.HitPointTypes.SLIP_C) {
                        ...
                        typeCurConnection = LayoutEditor.HitPointTypes.TRACK;
                    } else if (typeCurConnection == LayoutEditor.HitPointTypes.SLIP_B) {
                        if (ls.getTurnout() != null) {
            ...
            } else if (LayoutEditor.HitPointTypes.isTurntableRayHitType(typeCurConnection)) {
