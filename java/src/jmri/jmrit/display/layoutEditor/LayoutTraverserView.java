...
import jmri.*;
import jmri.jmrit.display.layoutEditor.LayoutTraverser.RayTrack;
...
public class LayoutTraverserView extends LayoutTrackView {
    ...
    //
    /**
     * Get the radius for this traverser.
     *
     * @return the radius for this traverser
     */
    public double getRadius() {
        return traverser.getRadius();
    }

    /**
     * Set the radius for this traverser.
     *
     * @param r the radius for this traverser
     */
    public void setRadius(double r) {
        traverser.setRadius(r);
    }
    ...
    public TrackSegment getRayConnectIndexed(int index) {
        return traverser.getRayConnectIndexed(index);
    }
    ...
    public TrackSegment getRayConnectOrdered(int i) {
        return traverser.getRayConnectOrdered(i);
    }
    ...
    public void setRayConnect(TrackSegment ts, int index) {
        traverser.setRayConnect(ts, index);
    }

    // should only be used by xml save code
    public List<RayTrack> getRayTrackList() {
        return traverser.getRayTrackList();
    }

    /**
     * Get the number of rays on traverser.
     *
     * @return the number of rays
     */
    public int getNumberRays() {
        return traverser.getNumberRays();
    }
    ...
    public int getRayIndex(int i) {
        return traverser.getRayIndex(i);
    }
    ...
    public double getRayAngle(int i) {
        return traverser.getRayAngle(i);
    }
    ...
    public void setRayTurnout(int index, String turnoutName, int state) {
        traverser.setRayTurnout(index, turnoutName, state);
    }
    ...
    public String getRayTurnoutName(int i) {
        return traverser.getRayTurnoutName(i);
    }
    ...
    public Turnout getRayTurnout(int i) {
        return traverser.getRayTurnout(i);
    }
    ...
    public int getRayTurnoutState(int i) {
        return traverser.getRayTurnoutState(i);
    }
    ...
    public boolean isRayDisabled(int i) {
        return traverser.isRayDisabled(i);
    }
    ...
    public void setRayDisabled(int i, boolean boo) {
        traverser.setRayDisabled(i, boo);
    }
    ...
    public boolean isRayDisabledWhenOccupied(int i) {
        return traverser.isRayDisabledWhenOccupied(i);
    }
    ...
    public void setRayDisabledWhenOccupied(int i, boolean boo) {
        traverser.setRayDisabledWhenOccupied(i, boo);
    }
    ...
    public Point2D getRayCoordsIndexed(int index) {
        ...
        for (RayTrack rt : traverser.rayTrackList) {
    ...
    public Point2D getRayCoordsOrdered(int i) {
        Point2D result = MathUtil.zeroPoint2D;
        if (i < traverser.rayTrackList.size()) {
            RayTrack rt = traverser.rayTrackList.get(i);
            if (rt != null) {
                double angle = Math.toRadians(rt.getAngle());
                double rayRadius = getRadius() + LayoutEditor.SIZE * layoutEditor.getTurnoutCircleSize();
                // calculate coordinates
    ...
    public Point2D getCoordsForConnectionType(HitPointType connectionType) {
        ...
        if (HitPointType.TURNTABLE_CENTER == connectionType) {
            ...
            result = getRayCoordsIndexed(connectionType.traverserTrackIndex());
        } else {
    ...
    public LayoutTrack getConnection(HitPointType connectionType) throws jmri.JmriException {
        ...
        if (HitPointType.isTraverserRayHitType(connectionType)) {
            result = getRayConnectIndexed(connectionType.traverserTrackIndex());
        } else {
    ...
    public void setConnection(HitPointType connectionType, LayoutTrack o, HitPointType type) throws jmri.JmriException {
        ...
        if (HitPointType.isTraverserRayHitType(connectionType)) {
            if ((o == null) || (o instanceof TrackSegment)) {
                setRayConnect((TrackSegment) o, connectionType.traverserTrackIndex());
            } else {
    ...
    public void rotateCoords(double angleDEG) {
        ...
        traverser.rayTrackList.forEach((rayTrack) -> {
    ...
    protected JPopupMenu showPopup(@Nonnull JmriMouseEvent mouseEvent) {
        ...
        JMenuItem jmi = popupMenu.add(Bundle.getMessage("MakeLabel", Bundle.getMessage("Traverser")) + getName());
        ...
        if (!traverser.rayTrackList.isEmpty()) {
            JMenu connectionsMenu = new JMenu(Bundle.getMessage("Connections"));
            traverser.rayTrackList.forEach((rt) -> {
                TrackSegment ts = rt.getConnect();
                if (ts != null) {
    ...
    protected void showRayPopUp(JmriMouseEvent e, int index) {
        ...
        for (RayTrack rt : traverser.rayTrackList) {
            if (rt.getConnectionIndex() == index) {
                JMenuItem jmi = rayPopup.add("Traverser Ray " + index);
                jmi.setEnabled(false);
    ...
    /**
     * Set traverser position to the ray with this index.
     *
     * @param index the index
     */
    ...
    public void deleteRay(RayTrack rayTrack) {
        TrackSegment t = null;
        if (traverser.rayTrackList == null) {
    ...
    protected void draw1(Graphics2D g2, boolean isMain, boolean isBlock) {
        log.trace("LayoutTraverser:draw1 at {}", getCoordsCenter());
        ...
        if (isBlock && isMain) {
            ...
            // draw traverser circle - default track color, side track width
    ...
    protected void draw2(Graphics2D g2, boolean isMain, float railDisplacement) {
        log.trace("LayoutTraverser:draw2 at {}", getCoordsCenter());
...
//            }
    ...
    protected void highlightUnconnected(Graphics2D g2, HitPointType specificType) {
        log.trace("LayoutTraverser:highlightUnconnected");
        for (int j = 0; j < getNumberRays(); j++) {
            if (  (specificType == HitPointType.NONE)
                    || (specificType == (HitPointType.traverserTrackIndexedValue(j)))
                )
            {
    ...
    /**
     * Draw this traverser's controls.
     *
     * @param g2 the graphics port to draw to
     */
    @Override
    protected void drawTurnoutControls(Graphics2D g2) {
        log.trace("LayoutTraverser:drawTurnoutControls");
        if (isTurnoutControlled()) {
            // draw control circles at all but current position ray tracks
            for (int j = 0; j < getNumberRays(); j++) {
                if (getPosition() != j) {
                    RayTrack rt = traverser.rayTrackList.get(j);
    ...
    /**
     * Draw this traverser's edit controls.
     *
     * @param g2 the graphics port to draw to
     */
    @Override
