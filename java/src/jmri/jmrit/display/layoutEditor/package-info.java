 ...
 * <h3>Connectivity</h3>
 * This is coded and stored as the following: (Some graphical attributes
 ...
 * <p>
 * For TrackSegment objects, a connection is represented by the name of the
 * other end, and the type (in <code>type2</code> or <code>type2</code>) of the
 * connection at the other end. For example, a connection in object T1 to a
 * turnout might give the turnouts TO1 name, and that the connection is to the
 * TURNOUT_C leg of that turnout.
 * <p>
 * Connections to END BUMPERs and EDGE CONNECTORs don't quite fit that. The far
 * end sees the connection to the EB and EC as to a POS_POINT, not specifically
 * a END BUMPER or EDGE CONNECTOR. There's no <code>type1</code> value
 * representing the far end of the connection <u>from</u> the EB and EC either.
 * But the EB and EC PositionablePoint knows what itself is through its
 * <code>type</code> variable, and that can be queried by following the link as
 * there's only one connection to a PositionablePoint.
 * <p>
 ...
 * <ul>
 * <li>The track is represented by a hierarchy of classes that's rooted in
 * {@link jmri.jmrit.display.layoutEditor.LayoutTrack} and completely
 * disconnected from the PanelEditor classes. New classes are being added to
 * represent the specific types previously implemented via type variables and
 * conditional code.
 ...
 * <li>{@link jmri.jmrit.display.layoutEditor.LayoutBlock} and
 * {@link jmri.jmrit.display.layoutEditor.LayoutBlockManager} are a
 * {@link jmri.NamedBean NamedBean} pair. The
 * {@link jmri.jmrit.display.layoutEditor.LayoutBlock} is a group of track
 * segments and turnouts that corresponds to a 'block'. It may contain a
 * {@link jmri.Memory} and/or {@link jmri.Sensor}. If present, the sensor
 * defines occupancy for the LayoutBlock. Each LayoutBlock is paired 1-to-1 with
 * a {@link jmri.Block} which it created; the Block has a specific form for its
 * system name and the LayoutBlock's name as its user-name.
 * <li>{@link jmri.jmrit.display.layoutEditor.BlockContentsIcon}, {@link jmri.jmrit.display.layoutEditor.MemoryIcon}
 * inherit from the equivalents in the {@link jmri.jmrit.display} package.
 * <li>{@link jmri.jmrit.display.layoutEditor.LayoutEditor} extends
 * {@link jmri.jmrit.display.panelEditor.PanelEditor PanelEditor}
 ...
 *      {@link jmri.jmrit.display.layoutEditor.MultiIconEditor}, and
 * {@link jmri.jmrit.display.layoutEditor.MultiSensorIconFrame}, extend Swing or
 * AWT components. (Although
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditorComponent} sounds like a
 * base class for others, it's not; it's part of the
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditor} implementation.)
 ...
 * JPane containing the layout drawing itself.
 * <p>
 * The LayoutTrack tree defines <code>draw1</code> and <code>draw2</code>
 * methods that draw two different representations of the track elements. These
 * are only invoked from {}@link
 * jmri.jmrit.display.layoutEditor.LayoutEditorComponent}. LayoutEditorComponent
 * is a JComponent with a
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditorComponent#paint} public
 * method that invokes a series of internal private methods to display the
 * layers of the layout drawing. That in turn is invoked via the usual repaint()
 * mechanism, although it's often kicked off by a call to
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditor#redrawPanel()}. Each of
 * those layer private methods sets up graphics and method options, then calls
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditorComponent#draw1} or
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditorComponent#draw2}. Those in
 * turn loop through a list from
 * {@link jmri.jmrit.display.layoutEditor.LayoutEditor#getLayoutTrackViews()}
 * calling their individual
 * {@link jmri.jmrit.display.layoutEditor.LayoutTrackView#draw1} and
 * {@link jmri.jmrit.display.layoutEditor.LayoutTrackView#draw2}
 * methods.
 * <p>
 ...
 * <li>{@link jmri.jmrit.display.layoutEditor.LayoutBlockManager} (registered
 * with ConfigurationManager)
 * <li>{@link jmri.jmrit.display.layoutEditor.LayoutEditor} (registered with
 ...
 * </ul>
 * Only LayoutBlockManager and LayoutEditor are registered with the
 * {@link jmri.ConfigureManager}.
 * <p>
 * LayoutBlockManager is stored and loaded in the usual manager way, including
 * the LayoutBlock objects. They are stored with the configuration information
 * because they can be used on multiple LayoutPanels.
 * <p>
 * LayoutEditorXml handles the storing and loading of all the track and icon
 * objects. It is stored at the user level ("Store Panels...")
 * <p>
 * A {@link jmri.jmrit.display.layoutEditor.BlockValueFile} stores and loads the
 * value (internal temporary contents) of Block objects from the BlockManager.
 * It doesn't reference {@link jmri.jmrit.display.layoutEditor.LayoutBlock}
 * objects.
 * <h2>More Info</h2>
