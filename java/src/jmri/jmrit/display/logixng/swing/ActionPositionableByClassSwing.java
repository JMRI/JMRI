...
import jmri.jmrit.display.logixng.ActionPositionableByClass.Operation;
 ...
 */
public class ActionPositionableByClassSwing extends AbstractDigitalActionSwing {

    private String _selectedEditor;

    private JComboBox<EditorItem> _editorComboBox;
    private JTabbedPane _tabbedPanePositionable;
    private JComboBox<String> _classNamesComboBox;
    ...
    protected void createPanel(@CheckForNull Base object, @Nonnull JPanel buttonPanel) {
        ActionPositionableByClass action = (ActionPositionableByClass)object;
        ...
        _classNamesComboBox = new JComboBox<>();
        JComboBoxUtil.setupComboBoxMaxRows(_classNamesComboBox);
        updatePositionables(null);
        _panelPositionableDirect.add(_classNamesComboBox);
        ...
        if (action != null) {
            ...
            if (action.getClassName() != null) {
                _classNamesComboBox.setSelectedItem(action.getClassName());
            }
        ...
        List<JComponent> componentList = SwingConfiguratorInterface.parseMessage(
                Bundle.getMessage("ActionPositionableByClass_Components"), components);

        for (JComponent c : componentList) panel.add(c);
    }

    private void updatePositionables(ActionEvent e) {
        _classNamesComboBox.removeAllItems();
        ...
        for (String className : item._editor.getClassNames()) {
            list.add(className);
        }
        Collections.sort(list);
        for (String s : list) {
            _classNamesComboBox.addItem(s);
        }
    }
    ...
    public boolean validate(@Nonnull List<String> errorMessages) {
        ...
        ActionPositionableByClass action = new ActionPositionableByClass("IQDA1", null);
    ...
    public MaleSocket createNewObject(@Nonnull String systemName, @CheckForNull String userName) {
        ActionPositionableByClass action = new ActionPositionableByClass(systemName, userName);
        updateObject(action);
    ...
    public void updateObject(@Nonnull Base object) {
        if (! (object instanceof ActionPositionableByClass)) {
            throw new IllegalArgumentException("object must be an ActionPositionableByClass but is a: "+object.getClass().getName());
        }
        ActionPositionableByClass action = (ActionPositionableByClass)object;
        ...
        if (_tabbedPanePositionable.getSelectedComponent() == _panelPositionableDirect) {
            action.setClassName(_classNamesComboBox.getItemAt(_classNamesComboBox.getSelectedIndex()));
        }
        try {
            if (_tabbedPanePositionable.getSelectedComponent() == _panelPositionableDirect) {
                action.setAddressing(NamedBeanAddressing.Direct);
    ...
    public String toString() {
        return Bundle.getMessage("ActionPositionableByClass_Short");
    }
...
//    private final static org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ActionPositionableByClassSwing.class);
