...
public class ConsistListCellRenderer extends JLabel implements ListCellRenderer<Object> {
    ...
    public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {        
        ...
        if (value instanceof LocoAddress) {            
            ...
            setIcon(getConsistIcon(consistAddress));
        }
        return this;        
    }
    
    public static ImageIcon getConsistIcon(LocoAddress consistAddress) {
        return getConsistIcon(consistAddress, InstanceManager.getDefault(RosterIconFactory.class));
    }
    ...
    public static ImageIcon getConsistIcon(LocoAddress consistAddress, RosterIconFactory iconFact ) {
        ...
        for (DccLocoAddress loco : consist.getConsistList()) {
            ...
            if (reName != null) {
                ...
                if (dir) {
                    icon = iconFact.getIcon(reName);
                } else {
                    icon = iconFact.getReversedIcon(reName);
                }
                if ( (icon != null) && (bi.getWidth() + icon.getIconWidth() > 0) ) {                        
                    BufferedImage ti = new BufferedImage(bi.getWidth() + icon.getIconWidth(),  iconHeight, BufferedImage.TYPE_INT_ARGB);
            ...
            if (label != null) {
                ...
                BufferedImage li =  new BufferedImage( label.length()*f.getSize(),   iconHeight, BufferedImage.TYPE_INT_ARGB);
                Graphics gli = li.createGraphics();
                gli.setFont(f);
                //gli.setColor(this.getForeground());
                gli.drawString(label, 0,  iconHeight/2 + gli.getFontMetrics().getMaxAscent()/2);                    
                // expand existing buffered image
                BufferedImage ti = new BufferedImage( gli.getFontMetrics().stringWidth(label) + bi.getWidth() +2, iconHeight, BufferedImage.TYPE_INT_ARGB);
