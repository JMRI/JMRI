...
public class Engineer extends Thread implements Runnable, java.beans.PropertyChangeListener {
    ...
    public void run() {
        ...
        while (_idxCurrentCommand + 1 < _warrant._commands.size()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Start Cmd #" + (_idxCurrentCommand + 1) + " for block \"" + ts.getBlockName()
            ...
            synchronized (this) {
                ...
                try {
                    ...
                    if (!_runOnET && _syncIdx > _warrant.getCurrentOrderIndex()) {
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("Wait for train to enter \"" + ts.getBlockName()
            ...
            try {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("Cmd #" + (_idxCurrentCommand) + ": "
    ...
    protected void setRunOnET(Boolean set) {
        if (log.isDebugEnabled()) {
            log.debug("setRunOnET " + set + " command #" + (_idxCurrentCommand + 1)
                    + " warrant " + _warrant.getDisplayName());
        }
        _setRunOnET = set;
        if (!set) {
    ...
    synchronized protected void rampSpeedTo(String endSpeedType, long waitTime) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("rampSpeedTo =" + endSpeedType
        ...
        _ramp = new ThrottleRamp(endSpeedType, waitTime);
        new Thread(_ramp).start();
    }

    /**
     * When restoring speed from a halt or signaled speed restriction, get the
     * last normal speed setting. Regress through commends, if necessary.
     */
    private float getLastSpeedCommand(int currentIndex) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("getLastSpeedCommand: speed= " + speed + ", from Command #" + idx
                        + " warrant " + _warrant.getDisplayName());
            }
    ...
    synchronized public void setHalt(boolean halt) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("setHalt(" + halt + "): throttle speed= " + _throttle.getSpeedSetting()
    ...
    private void getSensor(String sensorName, String action) {
        ...
        if (_waitSensor != null) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Listen for propertyChange of "
...
//                                       (endSpeed > speed ? "increasing" : "decreasing"));
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug("rampSpeed from \"" + old + "\" to \"" + endSpeedType
            ...
            if (log.isDebugEnabled()) {
                log.debug("rampSpeed complete to \"" + endSpeedType
