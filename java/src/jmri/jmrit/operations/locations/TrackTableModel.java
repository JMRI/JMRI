...
public class TrackTableModel extends AbstractTableModel
	...
	private void initTable() {
		...
		tcm.getColumn(LENGTH_COLUMN).setPreferredWidth(
				Math.max(50, new JLabel(getColumnName(LENGTH_COLUMN))
		...
		tcm.getColumn(RESERVED_COLUMN).setPreferredWidth(
				Math.max(65, new JLabel(getColumnName(RESERVED_COLUMN))
						.getPreferredSize().width + 10));
		...
		tcm.getColumn(PICKUPS_COLUMN).setPreferredWidth(
				Math.max(60, new JLabel(getColumnName(PICKUPS_COLUMN))
						.getPreferredSize().width + 10));
		tcm.getColumn(SETOUT_COLUMN).setPreferredWidth(
				Math.max(60, new JLabel(getColumnName(SETOUT_COLUMN))
						.getPreferredSize().width + 10));
		tcm.getColumn(SCHEDULE_COLUMN).setPreferredWidth(
				Math.max(90, new JLabel(getColumnName(SCHEDULE_COLUMN))
						.getPreferredSize().width + 10));
		...
		tcm.getColumn(ORDER_COLUMN).setPreferredWidth(
				Math.max(50, new JLabel(getColumnName(ORDER_COLUMN))
						.getPreferredSize().width + 10));
		...
		tcm.getColumn(EDIT_COLUMN)
				.setCellEditor(new ButtonEditor(new JButton()));
	...
	private void setColumnsVisible() {
		...
		tcm.setColumnVisible(tcm.getColumnByModelIndex(SCHEDULE_COLUMN),
				_location.hasSchedules());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(RESTRICTION_COLUMN),
				_location.hasServiceRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(LOAD_COLUMN),
				_location.hasLoadRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(SHIP_COLUMN),
				_location.hasShipLoadRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(ROAD_COLUMN),
				_location.hasRoadRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(DESTINATION_COLUMN),
				_location.hasDestinationRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(PLANPICKUP_COLUMN),
				_location.hasPlannedPickups());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(POOL_COLUMN),
				_location.hasPools());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(ALT_TRACK_COLUMN),
				_location.hasAlternateTracks());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(ORDER_COLUMN),
				_location.hasOrderRestrictions());
		tcm.setColumnVisible(tcm.getColumnByModelIndex(REPORTER_COLUMN),
				Setup.isRfidEnabled() && _location.hasReporters());

		tcm.setColumnVisible(tcm.getColumnByModelIndex(MOVES_COLUMN),
				Setup.isShowTrackMovesEnabled());
	}
	...
	public Object getValueAt(int row, int col) {
		...
		switch (col) {
			...
			case LOAD_COLUMN :
				return getModifiedString(track.getLoadNames().length,
						track.getLoadOption().equals(Track.ALL_LOADS),
			...
			case ROAD_COLUMN :
				return getModifiedString(track.getRoadNames().length,
						track.getRoadOption().equals(Track.ALL_ROADS),
						track.getRoadOption().equals(Track.INCLUDE_ROADS));
			case DESTINATION_COLUMN : {
				int size = track.getDestinationListSize();
				if (track.getDestinationOption()
						.equals(Track.EXCLUDE_DESTINATIONS)) {
					size = InstanceManager.getDefault(LocationManager.class)
							.getNumberOfLocations() -
							size;
				}
				return getModifiedString(size,
						track.getDestinationOption()
								.equals(Track.ALL_DESTINATIONS),
						track.getDestinationOption()
								.equals(Track.INCLUDE_DESTINATIONS));
			}
			case POOL_COLUMN :
	...
	private String getRestrictions(Track track) {
		...
		if (!track.getDropOption().equals(Track.ANY)) {
			...
			restrictions.append(Bundle.getMessage("AbbreviationSetOut") +
					suffix +
					track.getDropIds().length);
		}
		if (!track.getPickupOption().equals(Track.ANY)) {
			...
			restrictions.append(" " +
					Bundle.getMessage("AbbreviationPickUp") +
					suffix +
					track.getPickupIds().length);
		}
	...
	private void setMoves(int row, Object value) {
		...
		try {
			...
			log.error("Moves ({}) for track ({}) not a number", value,
					track.getName());
		}
	}
	...
	public void propertyChange(PropertyChangeEvent e) {
		...
		if (e.getPropertyName()
				.equals(Location.TRACK_LISTLENGTH_CHANGED_PROPERTY)) {
			updateList();
			fireTableDataChanged();
		}
		if (e.getPropertyName()
				.equals(Setup.SHOW_TRACK_MOVES_PROPERTY_CHANGE)) {
			setColumnsVisible();
		}
		if (e.getSource().getClass().equals(Track.class) &&
				(e.getPropertyName().equals(Track.DROP_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.PICKUP_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.SCHEDULE_ID_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.LOADS_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.ROADS_CHANGED_PROPERTY) ||
						e.getPropertyName().equals(
								Track.DESTINATION_OPTIONS_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.POOL_CHANGED_PROPERTY) ||
						e.getPropertyName().equals(
								Track.PLANNEDPICKUPS_CHANGED_PROPERTY) ||
						e.getPropertyName().equals(
								Track.ALTERNATE_TRACK_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.SERVICE_ORDER_CHANGED_PROPERTY) ||
						e.getPropertyName()
								.equals(Track.TRACK_REPORTER_PROPERTY))) {
			setColumnsVisible();
		}
	...
	private final static Logger log = LoggerFactory
			.getLogger(TrackTableModel.class);
}
