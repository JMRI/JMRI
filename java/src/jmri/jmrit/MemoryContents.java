...
/**
 * Models (and provides utility functions for) board memory as expressed in 
 * .hex files and .DMF files.
 * <p>
 * Provides mechanisms to read and interpret firmware update files into an internal 
 * data structure.  Provides mechanisms to in create firmware update files from 
 * an internal data structure.  Provides mechanisms to allow other agents to access
 * the data in the internal data structures for the purpose of sending the data to
 * the device to be updated.  Supports the Intel "I8HEX" file format and a derivative 
 * ".dmf" file format created by Digitrax.
 * <P>
 * Support for the Intel "I8HEX" format includes support for record types "00" 
 * and "01".  The "I8HEX" format implements records with a LOAD OFFSET field of 16 
 * bits.  To support the full 24-bit addressing range provided by the LocoNet
 * messaging protocol for firmware updates, this class is able to interpret 
 * record type "04" (Extended Linear Address) records for input files with 16-bit 
 * LOAD OFFSET fields.  Record type "04" are typically found in the Intel "I32HEX" 
 * 32-bit addressing format.  Because the class supports only 24 bits of address, 
 * interpretation of the "04" record type requires that the upper 8 bits of the 
 * 16-bit data field be 0.
 * <p>
 * Support for some .hex files emitted by some tool-sets requires support for the
 * Extended Segment Address record type (record type "02"), which may be used in 
 * I16HEX format files.  This version of the <code>readHex</code> method supports 
 * the Extended Segment Address record type ONLY when the segment specified in 
 * the data field is 0x0000.
 * <p>
 * Support for the Digitrax ".DMF" format is an extension to the "I8HEX" support.
 * This extension supports interpretation of the 24-bit LOAD OFFSET fields used 
 * in .DFM files.  The class does not allow files with 24-bit LOAD OFFSET fields 
 * to use the "04" (Extended Linear Address) record type unless its data field 
 * is 0x0000.
 * <p>
 * Support for the ".DMF" format allows capture of Key/Value pairs which may be
 * embedded in special comments within a .DMF file.  This support is enabled 
 * for I8HEX files.
 * <p>
 * The class treats the information within a file's records as having "big-endian"
 * address values in the record LOAD OFFSET field.  The INFO or DATA field 
 * information is interpreted as 8-bit values, with the left-most value in the 
 * INFO or DATA field corresponding to the address specified by the record's LOAD 
 * OFFSET field plus the influence of the most recent previous Extended Linear 
 * Address record, if any.  
 * <p>
 * The INFO or DATA field for Extended Linear Address records is interpreted as 
 * a big-endian value, where bits 7 thru 0 of the data field value are 
 * used as bits 23 thru 16 of the effective address, while bits 15 thru 0 of the
 ...
 * <code>writeHex</code>.  The mechanisms implemented within this 
 * class do not know about or care about the filename or its extension and do 
 * not use that information as part of its file interpretation or file creation.
 * <p>
...
public class MemoryContents {
     ...
     * within a .DMF or .hex file
     */
    private ArrayList<String> keyValComments = new ArrayList<String>(1);
    
    /**
     * Defines the LOAD OFFSET field type used/expected for records in "I8HEX" and 
     * ".DMF" file formats.
     * <p>
     ...
     * the incoming data to help ensure proper interpretation of the file and 
     * to help detect corrupted files.  Extracts "key/value" pair information from
     ...
     * <li>Verification that all subsequent records use the same LOAD OFFSET field type
     ...
     * <li>Identification of any records which have extra characters after the checksum
     * <ul>
     * When reading the file, <code>readHex</code> infers the addressing 
     * format from the first record found in the file, and future records are 
     * interpreted using that addressing format.  It is not necessary to pre-configure the
     * addressing format before reading the file.  This is a departure from previous
     ...
     * <p>
     * Comment lines which * begin with '!' may encode Key/Value pair information.  
     * Such Key/Value pair information is used within the .DMF format to provide 
     * configuration information for firmware update mechanism.  This class also
     * extracts key/value pair comments "I8HEX" format files.  After successful
     * completion of this <code>readHex</code>, the the <code>getValue(String 
     ...
     * @throws MemoryFileRecordLengthException if a record line is too long or short
     ...
     *                                      record type
     * @throws MemoryFileRecordContentException if a record contains inappropriate
     *                                          characters
     * @throws MemoryFileNoEOFRecordException if a file does not contain an EOF 
     *                                          record
     * @throws MemoryFileNoDataRecordsException if a file does not contain any data 
     *                                          records
     * @throws MemoryFileRecordFoundAfterEOFRecord if a file contains records after
     *                                              the EOF record
     * @throws MemoryFileAddressingRangeException if a file contains an Extended
     *                                              Linear Address record outside
     *                                              of the supported address range
    ...
    public void readHex(File file) throws FileNotFoundException , 
        ...
        } catch (IOException e) { log.error("Exception reading file",e);} // NOI18N
        finally { 
            try {
                fileStream.close();
            } catch (IOException e2) { log.error("Exception closing file", e2); } // NOI18N
        }
        if (!foundDataRecords) {
     ...
     * <p>
     * This method generates only RECTYPs "00" and "01", and does not 
     ...
     * <p>
     * The "I8HEX" format is used when the <code>loadOffsetFieldType</code> is configured for 
     * 16-bit addresses in the record LOAD OFFSET field.  The ".DMF" format is used 
     * when the <code>loadOffsetFieldType</code> is configured for 24-bit addresses in the record
     * LOAD OFFSET field.
     * <p>
    ...
    /**
     * Sends a character stream of an image of a programmatic representation of 
     * memory in either the Intel "I8HEX" or Digitrax ".DMF" file format to a Writer.
     * <p>
     * The "I8HEX" format is used when the <code>loadOffsetFieldType</code> is configured for 
     * 16-bit addresses in the record LOAD OFFSET field.  The ".DMF" format is used 
     * when the <code>loadOffsetFieldType</code> is configured for 24-bit addresses in the record
     ...
     * 
     * @param location indicates the address from which the next location is determined
     ...
     * value.
     * 
     * @param location location within programmatic representation of memory to modify
     * @param value value to be placed at location within programmatic representation of memory
     */
    ...
    /**
     * Queries the programmatic representation of memory to determine if location
     * is represented.
     * 
     * @param location location within programmatic representation of memory to inspect
     * @return true if location exists within programmatic representation of memory
     */
     ...
     * 
     * @param location location within programmatic representation of memory to report
     ...
     * <li>the length of recordString is not appropriate to define an integral number of
     * bytes
     * <li>the recordString checksum does not match a checksum computed for the
     * recordString
     * <li>if the record type extracted after inferring the addressing type is an 
     * unsupported record type
     * <li>if the length of recordString did not match the length expected for the 
     * inferred addressing type.
     * <ul>
     * 
     ...
     * of bytes.
     * <p>
     * When used to calculate the checksum of a record in I8HEX or similar format,
     * the infoString parameter is expected to include only those characters 
     * which are used for calculation of the checksum.  The "record mark" at the
     * beginning of a record should not be included in the infoString.  Similarly,
     * the checksum at the end of a record should generally not be included in 
     * the infoString.
     * <p>
     ...
     * 
     * @param infoString a string of characters for which the checksum is calculated
     ...
     * @return true if string of bytes will not cross into another page,
     *              else false.
     */
    ...
    /**
     * Finds the Value for a specified Key if that Key is found in the list
     * of Key/Value pair comment lines.  The list of Key/Value pair comment 
     * lines is created while the input file is processed.
     * <p>
    ...
    /**
     * Finds the index of the specified key within the array
     * containing key/value comments
     * 
     * @param keyName  Key to search for in the internal storage
     * @return      index in the arraylist for the specified key, or -1 if
    ...
    /**
     * Updates the internal key/value storage to reflect the
     * parameters.  If the key already exists, its value is updated
     * based on the parameter.  If the key does not exist, a new key/value
     * pair comment is added to the key/value storage list.
     * 
     * @param keyName
     * @param value 
     */
     ...
     * <p>
     * Since the key/value metadata is typically presented at the 
     * beginning of a firmware file, the method would typically be
     * invoked before invocation of the writeHex method.
     * <p>
     * @param writer Writer to which the character stream is sent
     * @throws IOException 
     */
    ...
    private int extractRecLen(String line) {
        return Integer.valueOf(line.substring(CHARS_IN_RECORD_MARK, 
            CHARS_IN_RECORD_MARK+CHARS_IN_RECORD_LENGTH)
            ,16).intValue();
    }
    ...
    /**
     * An exception for a record which has content which cannot be parsed. <p>
