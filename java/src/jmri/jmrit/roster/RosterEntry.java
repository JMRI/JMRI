...
//         loadSounds(e.getChild("soundlabels"), "RosterEntry");
    ...
    /**
     * Returns a list of system names of blocks which this roster entry appears to
     ...
     */
    public List<String> getBlocksIOccupy() {
        List<String> blockList = new ArrayList<>();
        BlockManager bm = jmri.InstanceManager.blockManagerInstance();
        
        for (String sysName : bm.getSystemNameList()) {
            Block b = bm.getBySystemName(sysName);
            Object o = b.getValue();
            if (o != null) {
                if (o instanceof jmri.jmrit.roster.RosterEntry && o == this) {
                    blockList.add(b.getSystemName());
                } else if (o.toString().equals(_id) || o.toString().equals(_dccAddress)){
                    blockList.add(b.getSystemName());
                }
            }
        }
    ...
    /**
     * Returns a list of user names of layout blocks which this roster entry appears to
     ...
     */
    public List<String> getLayoutBlocksIOccupy() {
        List<String> blockList = getBlocksIOccupy();
        List<String> layoutBlockList = new ArrayList<>();
        BlockManager bm = jmri.InstanceManager.blockManagerInstance();
        LayoutBlockManager lbm = jmri.InstanceManager.getDefault(LayoutBlockManager.class);
        
        for (String sysName : blockList ) {
            Block block = bm.getBlock(sysName);
            if (block.getUserName() != null) {
                LayoutBlock lb = lbm.getByUserName(block.getUserName());
                if (lb != null) {
                    layoutBlockList.add(block.getUserName());
                }
            }
        }
