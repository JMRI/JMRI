...
public class AllocationPlan {
	 ...
	 * Access methods
	 */
	protected int getPlanNum() {return _planNum;}  
	protected int getPlanType() {return _planType;}
	protected void setPlanType(int type) {_planType = type;}
	protected ActiveTrain getActiveTrain(int i) {
		if (i==1) return _atOne; 
		else if (i==2) return _atTwo;
		return null;
	}
	protected void setActiveTrain(ActiveTrain at, int i) {
		if (i==1) _atOne = at;
		else if (i==2) _atTwo = at;
		else {
			log.error("out of range index argument in call to 'setActiveTrain'");
		}
	}
	protected Section getTargetSection(int i) {
		if (i==1) return _tSectionOne; 
		else if (i==2) return _tSectionTwo;
		return null;
	}
	...
	protected int getTargetSectionSequenceNum(int i) {
		if (i==1) return _tSectionOneSeq; 
		else if (i==2) return _tSectionTwoSeq;
		return 0;
	}
	...
	protected boolean isComplete() {
		if ( (_atOne==null) || (_atTwo==null) ) return false;
		...
		for (int i = 0; i<aSections.size(); i++) {
			if ( (aSections.get(i).getSection() == _tSectionOne) &&
					(aSections.get(i).getSequence() == _tSectionOneSeq) ) complete = true;
		}
		...
		for (int j = 0; j<aSections.size(); j++) {
			if ( (aSections.get(j).getSection() == _tSectionTwo) &&
					(aSections.get(j).getSequence() == _tSectionTwoSeq) ) complete = true;
		}
		return complete;
	}
    ...
    static Logger log = Logger.getLogger(AllocationPlan.class.getName());
}
