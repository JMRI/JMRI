 ...
 *	  TrainInfo file is read when the user request it in the Activate New Train window
 *
 * <P>
...
public class TrainInfoFile extends jmri.jmrit.XmlFile {
	...
	/**
	 * Get the names of all current TrainInfo files
	 * Returns names as an array of Strings. Returns an empty array if no files are present.
	 * Note: Fill names still end with .xml or .XML.
	 *  (Modeled after a method in RecreateRosterAction.java by Bob Jacobsen)
	 */ 
    public String[] getTrainInfoFileNames() {
        ...
        String[] sp = null;
        XmlFile.ensurePrefsPresent(fileLocation);
        if (log.isDebugEnabled()) log.debug("directory of TrainInfoFiles - "+fileLocation);
