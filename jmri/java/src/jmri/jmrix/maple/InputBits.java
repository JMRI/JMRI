 ...
 * <P>
 * All of the Maple HMI panels are configured with the same input bits.  As each HMI is polled,
 *		the results are ORed together in an input array that is initialized to all 0 when a polling
 *		cycle is initiated.  That way, if a bit is 1 in any of the HMI's, it will be 
 *		1 in the input array at the end of the polling cycle.
 * At the end of each polling cycle, each input bit is compared to the input bit from the last polling
 *		cycle.  If a bit has changed, or if this is the first polling cycle, the correspnding Sensor 
 *		state is updated. No updating occurs if all panels timed out.
 * Serial systems with unique input bits for each node keep their input array in each node.  That 
...
public class InputBits {
	...
	// access routines
	public static void setNumInputBits(int n) {mNumInputBits = n;}
	public static int getNumInputBits() {return mNumInputBits;}
	public static void setTimeoutTime (int n) {mTimeoutTime = n;}
	public static int getTimeoutTime () {return mTimeoutTime;}
	public int getLastSensor() {return lastUsedSensor;}
	 ...
	 * Bits from all the polled panels are ORed together.  So if any panel has the 
	 *		bit on (after any inversion is provided for), the resulting bit will be on
	 *		when the polling cycle is complete.
    ...
    public void markChanges(SerialReply l) {
		...
		for (int i=0; i<count; i++) {
			if (sensorArray[i+begAddress-1] == null) continue; // skip ones that don't exist
    ...
    /**
     * This routine is invoked at the end of each polling cycle to change those Sensors
	 *		that have changed during the polling cycle.
     * After making whatever changes are warranted, this routine clears the array for 
	 *		accumulating a new polling cycle.
	 * Only sensors that are actually defined are updated
     */
    public void makeChanges() {
        ...
        } catch (JmriException e) { log.error("exception in makeChanges: "+e); }
		
		// clear the accumulation array;
		for (int i=0; i<lastUsedSensor; i++) sensorORedSetting[i] = false;
    }
