 ...
 //       			log.debug("list out road number isn't a number");
    ...
    public List getCarsByLocationList() {
    	...
    	for (int i=0; i<sortByRoad.size(); i++){
    		carAdded = false;
    		c = getCarById ((String)sortByRoad.get(i));
    		carLocation = c.getLocationName()+c.getSecondaryLocationName();
    		for (int j=0; j<out.size(); j++ ){
    			c = getCarById ((String)out.get(j));
    			String outCarLocation = c.getLocationName()+c.getSecondaryLocationName();
    ...
    public List getCarsByDestinationList() {
    	...
    	for (int i = 0; i < sortByLocation.size(); i++) {
			...
			carDestination = c.getDestinationName()+c.getSecondaryDestinationName();
			for (int j = 0; j < out.size(); j++) {
				c = getCarById((String) out.get(j));
				String outCarDestination = c.getDestinationName()+c.getSecondaryDestinationName();
    ...
    public List getCarsByTrainList() {
    	// first get by road list
    	List sortByRoad = getCarsByLocationList();
    ...
    public List getCarsByTrainList(Train train) {
    	...
    	for (int i = 0; i < inTrain.size(); i++) {
    		...
    		String carDestination = car.getSecondaryDestinationName();
    		for (int j = 0; j < out.size(); j++) {
    			Car carOut = getCarById ((String)out.get(j));
    			String carOutDest = carOut.getSecondaryDestinationName();
