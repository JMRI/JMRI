...
public class PaneProgPane extends javax.swing.JPanel
     ...
     * @param modelElem "model" element from the Decoder Index, used to check what decoder options are present.
     */
    public PaneProgPane(String name, Element pane, CvTableModel cvModel, IndexedCvTableModel icvModel, VariableTableModel varModel, Element modelElem) {
    ...
    void setToRead(boolean stat) {
        ...
        for (int i=0; i<indxCvList.size(); i++) {
            CvValue icv = _indxCvModel.getCvByRow(i);
            icv.setToRead(stat);
        }
    }
    ...
    boolean nextRead() {
        ...
        for (int i=0; i<varList.size(); i++) {
            ...
            VariableValue var = _varModel.getVariable(varNum);
            if (var.isToRead()) {
                if (!justChanges || (justChanges && (var.isChanged()))) {
                    if (log.isDebugEnabled()) log.debug(
                        "start read of variable " + _varModel.getLabel(varNum));
                    executeRead(var);

                    if (log.isDebugEnabled()) log.debug(
                        "return from starting var read");
        ...
        for (int i=0; i<cvList.size(); i++) {
            int cvNum = ( (Integer) cvList.get(i)).intValue();
            CvValue cv = _cvModel.getCvByRow(cvNum);
            if (log.isDebugEnabled()) log.debug("nextRead cv index " + cvNum +
                                                " state " + cv.getState());
            if (cv.isToRead()) {
                if (!justChanges || (justChanges && (VariableValue.considerChanged(cv)))) {
                    if (log.isDebugEnabled()) log.debug("start read of cv " +
                        cvNum);
                    setBusy(true);
                    if (_programmingCV != null) log.error(
    ...
    public JPanel newColumn(Element element, boolean showStdName, Element modelElem) {
        ...
        for (int i=0; i<elemList.size(); i++) {
            ...
            if (name.equals("display")) { // its a variable
                ...
                JTable indxcvTable   = new JTable(_indxCvModel);
